...
C EQUILIBRIUM ARGUEMENT IS REFERENCED TO THE GRENWICH MERIDIAN
        open(unit=64,file='1950_2199/'//trim(CNAME(IC))//'_nodal.txt',
     &  access='append')

        IF(IYR.EQ.1950) THEN
        WRITE(64,'(A4,A)')CNAME(IC),' degrees/h:'
        WRITE(64,*)SPEED(IC)
        WRITE(64,'(A)')'Year M D h m s  V0+u(deg)     f'
        ENDIF         


        WRITE(64,2002)IYR,' 1 1 0 0 0 ',GRTERM(IC),'  0.  ',NODFAC(IC) 
        close(64)

        WRITE(11,2001) CNAME(IC),NODFAC(IC),GRTERM(IC)
 2001   FORMAT(1X,A4,2x,F7.5,4x,F7.2,2x,F7.4)
 2002   FORMAT(I4,A11,F8.3,A6,F7.4)
   20   CONTINUE

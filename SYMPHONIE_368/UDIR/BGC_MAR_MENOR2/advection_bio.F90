...
! SCHEMA QUICKEST
                             ( & !-quickest->
! Terme C2
                             0.5*anyv3d(i,j,k1,id_webio)*( &
                                                     anyv3d(i,j,k  ,id_biobefo) &
                                                    +anyv3d(i,j,km1,id_biobefo) &
                                                    ) &
! terme CURV
        -(1.-anyv3d(i,j,k,id_cnk)**2)*(1./6.)*( & !CURV

          0.5*(anyv3d(i,j,k1,id_webio)+abs(anyv3d(i,j,k1,id_webio)))*( & ! courant  positif >
            (anyv3d(i,j,k  ,id_biobefo)-anyv3d(i,j,km1,id_biobefo)) &
           -(anyv3d(i,j,km1,id_biobefo)-anyv3d(i,j,km2,id_biobefo))*mask_t(i,j,kmax)  &
                                                           ) & ! courant  positif >
                                       
         +0.5*(anyv3d(i,j,k1,id_webio)-abs(anyv3d(i,j,k1,id_webio)))*( & ! courant  negatif >
            (anyv3d(i,j,kp1,id_biobefo)-anyv3d(i,j,k  ,id_biobefo))*mask_t(i,j,kmax)  &
           -(anyv3d(i,j,k  ,id_biobefo)-anyv3d(i,j,km1,id_biobefo)) &
                                                           ) & ! courant  negatif >
                                              ) & !CURV

! terme GRAD
       -anyv3d(i,j,k,id_cnk)*0.5*abs(anyv3d(i,j,k1,id_webio))*(anyv3d(i ,j,k,id_biobefo)-anyv3d(i,j,km1,id_biobefo)) &

                             ) & !-quickest->
#endif
#ifdef bidon
!SCHEMA C2+(cn**2)*GRAD + limiteur de valeurs negatives
                             ( & !-c2lim->
! Terme C2 
                             0.5*anyv3d(i,j,k1,id_webio)*( &
                                                     anyv3d(i,j,k  ,id_biobefo) &
...
! Limiteur valeurs negatives !18-11-23
! L'expression suivante est egale A 1 si traceur positif et egale A 0 sinon 
         *(0.5+sign(0.5, (abs(anyv3d(i,j,k1,id_webio))+anyv3d(i,j,k1,id_webio))*anyv3d(i,j,km1,id_biobefo)   &
                        +(abs(anyv3d(i,j,k1,id_webio))-anyv3d(i,j,k1,id_webio))*anyv3d(i,j,k  ,id_biobefo))) &

! L'expression suivante est nulle si traceur positif et est un flux upwind si traceur negatif
      +0.25*( (anyv3d(i,j,k1,id_webio)+abs(anyv3d(i,j,k1,id_webio)))*(anyv3d(i,j,km1,id_biobefo)-abs(anyv3d(i,j,km1,id_biobefo))) &
             +(anyv3d(i,j,k1,id_webio)-abs(anyv3d(i,j,k1,id_webio)))*(anyv3d(i,j,k  ,id_biobefo)-abs(anyv3d(i,j,k  ,id_biobefo)))) 
#endif
!#ifdef bidon
...
!....................................................
! SCHEMA 1
! SCHEMA QUICKEST AVEC LIMITEUR DE VALEURS NEGATIVES:
        anyv3d(i,j,k,id_fli)= ( & !-quickest->
! Terme C2
                             0.5*veldydz_u(i,j,k,1)*( &
                                                     anyv3d(i  ,j,k,id_biobefo) &
                                                    +anyv3d(i-1,j,k,id_biobefo) &
                                                    ) &
! terme CURV
        -(1.-anyv3d(i,j,k,id_nci)**2)*(1./6.)*( & !CURV

          0.5*(veldydz_u(i,j,k,1)+abs(veldydz_u(i,j,k,1)))*( & ! courant  positif >
            (anyv3d(i  ,j,k,id_biobefo)-anyv3d(i-1,j,k,id_biobefo)) &
           -(anyv3d(i-1,j,k,id_biobefo)-anyv3d(i-2,j,k,id_biobefo))*mask_t(i-2,j,kmax)  &
                                                           ) & ! courant  positif >
                                       
         +0.5*(veldydz_u(i,j,k,1)-abs(veldydz_u(i,j,k,1)))*( & ! courant  negatif >
            (anyv3d(i+1,j,k,id_biobefo)-anyv3d(i  ,j,k,id_biobefo))*mask_t(i+1,j,kmax)  &
           -(anyv3d(i  ,j,k,id_biobefo)-anyv3d(i-1,j,k,id_biobefo)) &
                                                           ) & ! courant  negatif >
                                              ) & !CURV

! terme GRAD
       -anyv3d(i,j,k,id_nci)*0.5*abs(veldydz_u(i,j,k,1))*(anyv3d(i ,j,k,id_biobefo)-anyv3d(i-1,j,k,id_biobefo)) &

                              ) & !-quickest->
! Limiteur valeurs neg
! L'expression suivante est egale A 1 si traceur positif et egale A 0 sinon 
      *(0.5+sign(0.5, (abs(veldydz_u(i,j,k,1))+veldydz_u(i,j,k,1))*anyv3d(i-1,j,k,id_biobefo)   &
                     +(abs(veldydz_u(i,j,k,1))-veldydz_u(i,j,k,1))*anyv3d(i  ,j,k,id_biobefo))) &

! L'expression suivante est nulle si traceur positif et est un flux upwind si traceur negatif
      +0.25*( (veldydz_u(i,j,k,1)+abs(veldydz_u(i,j,k,1)))*(anyv3d(i-1,j,k,id_biobefo)-abs(anyv3d(i-1,j,k,id_biobefo))) &
             +(veldydz_u(i,j,k,1)-abs(veldydz_u(i,j,k,1)))*(anyv3d(i  ,j,k,id_biobefo)-abs(anyv3d(i  ,j,k,id_biobefo)))) 
!....................................................
#endif
...
! de la contrainte precedente, agit auss en différé sur des oscillations existante !09-02-24
      +max(0.d0,sign(1.d0,                         &
       (abs(veldydz_u(i,j,k,1))+veldydz_u(i,j,k,1))            &
...
! SCHEMA 1
! SCHEMA QUICKEST AVEC LIMITEUR DE VALEURS NEGATIVES:
        anyv3d(i,j,k,id_flj)=( & !-quickest->
! Terme C2
                             0.5*veldxdz_v(i,j,k,1)*( &
                                                     anyv3d(i  ,j,k,id_biobefo) &
                                                    +anyv3d(i,j-1,k,id_biobefo) &
                                                    ) &
! terme CURV
        -(1.-anyv3d(i,j,k,id_ncj)**2)*(1./6.)*( & !CURV

          0.5*(veldxdz_v(i,j,k,1)+abs(veldxdz_v(i,j,k,1)))*( & ! courant  positif >
            (anyv3d(i  ,j,k,id_biobefo)-anyv3d(i,j-1,k,id_biobefo)) &
           -(anyv3d(i,j-1,k,id_biobefo)-anyv3d(i,j-2,k,id_biobefo))*mask_t(i,j-2,kmax)  &
                                                           ) & ! courant  positif >
                                       
         +0.5*(veldxdz_v(i,j,k,1)-abs(veldxdz_v(i,j,k,1)))*( & ! courant  negatif >
            (anyv3d(i,j+1,k,id_biobefo)-anyv3d(i  ,j,k,id_biobefo))*mask_t(i,j+1,kmax)  &
           -(anyv3d(i  ,j,k,id_biobefo)-anyv3d(i,j-1,k,id_biobefo)) &
                                                           ) & ! courant  negatif >
                                              ) & !CURV

! terme GRAD
       -anyv3d(i,j,k,id_ncj)*0.5*abs(veldxdz_v(i,j,k,1))*(anyv3d(i ,j,k,id_biobefo)-anyv3d(i,j-1,k,id_biobefo)) &

                             ) & !-quickest->
! Limiteur valeurs negatives
! L'expression suivante est egale A 1 si traceur positif et egale A 0 sinon 
         *(0.5+sign(0.5, (abs(veldxdz_v(i,j,k,1))+veldxdz_v(i,j,k,1))*anyv3d(i,j-1,k,id_biobefo)   &
                        +(abs(veldxdz_v(i,j,k,1))-veldxdz_v(i,j,k,1))*anyv3d(i  ,j,k,id_biobefo))) &

! L'expression suivante est nulle si traceur positif et est un flux upwind si traceur negatif
      +0.25*( (veldxdz_v(i,j,k,1)+abs(veldxdz_v(i,j,k,1)))*(anyv3d(i,j-1,k,id_biobefo)-abs(anyv3d(i,j-1,k,id_biobefo))) &
             +(veldxdz_v(i,j,k,1)-abs(veldxdz_v(i,j,k,1)))*(anyv3d(i  ,j,k,id_biobefo)-abs(anyv3d(i  ,j,k,id_biobefo)))) 
!....................................................
#endif

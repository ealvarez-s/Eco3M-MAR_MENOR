...
! release 320 - last update: 03-01-22
!______________________________________________________________________
...
!.................................................................
      ...
      use module_parallele
      implicit none
! note: le pas de temps est specifique de i,j et de la vitesse de chute !03-01-22
      double precision :: dti_fwsub_                                    !03-01-22
...
!..............................................
      do j=1,jmax ; do i=1,imax ! boucles i j no 2
       ...
       dti_fwsub_=dti_fw/ceiling(xy_t(i,j,1)) !03-01-22


       do k=kmerged_t(i,j)+1,kmax !anciennement 2,kmax

! Nombre de courant vis a vis de omega
       x0=min(1.,   abs(anyv3d(i,j,k,id_webio))*dti_fwsub_     & !03-01-22
...
!      do k=kmin_w(i,j),kmax ! anciennement 1,kmax  ! boucle k no 2
       ...
       x3=dti_fwsub_/(0.5*(dz_t(i,j,k,0)   & !03-01-22
                          +dz_t(i,j,k,1)))
...
!         if(i+par%timax(1)==541.and.j+par%tjmax(1)==833.and.vb==27) then
!           write(10+par%rank,*)loop_,loopmaxbio_v_,' verticale loop_,loopmaxbio_v_'
!         endif
...
!         if(i+par%timax(1)==541.and.j+par%tjmax(1)==833.and.vb==27) then
!           write(10+par%rank,*)k,anyv3d(i  ,j  ,k  ,id_bioaftr),' surface tot'
!           write(10+par%rank,*)real(anyv3d(i,j,k,13:17))
!           write(10+par%rank,*)anyv3d(i,j,k,12),' terme 12 div'
!         endif
...
!         if(i+par%timax(1)==541.and.j+par%tjmax(1)==833.and.vb==27) then
!           write(10+par%rank,*)k,anyv3d(i  ,j  ,k  ,id_bioaftr),' verticale'
!         endif
...
!         if(i+par%timax(1)==541.and.j+par%tjmax(1)==833.and.vb==27) then
!           write(10+par%rank,*)k,anyv3d(i,j,k,id_bdiv) &
!                            /(0.5*(dz_t(i,j,k,0)+dz_t(i,j,k,1)))  & 
!                                ,'divergence'
!         endif
...
! Equivalent mais plus compact
                        +(veldydz_u(i+1,j,k,1)-veldydz_u(i,j,k,1) &
                            ...
                            + anyv3d(i  ,j,k,1)                   *anyv3d(i-2,j,k,id_biobefo) &
                                       ...
                                       /(0.5*(dz_t(i,j,k,0)+dz_t(i,j,k,1))) !05-11-21
...
! Cumul du terme de correction de divergence: (A oter A la toute fin de l'advection)
       ...
       call my_outputs_zone2bioflux('i_bio',0)
       call my_outputs_zone3bioflux('i_bio',0)
       call my_outputs_zone4bioflux('i_bio',0)
#endif
...
! Cumul du terme de correction de divergence: (A oter A la toute fin de l'advection)
       ...
       call my_outputs_zone2bioflux('j_bio',0)
       call my_outputs_zone3bioflux('j_bio',0)
       call my_outputs_zone4bioflux('j_bio',0)
#endif
...
!     write(66,*)'par%rank=',par%rank
      ...
      do j=1,jmax ; do i=1,imax
       ...
       do k=kmin_w(i,j)+1,kmax
!       if(par%rank==231)write(66,*)'i,j,k,vb=',i,j,k,vb,i+par%timax(1),j+par%tjmax(1)
        anyv3d(i,j,k,3)=                                                           &

...
! release 289 - last update: 24-09-20
!...............................................................................
...
!      if(elapsedtime_now+dti_fw>=elapsedtime_end)call my_outputs_mean_z_profile
!      if(int(elapsedtime_now/2592000)-int((elapsedtime_now-dti_fw)/2592000)==1.or.iteration3d==0)call my_outputs_mean_z_profile
...
!............................................................................
      ...
      REAL*8 lon_glider,lat_glider,z_glider
...
!      write(10+par%rank,*)'DATE MODELE',month_now,day_now,hour_now,minute_now,second_now
      ...
      if(flag_glider==0) open(unit=543,file='glider_in.txt')
      flag_glider=1
...
!     write(10+par%rank,*)'------DANS GLIDER je suis dans le dom'
       ...
       if(elapsedtime_now       < elapsedtime_glider_now_.and.  &
         ...
         open(unit=4,file='tmp/glider_out.txt',position='append')
          write(4,'(3(1x,e14.7),12(1x,i4),2(1x,e14.7))') &
...
! et le bilan ne sera pas équilibrer
      ...
      name_poly='Bilan_symphonie/Poly1_Levant.kml'
       call polygon_in(ichoix,irep,xlon,xlat,name_poly)
       ichoix=1
       do i=0,imax+1
       do j=0,jmax+1
       call polygon_in(ichoix,irep,lon_t(i,j)*rad2deg,lat_t(i,j)*rad2deg,name_poly)
       if(irep==1) zone1_mask(i,j)=1*mask_t(i,j,kmax)
       enddo
       enddo

      ichoix=0 ; xlon=0. ; xlat=0.
      name_poly='Bilan_symphonie/Poly2_Egee.kml'
       call polygon_in(ichoix,irep,xlon,xlat,name_poly)
       ichoix=1
       do i=0,imax+1
       do j=0,jmax+1
       call polygon_in(ichoix,irep,lon_t(i,j)*rad2deg,lat_t(i,j)*rad2deg,name_poly)
       if(irep==1) zone1_mask(i,j)=2*mask_t(i,j,kmax)
       enddo
       enddo

      ichoix=0 ; xlon=0. ; xlat=0.
      name_poly='Bilan_symphonie/Poly5_Adria.kml'
       call polygon_in(ichoix,irep,xlon,xlat,name_poly)
       ichoix=1
       do i=0,imax+1
       do j=0,jmax+1
       call polygon_in(ichoix,irep,lon_t(i,j)*rad2deg,lat_t(i,j)*rad2deg,name_poly)
       if(irep==1) zone1_mask(i,j)=3*mask_t(i,j,kmax)
       enddo
       enddo

      ichoix=0 ; xlon=0. ; xlat=0.
      name_poly='Bilan_symphonie/Poly6_WMed.kml'
       call polygon_in(ichoix,irep,xlon,xlat,name_poly)
       ichoix=1
       do i=0,imax+1
       do j=0,jmax+1
       call polygon_in(ichoix,irep,lon_t(i,j)*rad2deg,lat_t(i,j)*rad2deg,name_poly)
       if(irep==1) zone1_mask(i,j)=4*mask_t(i,j,kmax)
       enddo
       enddo
...
! Messine
      ...
      do j1=430,434
       ...
       if(i>=0.and.i<=imax+1.and.j>=0.and.j<=jmax+1)then
         zone1_mask(i,j)=5*mask_t(i,j,kmax)
       endif
      enddo
      enddo
!#endif
...
! salt
           ...
           *(omega_w(i,j,kmax+1,1)-0.5*(omega_evaprec_w(i,j,0)+omega_evaprec_w(i,j,1))) & !isole w_surf wetdry
           *sal_t(i,j,kmax,2)
! temp
           zone1tempflux_w=zone1tempflux_w  &
                         -max(-zone1_mask(i,j),0) & ! selectionne zone1_mask=-1
           *dti_fw*dxdy_t(i,j)*mask_i_w(i)*mask_j_w(j)*( & !m°v°m>
            ( omega_w(i,j,kmax+1,1)*tem_t(i,j,kmax,2)-omega_w(i,j,1,1)*tem_t(i,j,1,2) ) &
                             -1./( & !pmx>                   ! 2.Dt / !24-04-16
...
!............................................................................
!#ifdef bidon
      subroutine my_outputs_zone2salttempflux(txt_,id_flux_) !13-04-20
      implicit none
      integer id_flux_
      character(len=*)txt_
      character*100 name_poly
...
!      zone2_stretch_dz=0.5 ! epaisseur augmentant avec la profondeur (en surface 1/zone2_inv_dz)

! ICI CHACUN SE DEBROUILLE POUR LE LIRE SON FICHIER DE MASQUE AVEC LE
! BON FORMAT.....
!        allocate(glob_mask(0:iglb+1,0:jglb+1)) ; glob_mask=0
!        zone2_max=maxval(glob_mask)
!        allocate(zone2saltflux_glb(0:zone2_max)) ; zone2saltflux_glb=0.
!        allocate(zone2_mask(0:imax+1,0:jmax+1)) ; zone2_mask=0
!        do j=0,jmax+1 ; do i=0,imax+1
!          zone2_mask(i,j)=glob_mask(i+par%timax(1),j+par%tjmax(1))*mask_t(i,j,kmax)
!        enddo       ; enddo
!      deallocate(glob_mask)
! BIDOUILLE PATRICK
         allocate(zone2_mask(0:imax+1,0:jmax+1)) ; zone2_mask=0
...
! et le bilan ne sera pas équilibrer
      ...
      name_poly='Bilan_symphonie/Poly1_Levant.kml'
       ...
       do j=0,jmax+1
       call polygon_in(ichoix,irep,lon_t(i,j)*rad2deg,lat_t(i,j)*rad2deg,name_poly)
       if(irep==1) zone2_mask(i,j)=-1*mask_t(i,j,kmax)
       enddo
       enddo
      ...
      name_poly='Bilan_symphonie/Poly2_Egee.kml'
       ...
       do j=0,jmax+1
       call polygon_in(ichoix,irep,lon_t(i,j)*rad2deg,lat_t(i,j)*rad2deg,name_poly)
       if(irep==1) zone2_mask(i,j)=2*mask_t(i,j,kmax)
       enddo
       enddo
!#endif
         ...
         allocate(zone2saltflux_glb (0:zone2_max+1,0:zone2_nlayer-1))   ; zone2saltflux_glb=0.
         allocate(zone2saltflux_u   (0:zone2_max  ,0:zone2_nlayer-1))   ; zone2saltflux_u=0.
         allocate(zone2saltflux_v   (0:zone2_max  ,0:zone2_nlayer-1))   ; zone2saltflux_v=0.
         allocate(zone2tempflux_glb (0:zone2_max+1,0:zone2_nlayer-1))   ; zone2tempflux_glb=0.
         allocate(zone2tempflux_u   (0:zone2_max  ,0:zone2_nlayer-1))   ; zone2tempflux_u=0.
         allocate(zone2tempflux_v   (0:zone2_max  ,0:zone2_nlayer-1))   ; zone2tempflux_v=0.
         allocate(zone2waterflux_glb(0:zone2_max+1,0:zone2_nlayer-1))   ; zone2waterflux_glb=0. !08-07-19
         ...
         allocate(zone2tempcumul_glb (0:zone2_nlayer-1))   ; zone2tempcumul_glb=0. !22-09-20 v289
         allocate(zone2tempcumul_loc (0:zone2_nlayer-1))   ; zone2tempcumul_loc=0.
         allocate(zone2tempmasst0    (0:zone2_nlayer-1))   ; zone2tempmasst0=0.

         allocate(zone2saltcumul_glb (0:zone2_nlayer-1))   ; zone2saltcumul_glb=0.
         allocate(zone2saltcumul_loc (0:zone2_nlayer-1))   ; zone2saltcumul_loc=0.
         allocate(zone2saltmasst0    (0:zone2_nlayer-1))   ; zone2saltmasst0=0.

         allocate(zone2watercumul_glb (0:zone2_nlayer-1))   ; zone2watercumul_glb=0.
...
! Identifier points de flux v
      ...
      open(unit=3,file=trim(tmpdirname)//'messages',position='append')
      write(3,*)'-----------------------------------------------------'
      write(3,*)'subroutine my_outputs_zone2salttempflux'
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone2_nlayer-1 pour z=zmax_zone2_nlayer.
            k3=min(max(int(   &
                           abs(depth_u(i,j,k)*zone2_inv_dz)**zone2_stretch_dz & !22-09-20 v289
                          ),0),zone2_nlayer-1)
            zone2saltflux_u(k2,k3)=zone2saltflux_u(k2,k3)+0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
           enddo
          enddo ! k1
       return
       endif            !-flux-u-salt->
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone2_nlayer-1 pour z=zmax_zone2_nlayer.
            ...
            zone2tempflux_u(k2,k3)=zone2tempflux_u(k2,k3)+0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
           enddo
          enddo ! k1
       return
       endif            !-flux-u-temp->
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone2_nlayer-1 pour z=zmax_zone2_nlayer.
            k3=min(max(int(   &
                            abs(depth_v(i,j,k)*zone2_inv_dz)**zone2_stretch_dz & !22-09-20
                          ),0),zone2_nlayer-1)
            zone2saltflux_v(k2,k3)=zone2saltflux_v(k2,k3)+0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
           enddo
          enddo ! k1
       return
       endif            !-flux-v-salt->
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone2_nlayer-1 pour z=zmax_zone2_nlayer.
            ...
            zone2tempflux_v(k2,k3)=zone2tempflux_v(k2,k3)+0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
           enddo
          enddo ! k1
       return
       endif            !-flux-v-temp->
       ...
       if(txt_=='k') then !-flux-w->
         do j=1,jmax ; do i=1,imax 
! salt
           zone2saltflux_w=zone2saltflux_w  &
                         -max(-zone2_mask(i,j),0) & ! selectionne zone2_mask=-1
           *dti_fw*dxdy_t(i,j)*mask_i_w(i)*mask_j_w(j) &
           *(omega_w(i,j,kmax+1,1)-0.5*(omega_evaprec_w(i,j,0)+omega_evaprec_w(i,j,1))) & !isole w_surf wetdry
           *sal_t(i,j,kmax,2)
! temp
           zone2tempflux_w=zone2tempflux_w  &
                         -max(-zone2_mask(i,j),0) & ! selectionne zone2_mask=-1
           *dti_fw*dxdy_t(i,j)*mask_i_w(i)*mask_j_w(j)*( & !m°v°m>
            ( omega_w(i,j,kmax+1,1)*tem_t(i,j,kmax,2)-omega_w(i,j,1,1)*tem_t(i,j,1,2) ) &
                             -1./( & !pmx>                   ! 2.Dt / !24-04-16
...
!        x0=-dti_fw/real(iteration2d_max_now)
         x0=-dti_fw
          ...
          do k1=1,zone2_u_max
           ...
           k0=sign(1,zone2_mask(i,j)-zone2_mask(i-1,j))
           do k=1,kmax
!           zone2waterflux_u(k2)=zone2waterflux_u(k2)+k0*x0*fluxbar_sumt_u(i,j,1)
! k3 est la classe de profondeur  k3=0 en surface, k3=zone2_nlayer-1 pour z=zmax_zone2_nlayer.
            k3=min(max(int(   &
                            abs(depth_u(i,j,k)*zone2_inv_dz)**zone2_stretch_dz & !22-09-20
                          ),0),zone2_nlayer-1)
            zone2waterflux_u(k2,k3)=zone2waterflux_u(k2,k3)+k0*x0*veldydz_u(i,j,k,1)
           enddo ! k
          enddo ! k1
          ...
          do k1=1,zone2_v_max
           ...
           k0=sign(1,zone2_mask(i,j)-zone2_mask(i,j-1))
           do k=1,kmax
!           zone2waterflux_v(k2)=zone2waterflux_v(k2)+k0*x0*fluxbar_sumt_v(i,j,1)
! k3 est la classe de profondeur  k3=0 en surface, k3=zone2_nlayer-1 pour z=zmax_zone2_nlayer.
            k3=min(max(int(   &
                            abs(depth_v(i,j,k)*zone2_inv_dz)**zone2_stretch_dz & !22-09-20
                          ),0),zone2_nlayer-1)
            zone2waterflux_v(k2,k3)=zone2waterflux_v(k2,k3)+k0*x0*veldxdz_v(i,j,k,1)
           enddo ! k
          enddo ! k1
...
! mpi sum
       if(txt_=='mpi') then !--mpi-->
        ...
        do k3=0,zone2_nlayer-1
         ...
         do k2=0,zone2_max
          ...
          if(zone2_u_max>0) then !>>>
           x1=   zone2saltflux_u(k2,k3)
           x3=   zone2tempflux_u(k2,k3)
           x5=  zone2waterflux_u(k2,k3)
          endif                 !>>>
          if(zone2_v_max>0) then !>>>
           x1=x1+ zone2saltflux_v(k2,k3)
           x3=x3+ zone2tempflux_v(k2,k3)
           x5=x5+zone2waterflux_v(k2,k3)
          endif                 !>>>
          call mpi_allreduce(x1,x2,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone2saltflux_glb(k2,k3)=zone2saltflux_glb(k2,k3)+x2
          call mpi_allreduce(x3,x4,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone2tempflux_glb(k2,k3)=zone2tempflux_glb(k2,k3)+x4
          call mpi_allreduce(x5,x6,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone2waterflux_glb(k2,k3)=zone2waterflux_glb(k2,k3)+x6
        ...
        enddo ! k3
         ...
         x1=zone2tempflux_w
         call mpi_allreduce(x1,x2,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
         zone2tempflux_glb(k2,k3)=zone2tempflux_glb(k2,k3)+x2
         x1=zone2waterflux_w
         call mpi_allreduce(x1,x2,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
         zone2waterflux_glb(k2,k3)=zone2waterflux_glb(k2,k3)+x2

! reset des tableaux locaux
          zone2saltflux_u=0. ;  zone2saltflux_v=0. ;  zone2saltflux_w=0.
          zone2tempflux_u=0. ;  zone2tempflux_v=0. ;  zone2tempflux_w=0.
         zone2waterflux_u=0. ; zone2waterflux_v=0. ; zone2waterflux_w=0.
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone2_nlayer-1 pour z=zmax_zone2_nlayer.
         ...
         if(iteration3d==0) then !m°v°m> !22-09-20
           do k=0,zone2_nlayer-1
            zone2watermasst0(k)=-zone2watercumul_glb(k)+sum(zone2waterflux_glb(:,k))
           enddo
         endif                   !m°v°m> !22-09-20
         if(par%rank==0) then !-rank0->
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone2_nlayer-1 pour z=zmax_zone2_nlayer.
         ...
         call mpi_allreduce(zone2saltcumul_loc,zone2saltcumul_glb,zone2_nlayer,mpi_double_precision,mpi_sum,par%comm2d,ierr)
         if(iteration3d==0) then !m°v°m> !22-09-20
           do k=0,zone2_nlayer-1
            zone2saltmasst0(k)=-zone2saltcumul_glb(k)+sum(zone2saltflux_glb(:,k))
           enddo
         endif                   !m°v°m> !22-09-20

         if(par%rank==0) then !-rank0->
           write(texte30,'(a)')'tmp/obcsaltflux_zone2'
           open(unit=3,file=texte30,position='append')
! Notes: - zone2saltmasst0 sert A transformer sum2 en une anomalie
!        - colonnes 2 et 3 doivent etre identiques
                      write(3,'(100(1x,e14.7))')                 &
                      real(elapsedtime_now/86400.)               & ! colonne 1
                     ,sum(zone2saltcumul_glb)+sum(zone2saltmasst0) & ! colonne 2
                     ,sum(zone2saltflux_glb)                      & ! colonne 3
                     ,zone2saltflux_glb(0:zone2_max+1,0:zone2_nlayer-1) &
                     ,zone2saltcumul_glb(0:zone2_nlayer-1)+zone2saltmasst0(0:zone2_nlayer-1)
           close(3)
         endif                !-rank0->

! integrale temperature
         zone2tempcumul_loc=0.
         do k=1,kmax ; do j=1,jmax ; do i=1,imax
! k3 est la classe de profondeur  k3=0 en surface, k3=zone2_nlayer-1 pour z=zmax_zone2_nlayer.
            k3=min(max(int(   &
                            abs(depth_t(i,j,k)*zone2_inv_dz)**zone2_stretch_dz & !22-09-20
         ...
         enddo       ; enddo       ; enddo
         call mpi_allreduce(zone2tempcumul_loc,zone2tempcumul_glb,zone2_nlayer,mpi_double_precision,mpi_sum,par%comm2d,ierr)
         if(iteration3d==0) then !m°v°m> !22-09-20
           do k=0,zone2_nlayer-1
            zone2tempmasst0(k)=-zone2tempcumul_glb(k)+sum(zone2tempflux_glb(:,k))
           enddo
         endif                   !m°v°m> !22-09-20
         if(par%rank==0) then !-rank0->
           write(texte30,'(a)')'tmp/obctempflux_zone2'
           open(unit=3,file=texte30,position='append')
! Notes: - zone2saltmasst0 sert A transformer sum2 en une anomalie
!        - colonnes 2 et 3 doivent etre identiques
                      write(3,'(100(1x,e14.7))')                  &
                      real(elapsedtime_now/86400.)               & ! colonne 1
                     ,sum(zone2tempcumul_glb)+sum(zone2tempmasst0) & ! colonne 2
                     ,sum(zone2tempflux_glb)                      & ! colonne 3
                     ,zone2tempflux_glb(0:zone2_max+1,0:zone2_nlayer-1) &
                     ,zone2tempcumul_glb(0:zone2_nlayer-1)+zone2tempmasst0(0:zone2_nlayer-1)
           close(3)
         endif                !-rank0->
       ...
       stop 'Err undefined txt_ in my_outputs_zone2saltflux'

      end subroutine my_outputs_zone2salttempflux

      subroutine my_outputs_zone3salttempflux(txt_,id_flux_) !13-04-20
      implicit none
      integer id_flux_
      character(len=*)txt_
      character*100 name_poly
...
!      zone3_stretch_dz=0.5 ! epaisseur augmentant avec la profondeur (en surface 1/zone3_inv_dz)

! ICI CHACUN SE DEBROUILLE POUR LE LIRE SON FICHIER DE MASQUE AVEC LE
! BON FORMAT.....
!        allocate(glob_mask(0:iglb+1,0:jglb+1)) ; glob_mask=0
!        zone3_max=maxval(glob_mask)
!        allocate(zone3saltflux_glb(0:zone3_max)) ; zone3saltflux_glb=0.
!        allocate(zone3_mask(0:imax+1,0:jmax+1)) ; zone3_mask=0
!        do j=0,jmax+1 ; do i=0,imax+1
!          zone3_mask(i,j)=glob_mask(i+par%timax(1),j+par%tjmax(1))*mask_t(i,j,kmax)
!        enddo       ; enddo
!      deallocate(glob_mask)
! BIDOUILLE PATRICK
         allocate(zone3_mask(0:imax+1,0:jmax+1)) ; zone3_mask=0
...
! et le bilan ne sera pas équilibrer
      ...
      ichoix=0 ; xlon=0. ; xlat=0.
      name_poly='Bilan_symphonie/Poly3_Egee_ssMarmara.kml'
       call polygon_in(ichoix,irep,xlon,xlat,name_poly)
       ichoix=1
       do i=0,imax+1
       do j=0,jmax+1
       call polygon_in(ichoix,irep,lon_t(i,j)*rad2deg,lat_t(i,j)*rad2deg,name_poly)
       if(irep==1) zone3_mask(i,j)=-1*mask_t(i,j,kmax)
       enddo
       enddo
...
!#endif
         ...
         allocate(zone3saltflux_glb (0:zone3_max+1,0:zone3_nlayer-1))   ; zone3saltflux_glb=0.
         allocate(zone3saltflux_u   (0:zone3_max  ,0:zone3_nlayer-1))   ; zone3saltflux_u=0.
         allocate(zone3saltflux_v   (0:zone3_max  ,0:zone3_nlayer-1))   ; zone3saltflux_v=0.
         allocate(zone3tempflux_glb (0:zone3_max+1,0:zone3_nlayer-1))   ; zone3tempflux_glb=0.
         allocate(zone3tempflux_u   (0:zone3_max  ,0:zone3_nlayer-1))   ; zone3tempflux_u=0.
         allocate(zone3tempflux_v   (0:zone3_max  ,0:zone3_nlayer-1))   ; zone3tempflux_v=0.
         allocate(zone3waterflux_glb(0:zone3_max+1,0:zone3_nlayer-1))   ; zone3waterflux_glb=0. !08-07-19
         ...
         allocate(zone3tempcumul_glb (0:zone3_nlayer-1))   ; zone3tempcumul_glb=0. !22-09-20 v289
         allocate(zone3tempcumul_loc (0:zone3_nlayer-1))   ; zone3tempcumul_loc=0.
         allocate(zone3tempmasst0    (0:zone3_nlayer-1))   ; zone3tempmasst0=0.

         allocate(zone3saltcumul_glb (0:zone3_nlayer-1))   ; zone3saltcumul_glb=0.
         allocate(zone3saltcumul_loc (0:zone3_nlayer-1))   ; zone3saltcumul_loc=0.
         allocate(zone3saltmasst0    (0:zone3_nlayer-1))   ; zone3saltmasst0=0.

         allocate(zone3watercumul_glb (0:zone3_nlayer-1))   ; zone3watercumul_glb=0.
...
! Identifier points de flux v
      ...
      open(unit=3,file=trim(tmpdirname)//'messages',position='append')
      write(3,*)'-----------------------------------------------------'
      write(3,*)'subroutine my_outputs_zone3salttempflux'
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone3_nlayer-1 pour z=zmax_zone3_nlayer.
            k3=min(max(int(   &
                           abs(depth_u(i,j,k)*zone3_inv_dz)**zone3_stretch_dz & !22-09-20 v289
                          ),0),zone3_nlayer-1)
            zone3saltflux_u(k2,k3)=zone3saltflux_u(k2,k3)+0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
           enddo
          enddo ! k1
       return
       endif            !-flux-u-salt->
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone3_nlayer-1 pour z=zmax_zone3_nlayer.
            ...
            zone3tempflux_u(k2,k3)=zone3tempflux_u(k2,k3)+0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
           enddo
          enddo ! k1
       return
       endif            !-flux-u-temp->
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone3_nlayer-1 pour z=zmax_zone3_nlayer.
            k3=min(max(int(   &
                            abs(depth_v(i,j,k)*zone3_inv_dz)**zone3_stretch_dz & !22-09-20
                          ),0),zone3_nlayer-1)
            zone3saltflux_v(k2,k3)=zone3saltflux_v(k2,k3)+0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
           enddo
          enddo ! k1
       return
       endif            !-flux-v-salt->
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone3_nlayer-1 pour z=zmax_zone3_nlayer.
            ...
            zone3tempflux_v(k2,k3)=zone3tempflux_v(k2,k3)+0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
           enddo
          enddo ! k1
       return
       endif            !-flux-v-temp->
       ...
       if(txt_=='k') then !-flux-w->
         do j=1,jmax ; do i=1,imax 
! salt
           zone3saltflux_w=zone3saltflux_w  &
                         -max(-zone3_mask(i,j),0) & ! selectionne zone3_mask=-1
           *dti_fw*dxdy_t(i,j)*mask_i_w(i)*mask_j_w(j) &
           *(omega_w(i,j,kmax+1,1)-0.5*(omega_evaprec_w(i,j,0)+omega_evaprec_w(i,j,1))) & !isole w_surf wetdry
           *sal_t(i,j,kmax,2)
! temp
           zone3tempflux_w=zone3tempflux_w  &
                         -max(-zone3_mask(i,j),0) & ! selectionne zone3_mask=-1
           *dti_fw*dxdy_t(i,j)*mask_i_w(i)*mask_j_w(j)*( & !m°v°m>
            ( omega_w(i,j,kmax+1,1)*tem_t(i,j,kmax,2)-omega_w(i,j,1,1)*tem_t(i,j,1,2) ) &
                             -1./( & !pmx>                   ! 2.Dt / !24-04-16
...
!        x0=-dti_fw/real(iteration2d_max_now)
         x0=-dti_fw
          ...
          do k1=1,zone3_u_max
           ...
           k0=sign(1,zone3_mask(i,j)-zone3_mask(i-1,j))
           do k=1,kmax
!           zone3waterflux_u(k2)=zone3waterflux_u(k2)+k0*x0*fluxbar_sumt_u(i,j,1)
! k3 est la classe de profondeur  k3=0 en surface, k3=zone3_nlayer-1 pour z=zmax_zone3_nlayer.
            k3=min(max(int(   &
                            abs(depth_u(i,j,k)*zone3_inv_dz)**zone3_stretch_dz & !22-09-20
                          ),0),zone3_nlayer-1)
            zone3waterflux_u(k2,k3)=zone3waterflux_u(k2,k3)+k0*x0*veldydz_u(i,j,k,1)
           enddo ! k
          enddo ! k1
          ...
          do k1=1,zone3_v_max
           ...
           k0=sign(1,zone3_mask(i,j)-zone3_mask(i,j-1))
           do k=1,kmax
!           zone3waterflux_v(k2)=zone3waterflux_v(k2)+k0*x0*fluxbar_sumt_v(i,j,1)
! k3 est la classe de profondeur  k3=0 en surface, k3=zone3_nlayer-1 pour z=zmax_zone3_nlayer.
            k3=min(max(int(   &
                            abs(depth_v(i,j,k)*zone3_inv_dz)**zone3_stretch_dz & !22-09-20
                          ),0),zone3_nlayer-1)
            zone3waterflux_v(k2,k3)=zone3waterflux_v(k2,k3)+k0*x0*veldxdz_v(i,j,k,1)
           enddo ! k
          enddo ! k1
...
! mpi sum
       if(txt_=='mpi') then !--mpi-->
        ...
        do k3=0,zone3_nlayer-1
         ...
         do k2=0,zone3_max
          ...
          if(zone3_u_max>0) then !>>>
           x1=   zone3saltflux_u(k2,k3)
           x3=   zone3tempflux_u(k2,k3)
           x5=  zone3waterflux_u(k2,k3)
          endif                 !>>>
          if(zone3_v_max>0) then !>>>
           x1=x1+ zone3saltflux_v(k2,k3)
           x3=x3+ zone3tempflux_v(k2,k3)
           x5=x5+zone3waterflux_v(k2,k3)
          endif                 !>>>
          call mpi_allreduce(x1,x2,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone3saltflux_glb(k2,k3)=zone3saltflux_glb(k2,k3)+x2
          call mpi_allreduce(x3,x4,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone3tempflux_glb(k2,k3)=zone3tempflux_glb(k2,k3)+x4
          call mpi_allreduce(x5,x6,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone3waterflux_glb(k2,k3)=zone3waterflux_glb(k2,k3)+x6
        ...
        enddo ! k3
         ...
         x1=zone3tempflux_w
         call mpi_allreduce(x1,x2,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
         zone3tempflux_glb(k2,k3)=zone3tempflux_glb(k2,k3)+x2
         x1=zone3waterflux_w
         call mpi_allreduce(x1,x2,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
         zone3waterflux_glb(k2,k3)=zone3waterflux_glb(k2,k3)+x2

! reset des tableaux locaux
          zone3saltflux_u=0. ;  zone3saltflux_v=0. ;  zone3saltflux_w=0.
          zone3tempflux_u=0. ;  zone3tempflux_v=0. ;  zone3tempflux_w=0.
         zone3waterflux_u=0. ; zone3waterflux_v=0. ; zone3waterflux_w=0.
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone3_nlayer-1 pour z=zmax_zone3_nlayer.
         ...
         if(iteration3d==0) then !m°v°m> !22-09-20
           do k=0,zone3_nlayer-1
            zone3watermasst0(k)=-zone3watercumul_glb(k)+sum(zone3waterflux_glb(:,k))
           enddo
         endif                   !m°v°m> !22-09-20
         if(par%rank==0) then !-rank0->
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone3_nlayer-1 pour z=zmax_zone3_nlayer.
         ...
         call mpi_allreduce(zone3saltcumul_loc,zone3saltcumul_glb,zone3_nlayer,mpi_double_precision,mpi_sum,par%comm2d,ierr)
         if(iteration3d==0) then !m°v°m> !22-09-20
           do k=0,zone3_nlayer-1
            zone3saltmasst0(k)=-zone3saltcumul_glb(k)+sum(zone3saltflux_glb(:,k))
           enddo
         endif                   !m°v°m> !22-09-20

         if(par%rank==0) then !-rank0->
           write(texte30,'(a)')'tmp/obcsaltflux_zone3'
           open(unit=3,file=texte30,position='append')
! Notes: - zone3saltmasst0 sert A transformer sum2 en une anomalie
!        - colonnes 2 et 3 doivent etre identiques
                      write(3,'(100(1x,e14.7))')                 &
                      real(elapsedtime_now/86400.)               & ! colonne 1
                     ,sum(zone3saltcumul_glb)+sum(zone3saltmasst0) & ! colonne 2
                     ,sum(zone3saltflux_glb)                      & ! colonne 3
                     ,zone3saltflux_glb(0:zone3_max+1,0:zone3_nlayer-1) &
                     ,zone3saltcumul_glb(0:zone3_nlayer-1)+zone3saltmasst0(0:zone3_nlayer-1)
           close(3)
         endif                !-rank0->

! integrale temperature
         zone3tempcumul_loc=0.
         do k=1,kmax ; do j=1,jmax ; do i=1,imax
! k3 est la classe de profondeur  k3=0 en surface, k3=zone3_nlayer-1 pour z=zmax_zone3_nlayer.
            k3=min(max(int(   &
                            abs(depth_t(i,j,k)*zone3_inv_dz)**zone3_stretch_dz & !22-09-20
         ...
         enddo       ; enddo       ; enddo
         call mpi_allreduce(zone3tempcumul_loc,zone3tempcumul_glb,zone3_nlayer,mpi_double_precision,mpi_sum,par%comm2d,ierr)
         if(iteration3d==0) then !m°v°m> !22-09-20
           do k=0,zone3_nlayer-1
            zone3tempmasst0(k)=-zone3tempcumul_glb(k)+sum(zone3tempflux_glb(:,k))
           enddo
         endif                   !m°v°m> !22-09-20
         if(par%rank==0) then !-rank0->
           write(texte30,'(a)')'tmp/obctempflux_zone3'
           open(unit=3,file=texte30,position='append')
! Notes: - zone3saltmasst0 sert A transformer sum2 en une anomalie
!        - colonnes 2 et 3 doivent etre identiques
                      write(3,'(100(1x,e14.7))')                  &
                      real(elapsedtime_now/86400.)               & ! colonne 1
                     ,sum(zone3tempcumul_glb)+sum(zone3tempmasst0) & ! colonne 2
                     ,sum(zone3tempflux_glb)                      & ! colonne 3
                     ,zone3tempflux_glb(0:zone3_max+1,0:zone3_nlayer-1) &
                     ,zone3tempcumul_glb(0:zone3_nlayer-1)+zone3tempmasst0(0:zone3_nlayer-1)
           close(3)
         endif                !-rank0->
       ...
       stop 'Err undefined txt_ in my_outputs_zone3saltflux'

      end subroutine my_outputs_zone3salttempflux
#endif
...
!#endif
         ...
         allocate(zone2saltflux_glb_in (0:zone2_max+1,0:zone2_nlayer-1))   ; zone2saltflux_glb_in=0.
         allocate(zone2saltflux_u_in   (0:zone2_max  ,0:zone2_nlayer-1))   ; zone2saltflux_u_in=0.
         allocate(zone2saltflux_v_in   (0:zone2_max  ,0:zone2_nlayer-1))   ; zone2saltflux_v_in=0.
         allocate(zone2tempflux_glb_in (0:zone2_max+1,0:zone2_nlayer-1))   ; zone2tempflux_glb_in=0.
         allocate(zone2tempflux_u_in   (0:zone2_max  ,0:zone2_nlayer-1))   ; zone2tempflux_u_in=0.
         allocate(zone2tempflux_v_in   (0:zone2_max  ,0:zone2_nlayer-1))   ; zone2tempflux_v_in=0.
         allocate(zone2waterflux_glb_in(0:zone2_max+1,0:zone2_nlayer-1))   ; zone2waterflux_glb_in=0. 
         ...
         allocate(zone2saltflux_glb_out (0:zone2_max+1,0:zone2_nlayer-1))   ; zone2saltflux_glb_out=0.
         allocate(zone2saltflux_u_out   (0:zone2_max  ,0:zone2_nlayer-1))   ; zone2saltflux_u_out=0.
         allocate(zone2saltflux_v_out   (0:zone2_max  ,0:zone2_nlayer-1))   ; zone2saltflux_v_out=0.
         allocate(zone2tempflux_glb_out (0:zone2_max+1,0:zone2_nlayer-1))   ; zone2tempflux_glb_out=0.
         allocate(zone2tempflux_u_out   (0:zone2_max  ,0:zone2_nlayer-1))   ; zone2tempflux_u_out=0.
         allocate(zone2tempflux_v_out   (0:zone2_max  ,0:zone2_nlayer-1))   ; zone2tempflux_v_out=0.
         allocate(zone2waterflux_glb_out(0:zone2_max+1,0:zone2_nlayer-1))   ; zone2waterflux_glb_out=0. 
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone2_nlayer-1 pour z=zmax_zone2_nlayer.
            ...
            if (dti_lpsub*k0*anyv3d(i,j,k,id_flux_) > 0) then
            zone2saltflux_u_in(k2,k3) =zone2saltflux_u_in(k2,k3) +0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
            else
            zone2saltflux_u_out(k2,k3)=zone2saltflux_u_out(k2,k3)+0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
            endif
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone2_nlayer-1 pour z=zmax_zone2_nlayer.
            ...
            if (dti_lpsub*k0*anyv3d(i,j,k,id_flux_) > 0) then
            zone2tempflux_u_in(k2,k3) =zone2tempflux_u_in(k2,k3) +0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
            else
            zone2tempflux_u_out(k2,k3)=zone2tempflux_u_out(k2,k3)+0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
            endif
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone2_nlayer-1 pour z=zmax_zone2_nlayer.
            ...
            if (dti_lpsub*k0*anyv3d(i,j,k,id_flux_) > 0) then
            zone2saltflux_v_in(k2,k3) =zone2saltflux_v_in(k2,k3) +0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
            else
            zone2saltflux_v_out(k2,k3)=zone2saltflux_v_out(k2,k3)+0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
            endif
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone2_nlayer-1 pour z=zmax_zone2_nlayer.
            ...
            if (dti_lpsub*k0*anyv3d(i,j,k,id_flux_) > 0) then
            zone2tempflux_v_in(k2,k3) =zone2tempflux_v_in(k2,k3) +0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
            else
            zone2tempflux_v_out(k2,k3)=zone2tempflux_v_out(k2,k3)+0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
            endif
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone2_nlayer-1 pour z=zmax_zone2_nlayer.
            ...
            if (k0*x0*veldydz_u(i,j,k,1) > 0) then
            zone2waterflux_u_in(k2,k3) = zone2waterflux_u_in(k2,k3)+k0*x0*veldydz_u(i,j,k,1)
            else
            zone2waterflux_u_out(k2,k3)= zone2waterflux_u_out(k2,k3)+k0*x0*veldydz_u(i,j,k,1)
            endif
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone2_nlayer-1 pour z=zmax_zone2_nlayer.
            ...
            if (k0*x0*veldxdz_v(i,j,k,1) > 0) then
            zone2waterflux_v_in(k2,k3) = zone2waterflux_v_in(k2,k3)+k0*x0*veldxdz_v(i,j,k,1)
            else
            zone2waterflux_v_out(k2,k3)= zone2waterflux_v_out(k2,k3)+k0*x0*veldxdz_v(i,j,k,1)
            endif
       ...
       if(txt_=='mpi') then !--mpi-->
        ...
        do k3=0,zone2_nlayer-1
         ...
         do k2=0,zone2_max
          ...
          x1=0. ; x3=0. ; x5=0.
          if(zone2_u_max>0) then !>>>
           x1=   zone2saltflux_u_in(k2,k3)
           x3=   zone2tempflux_u_in(k2,k3)
           x5=  zone2waterflux_u_in(k2,k3)
          endif                 !>>>
          if(zone2_v_max>0) then !>>>
           x1=x1+ zone2saltflux_v_in(k2,k3)
           x3=x3+ zone2tempflux_v_in(k2,k3)
           x5=x5+zone2waterflux_v_in(k2,k3)
          endif                 !>>>
          call mpi_allreduce(x1,x2,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone2saltflux_glb_in(k2,k3)=zone2saltflux_glb_in(k2,k3)+x2
          call mpi_allreduce(x3,x4,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone2tempflux_glb_in(k2,k3)=zone2tempflux_glb_in(k2,k3)+x4
          call mpi_allreduce(x5,x6,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone2waterflux_glb_in(k2,k3)=zone2waterflux_glb_in(k2,k3)+x6



          x1=0. ; x3=0. ; x5=0.
          if(zone2_u_max>0) then !>>>
           x1=   zone2saltflux_u_out(k2,k3)
           x3=   zone2tempflux_u_out(k2,k3)
           x5=  zone2waterflux_u_out(k2,k3)
          endif                 !>>>
          if(zone2_v_max>0) then !>>>
           x1=x1+ zone2saltflux_v_out(k2,k3)
           x3=x3+ zone2tempflux_v_out(k2,k3)
           x5=x5+zone2waterflux_v_out(k2,k3)
          endif                 !>>>
          call mpi_allreduce(x1,x2,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone2saltflux_glb_out(k2,k3)=zone2saltflux_glb_out(k2,k3)+x2
          call mpi_allreduce(x3,x4,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone2tempflux_glb_out(k2,k3)=zone2tempflux_glb_out(k2,k3)+x4
          call mpi_allreduce(x5,x6,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone2waterflux_glb_out(k2,k3)=zone2waterflux_glb_out(k2,k3)+x6
...
! reset des tableaux locaux
         ...
         zone2saltflux_u_in=0. ;  zone2saltflux_v_in=0. ; 
         zone2tempflux_u_in=0. ;  zone2tempflux_v_in=0. ;
         zone2waterflux_u_in=0. ; zone2waterflux_v_in=0. ; 

         zone2saltflux_u_out=0. ;  zone2saltflux_v_out=0. ;  
         zone2tempflux_u_out=0. ;  zone2tempflux_v_out=0. ; 
         zone2waterflux_u_out=0. ; zone2waterflux_v_out=0. ; 
...
!        - colonnes 2 et 3 doivent etre identiques
           ...
           write(texte30,'(a)')'tmp/obcwaterflux_zone2_in'
           open(unit=3,file=texte30,position='append')
                      write(3,'(100(1x,e14.7))')                     &
                      real(elapsedtime_now/86400.)                   & ! colonne 1
                     ,sum(zone2waterflux_glb_in)                        & ! colonne 2
                     ,zone2waterflux_glb_in(0:zone2_max+1,0:zone2_nlayer-1) 
           close(3)

           write(texte30,'(a)')'tmp/obcwaterflux_zone2_out'
           open(unit=3,file=texte30,position='append')
                      write(3,'(100(1x,e14.7))')                     &
                      real(elapsedtime_now/86400.)                   & ! colonne 1
                     ,sum(zone2waterflux_glb_out)                        & ! colonne 2
                     ,zone2waterflux_glb_out(0:zone2_max+1,0:zone2_nlayer-1) 
           close(3)
...
!        - colonnes 2 et 3 doivent etre identiques
           ...
           write(texte30,'(a)')'tmp/obcsaltflux_zone2_in'
           open(unit=3,file=texte30,position='append')
                      write(3,'(100(1x,e14.7))')                     &
                      real(elapsedtime_now/86400.)                   & ! colonne 1
                     ,sum(zone2saltflux_glb_in)                        & ! colonne 2
                     ,zone2saltflux_glb_in(0:zone2_max+1,0:zone2_nlayer-1) 
           close(3)

           write(texte30,'(a)')'tmp/obcsaltflux_zone2_out'
           open(unit=3,file=texte30,position='append')
                      write(3,'(100(1x,e14.7))')                     &
                      real(elapsedtime_now/86400.)                   & ! colonne 1
                     ,sum(zone2saltflux_glb_out)                        & ! colonne 2
                     ,zone2saltflux_glb_out(0:zone2_max+1,0:zone2_nlayer-1) 
           close(3)
...
!        - colonnes 2 et 3 doivent etre identiques
           ...
           write(texte30,'(a)')'tmp/obctempflux_zone2_in'
           open(unit=3,file=texte30,position='append')
                      write(3,'(100(1x,e14.7))')                     &
                      real(elapsedtime_now/86400.)                   & ! colonne 1
                     ,sum(zone2tempflux_glb_in)                        & ! colonne 2
                     ,zone2tempflux_glb_in(0:zone2_max+1,0:zone2_nlayer-1) 
           close(3)

           write(texte30,'(a)')'tmp/obctempflux_zone2_out'
           open(unit=3,file=texte30,position='append')
                      write(3,'(100(1x,e14.7))')                     &
                      real(elapsedtime_now/86400.)                   & ! colonne 1
                     ,sum(zone2tempflux_glb_out)                        & ! colonne 2
                     ,zone2tempflux_glb_out(0:zone2_max+1,0:zone2_nlayer-1) 
           close(3)
...
!#endif
         ...
         allocate(zone3saltflux_glb_in (0:zone3_max+1,0:zone3_nlayer-1))   ; zone3saltflux_glb_in=0.
         allocate(zone3saltflux_u_in   (0:zone3_max  ,0:zone3_nlayer-1))   ; zone3saltflux_u_in=0.
         allocate(zone3saltflux_v_in   (0:zone3_max  ,0:zone3_nlayer-1))   ; zone3saltflux_v_in=0.
         allocate(zone3tempflux_glb_in (0:zone3_max+1,0:zone3_nlayer-1))   ; zone3tempflux_glb_in=0.
         allocate(zone3tempflux_u_in   (0:zone3_max  ,0:zone3_nlayer-1))   ; zone3tempflux_u_in=0.
         allocate(zone3tempflux_v_in   (0:zone3_max  ,0:zone3_nlayer-1))   ; zone3tempflux_v_in=0.
         allocate(zone3waterflux_glb_in(0:zone3_max+1,0:zone3_nlayer-1))   ; zone3waterflux_glb_in=0. 
         ...
         allocate(zone3saltflux_glb_out (0:zone3_max+1,0:zone3_nlayer-1))   ; zone3saltflux_glb_out=0.
         allocate(zone3saltflux_u_out   (0:zone3_max  ,0:zone3_nlayer-1))   ; zone3saltflux_u_out=0.
         allocate(zone3saltflux_v_out   (0:zone3_max  ,0:zone3_nlayer-1))   ; zone3saltflux_v_out=0.
         allocate(zone3tempflux_glb_out (0:zone3_max+1,0:zone3_nlayer-1))   ; zone3tempflux_glb_out=0.
         allocate(zone3tempflux_u_out   (0:zone3_max  ,0:zone3_nlayer-1))   ; zone3tempflux_u_out=0.
         allocate(zone3tempflux_v_out   (0:zone3_max  ,0:zone3_nlayer-1))   ; zone3tempflux_v_out=0.
         allocate(zone3waterflux_glb_out(0:zone3_max+1,0:zone3_nlayer-1))   ; zone3waterflux_glb_out=0. 
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone3_nlayer-1 pour z=zmax_zone3_nlayer.
            ...
            if (dti_lpsub*k0*anyv3d(i,j,k,id_flux_) > 0) then
            zone3saltflux_u_in(k2,k3) =zone3saltflux_u_in(k2,k3) +0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
            else
            zone3saltflux_u_out(k2,k3)=zone3saltflux_u_out(k2,k3)+0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
            endif
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone3_nlayer-1 pour z=zmax_zone3_nlayer.
            ...
            if (dti_lpsub*k0*anyv3d(i,j,k,id_flux_) > 0) then
            zone3tempflux_u_in(k2,k3) =zone3tempflux_u_in(k2,k3) +0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
            else
            zone3tempflux_u_out(k2,k3)=zone3tempflux_u_out(k2,k3)+0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
            endif
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone3_nlayer-1 pour z=zmax_zone3_nlayer.
            ...
            if (dti_lpsub*k0*anyv3d(i,j,k,id_flux_) > 0) then
            zone3saltflux_v_in(k2,k3) =zone3saltflux_v_in(k2,k3) +0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
            else
            zone3saltflux_v_out(k2,k3)=zone3saltflux_v_out(k2,k3)+0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
            endif
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone3_nlayer-1 pour z=zmax_zone3_nlayer.
            ...
            if (dti_lpsub*k0*anyv3d(i,j,k,id_flux_) > 0) then
            zone3tempflux_v_in(k2,k3) =zone3tempflux_v_in(k2,k3) +0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
            else
            zone3tempflux_v_out(k2,k3)=zone3tempflux_v_out(k2,k3)+0.5*dti_lpsub*k0*anyv3d(i,j,k,id_flux_)
            endif
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone3_nlayer-1 pour z=zmax_zone3_nlayer.
            ...
            if (k0*x0*veldydz_u(i,j,k,1) > 0) then
            zone3waterflux_u_in(k2,k3) = zone3waterflux_u_in(k2,k3)+k0*x0*veldydz_u(i,j,k,1)
            else
            zone3waterflux_u_out(k2,k3)= zone3waterflux_u_out(k2,k3)+k0*x0*veldydz_u(i,j,k,1)
            endif
...
! k3 est la classe de profondeur  k3=0 en surface, k3=zone3_nlayer-1 pour z=zmax_zone3_nlayer.
            ...
            if (k0*x0*veldxdz_v(i,j,k,1) > 0) then
            zone3waterflux_v_in(k2,k3) = zone3waterflux_v_in(k2,k3)+k0*x0*veldxdz_v(i,j,k,1)
            else
            zone3waterflux_v_out(k2,k3)= zone3waterflux_v_out(k2,k3)+k0*x0*veldxdz_v(i,j,k,1)
            endif
       ...
       if(txt_=='mpi') then !--mpi-->
        ...
        do k3=0,zone3_nlayer-1
         ...
         do k2=0,zone3_max
          ...
          x1=0. ; x3=0. ; x5=0.
          if(zone3_u_max>0) then !>>>
           x1=   zone3saltflux_u_in(k2,k3)
           x3=   zone3tempflux_u_in(k2,k3)
           x5=  zone3waterflux_u_in(k2,k3)
          endif                 !>>>
          if(zone3_v_max>0) then !>>>
           x1=x1+ zone3saltflux_v_in(k2,k3)
           x3=x3+ zone3tempflux_v_in(k2,k3)
           x5=x5+zone3waterflux_v_in(k2,k3)
          endif                 !>>>
          call mpi_allreduce(x1,x2,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone3saltflux_glb_in(k2,k3)=zone3saltflux_glb_in(k2,k3)+x2
          call mpi_allreduce(x3,x4,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone3tempflux_glb_in(k2,k3)=zone3tempflux_glb_in(k2,k3)+x4
          call mpi_allreduce(x5,x6,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone3waterflux_glb_in(k2,k3)=zone3waterflux_glb_in(k2,k3)+x6



          x1=0. ; x3=0. ; x5=0.
          if(zone3_u_max>0) then !>>>
           x1=   zone3saltflux_u_out(k2,k3)
           x3=   zone3tempflux_u_out(k2,k3)
           x5=  zone3waterflux_u_out(k2,k3)
          endif                 !>>>
          if(zone3_v_max>0) then !>>>
           x1=x1+ zone3saltflux_v_out(k2,k3)
           x3=x3+ zone3tempflux_v_out(k2,k3)
           x5=x5+zone3waterflux_v_out(k2,k3)
          endif                 !>>>
          call mpi_allreduce(x1,x2,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone3saltflux_glb_out(k2,k3)=zone3saltflux_glb_out(k2,k3)+x2
          call mpi_allreduce(x3,x4,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone3tempflux_glb_out(k2,k3)=zone3tempflux_glb_out(k2,k3)+x4
          call mpi_allreduce(x5,x6,1,mpi_double_precision,mpi_sum,par%comm2d,ierr)
          zone3waterflux_glb_out(k2,k3)=zone3waterflux_glb_out(k2,k3)+x6
...
! reset des tableaux locaux
         ...
         zone3saltflux_u_in=0. ;  zone3saltflux_v_in=0. ; 
         zone3tempflux_u_in=0. ;  zone3tempflux_v_in=0. ;
         zone3waterflux_u_in=0. ; zone3waterflux_v_in=0. ; 

         zone3saltflux_u_out=0. ;  zone3saltflux_v_out=0. ;  
         zone3tempflux_u_out=0. ;  zone3tempflux_v_out=0. ; 
         zone3waterflux_u_out=0. ; zone3waterflux_v_out=0. ; 
...
!        - colonnes 2 et 3 doivent etre identiques
           ...
           write(texte30,'(a)')'tmp/obcwaterflux_zone3_in'
           open(unit=3,file=texte30,position='append')
                      write(3,'(100(1x,e14.7))')                     &
                      real(elapsedtime_now/86400.)                   & ! colonne 1
                     ,sum(zone3waterflux_glb_in)                        & ! colonne 2
                     ,zone3waterflux_glb_in(0:zone3_max+1,0:zone3_nlayer-1) 
           close(3)

           write(texte30,'(a)')'tmp/obcwaterflux_zone3_out'
           open(unit=3,file=texte30,position='append')
                      write(3,'(100(1x,e14.7))')                     &
                      real(elapsedtime_now/86400.)                   & ! colonne 1
                     ,sum(zone3waterflux_glb_out)                        & ! colonne 2
                     ,zone3waterflux_glb_out(0:zone3_max+1,0:zone3_nlayer-1) 
           close(3)
...
!        - colonnes 2 et 3 doivent etre identiques
           ...
           write(texte30,'(a)')'tmp/obcsaltflux_zone3_in'
           open(unit=3,file=texte30,position='append')
                      write(3,'(100(1x,e14.7))')                     &
                      real(elapsedtime_now/86400.)                   & ! colonne 1
                     ,sum(zone3saltflux_glb_in)                        & ! colonne 2
                     ,zone3saltflux_glb_in(0:zone3_max+1,0:zone3_nlayer-1) 
           close(3)

           write(texte30,'(a)')'tmp/obcsaltflux_zone3_out'
           open(unit=3,file=texte30,position='append')
                      write(3,'(100(1x,e14.7))')                     &
                      real(elapsedtime_now/86400.)                   & ! colonne 1
                     ,sum(zone3saltflux_glb_out)                        & ! colonne 2
                     ,zone3saltflux_glb_out(0:zone3_max+1,0:zone3_nlayer-1) 
           close(3)
...
!        - colonnes 2 et 3 doivent etre identiques
           ...
           write(texte30,'(a)')'tmp/obctempflux_zone3_in'
           open(unit=3,file=texte30,position='append')
                      write(3,'(100(1x,e14.7))')                     &
                      real(elapsedtime_now/86400.)                   & ! colonne 1
                     ,sum(zone3tempflux_glb_in)                        & ! colonne 2
                     ,zone3tempflux_glb_in(0:zone3_max+1,0:zone3_nlayer-1) 
           close(3)

           write(texte30,'(a)')'tmp/obctempflux_zone3_out'
           open(unit=3,file=texte30,position='append')
                      write(3,'(100(1x,e14.7))')                     &
                      real(elapsedtime_now/86400.)                   & ! colonne 1
                     ,sum(zone3tempflux_glb_out)                        & ! colonne 2
                     ,zone3tempflux_glb_out(0:zone3_max+1,0:zone3_nlayer-1) 
           close(3)

...
! sur S permet de conserver le bilan de sel, à savoir Somme(vel.S)=0.
...
! Note3: le principe est de corriger une valeur a priori de Ssortant. Jusqu'à maintenant l'appel
!        au case1 imposait comme valeur de depart 0. Mais on peut reflechir à partir d'un
...
!      write(*,*)'DS=',x1
!      sum1=0.
       do k=1,kmax ! attention k demarre à 1 et non pas kmin_v
                   ! car les tableaux sont integralement considérés dans
                   ! l'etape d'advection à suivre...
...
!      sum1=0.
       ...
       do k=1,kmax ! attention k demarre à 1 et pas kmin_u
        i1=i-( nint(sign(un,veldydz_u(i,j,k,1))) + 1 )/2

...
! export par sédimentation
      if(abs(bio_t(i,j,k,vb)>1.e-30))then 
...
! export advectif
      EXP_ADV=0.
      if(abs(bio_t(i,j,k,vb)+bio_t(i,j,k-1,vb))>1.e-30.and.mask_t(I,J,k-1)==1)then
      EXP_ADV=-0.5*(bio_t(i,j,k,vb)    &
                   +bio_t(i,j,k-1,vb)) & 
                   *omega_w(i,j,k,1) 
      else
      EXP_ADV=0.
...
! export turbulent
      EXP_TURB=0.
      if(k/=kmin_w(i,j).and.abs(bio_t(i,j,k,vb)-bio_t(i,j,k-1,vb))>1.e-30.and.mask_t(I,J,k-1)==1)then        !ce 09-12-2014
      EXP_TURB=                                        &
             (bio_t(i,j,k  ,vb)                        &
             -bio_t(i,j,k-1,vb))                       &
...
! import de Nitrate de mod en mgC m-2 j-1
      if(vb==iNitrate) then
         EXP2D(I,J,1)= EXP2D(I,J,1)+EXP_ADV*86400
         EXP2D(I,J,2)= EXP2D(I,J,2)+EXP_TURB*86400
      endif

! export de mod en mmolN m-2 j-1
       if(vb==iMODN) then
!       if(x2.gt.0.) then
         EXP2D(I,J,3)= EXP2D(I,J,3)+( EXP_ADV)*86400
         EXP2D(I,J,4)= EXP2D(I,J,4)+( EXP_TURB)*86400
!       endif
       endif
...
!       if(x2.gt.0.) then
         EXP2D(I,J,5)= EXP2D(I,J,5)+(EXP_SED)*86400
         EXP2D(I,J,6)= EXP2D(I,J,6)+( EXP_ADV)*86400 !/x2
         EXP2D(I,J,7)= EXP2D(I,J,7)+( EXP_TURB)*86400 !/x2
!       endif
       endif
...
!       if(x2.gt.0.) then
         EXP2D(I,J,8)= EXP2D(I,J,8)+( EXP_ADV)*86400 !/x2
         EXP2D(I,J,9)= EXP2D(I,J,9)+( EXP_TURB)*86400 !/x2
!       endif
       endif
...
!       if(x2.gt.0.) then
         EXP2D(I,J,10)= EXP2D(I,J,10)+( EXP_ADV)*0.18*86400 !/x2
         EXP2D(I,J,11)= EXP2D(I,J,11)+( EXP_TURB)*0.18*86400 !/x2
!       endif
       endif
...
! export de BACTN en mmolN m-2 j-1
       if(vb==iBACTC) then
!       if(x2.gt.0.) then
         EXP2D(I,J,12)= EXP2D(I,J,12)+( EXP_ADV)*0.232*86400 !/x2
         EXP2D(I,J,13)= EXP2D(I,J,13)+( EXP_TURB)*0.232*86400 !/x2
       endif




! import de Phosphate en mgC m-2 j-1
!      if(vb==iPhosphate) then
!         EXP2D(I,J,9)= EXP2D(I,J,9)+( EXP_ADV+ EXP_TURB)*86400*12
!      endif
...
! Export sur le fond
      ...
      EXP_SED_BOT = -wsed(1,vb)*bio_t(i,j,kmin_w(i,j),vb)
      else
      EXP_SED_BOT = 0.
      endif
...
!! caroline different ici mais ne semble pas logique- voir aussi *86400*dti
      ...
      EXP2D(I,J,14)= EXP2D(I,J,14)+EXP_SED_BOT*86400  !mmol/m2/j
      endif                                                       ! idem plus bas
...
! import de Nitrate de mod en mgC m-2 j-1
      if(vb==iNitrate) then
         EXP2D(I,J,15)= EXP2D(I,J,15)+EXP_ADV*86400
         EXP2D(I,J,16)= EXP2D(I,J,16)+EXP_TURB*86400
      endif

! export de mod en mmolN m-2 j-1
       if(vb==iMODN) then
!       if(x2.gt.0.) then
         EXP2D(I,J,17)= EXP2D(I,J,17)+( EXP_ADV)*86400
         EXP2D(I,J,18)= EXP2D(I,J,18)+( EXP_TURB)*86400
!       endif
       endif
...
! export de PON en mmolN m-2 j-1
       if(vb==iDIAN.or.vb==iNANON.or.vb==iSYNEN.or. &
          vb==iLMOPN.or.vb==iSMOPN) then
!       if(x2.gt.0.) then
         EXP2D(I,J,19)= EXP2D(I,J,19)+(EXP_SED)*86400
...
! export de lammonium en mmolN m-2 j-1
       if(vb==iAMMONIUM) then
!       if(x2.gt.0.) then
         EXP2D(I,J,22)= EXP2D(I,J,22)+( EXP_ADV)*86400 !/x2
         EXP2D(I,J,23)= EXP2D(I,J,23)+( EXP_TURB)*86400 !/x2
!       endif
       endif
...
! export de ZOON en mmolN m-2 j-1
      if(vb==iZOONANOC.or.vb==iZOOMESOC.or.vb==iZOOMICROC) then
!       if(x2.gt.0.) then
         EXP2D(I,J,24)= EXP2D(I,J,24)+( EXP_ADV)*0.18*86400 !/x2
         EXP2D(I,J,25)= EXP2D(I,J,25)+( EXP_TURB)*0.18*86400 !/x2
!       endif
       endif
...
! export de BACTN en mmolN m-2 j-1
       if(vb==iBACTC) then
!       if(x2.gt.0.) then
         EXP2D(I,J,26)= EXP2D(I,J,26)+( EXP_ADV)*0.232*86400 !/x2
         EXP2D(I,J,27)= EXP2D(I,J,27)+( EXP_TURB)*0.232*86400 !/x2
       endif
...
! import de Nitrate de mod en mgC m-2 j-1
      if(vb==iNitrate) then
         EXP2D(I,J,28)= EXP2D(I,J,28)+EXP_ADV*86400
         EXP2D(I,J,29)= EXP2D(I,J,29)+EXP_TURB*86400
      endif

! export de mod en mmolN m-2 j-1
       if(vb==iMODN) then
!       if(x2.gt.0.) then
         EXP2D(I,J,30)= EXP2D(I,J,30)+( EXP_ADV)*86400
         EXP2D(I,J,31)= EXP2D(I,J,31)+( EXP_TURB)*86400
!       endif
       endif
...
! export de PON en mmolN m-2 j-1
       if(vb==iDIAN.or.vb==iNANON.or.vb==iSYNEN.or. &
          vb==iLMOPN.or.vb==iSMOPN) then
!       if(x2.gt.0.) then
         EXP2D(I,J,32)= EXP2D(I,J,32)+(EXP_SED)*86400
         EXP2D(I,J,33)= EXP2D(I,J,33)+( EXP_ADV)*86400 !/x2
         EXP2D(I,J,34)= EXP2D(I,J,34)+( EXP_TURB)*86400 !/x2
!       endif
       endif
...
! export de lammonium en mmolN m-2 j-1
       if(vb==iAMMONIUM) then
!       if(x2.gt.0.) then
         EXP2D(I,J,35)= EXP2D(I,J,35)+( EXP_ADV)*86400 !/x2
         EXP2D(I,J,36)= EXP2D(I,J,36)+( EXP_TURB)*86400 !/x2
!       endif
       endif
...
! export de ZOON en mmolN m-2 j-1
      if(vb==iZOONANOC.or.vb==iZOOMESOC.or.vb==iZOOMICROC) then
!       if(x2.gt.0.) then
         EXP2D(I,J,37)= EXP2D(I,J,37)+( EXP_ADV)*0.18*86400 !/x2
         EXP2D(I,J,38)= EXP2D(I,J,38)+( EXP_TURB)*0.18*86400 !/x2
!       endif
       endif
...
! export de BACTN en mmolN m-2 j-1
       if(vb==iBACTC) then
!       if(x2.gt.0.) then
         EXP2D(I,J,39)= EXP2D(I,J,39)+( EXP_ADV)*0.232*86400 !/x2
         EXP2D(I,J,40)= EXP2D(I,J,40)+( EXP_TURB)*0.232*86400 !/x2
       endif

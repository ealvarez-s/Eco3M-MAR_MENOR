...
!Phyto
	...
	TotalExcZooP	     =  0.D0
        ...
        TotalUptBactP        =  0.D0
        TotalBactExcNH4      =  0.D0
        TotalBactExcPO4      =  0.D0
        TotalRespBact        =  0.D0
...
! Nano-zooplankton grazing
		...
		fZooNano = Tempfaczoo  &
              / (kZooNano*(prefNanoBact*BactC(I)+prefNanoSyne*SyneC(I)+prefNanoNano*NanoC(I)  &
                   +prefNanoZooNano*ZooNanoC(I)    ) +   &
                           prefNanoBact*BactC(I)**2 + prefNanoSyne*SyneC(I)**2 +prefNanoNano*NanoC(I)**2 &
                          +prefNanoZooNano*ZooNanoC(I)**2 )
...
! Micro-zooplankton grazing
        ...
        Test = prefMicroZooMicro*ZooMicroC(I)    + prefMicroDia*DiaC(I)    +prefMicroZooNano*ZooNanoC(I)  &
                  +prefMicroNano*NanoC(I) &
              +prefMicroSyne*SyneC(I)    + prefMicroBact*BactC(I)     + prefMicroSMOP*S_MOPC(I)   
...
!        endif
        ...
        ! mmolC/mgChl/hr
        fppbSyne   = f_pproduction(AbsChlSyne,PhymaxSyne,Sig_Ps2Syne,PARZ,kd,krep,Tau_renewSyne,Gamma_M,TempfacppbSyne)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
        ...
        ExuSyneNit  = UptSyneNit  * (1. - NCLimSyne)
...
! Dissolved Inorganic Matter
        ...
        PPBi(I)      = (PPBSyne + PPBNano + PPBDia)* Thickness(I) * 12. * 24. ! mgC/m2/d
        ...
        RespCi(I)    = (RespBact                                                           &
                     ...
                     + ZooMesoGrazNetGrowthEffC  / kcNetGrowthEff *(1- kcNetGrowthEff))* Thickness(I) * 12. * 24.
        ExuCi(I)     = (ExuC)* Thickness(I) * 12. * 24.
        MortCi(I)    = (MortSyneC + MortNanoC + MortDiaC)* Thickness(I) * 12. * 24. ! mgC/m2/d
...
! Phyt
	...
	v2PPBTotale(I)  = (PPBSyne + PPBNano + PPBDia) * Thickness(I) * 12. * 24.
        ...
        TotalNetPPBSyne         =  TotalNetPPBSyne  &
                                + (PPBSyne - ExuSyneC - MortSyneC - RespSyneC - RespSyneNit - RespSyneAmmo - RespSyneP) &
                               * Thickness(I) * 12. * 24.											! mgC/m2/d
        TotalNetPPBDia          =  TotalNetPPBDia   & 
                               + (PPBDia  - ExuDiaC  - MortDiaC - RespDiaC - RespDiaNit - RespDiaAmmo &
                                            - RespDiaP - RespDiaSi) &								!
                               * Thickness(I) * 12. * 24.											! mgC/m2/d
        TotalNetPPBNano         =  TotalNetPPBNano &
                               + (PPBNano - ExuNanoC - MortNanoC - RespNanoC - RespNanoNit - RespNanoAmmo - RespNanoP) &
                               * Thickness(I) * 12. * 24.											! mgC/m2/d
...
!**********************************************************************
   ...
   DOUBLE PRECISION FUNCTION  f_pproduction (AbsChl,PhyMax,Sig_Ps2,EPAR,kd,kr,Tau_renew,Gamma_M,Tempfacppb)

   IMPLICIT NONE
   DOUBLE PRECISION, INTENT (IN) :: AbsChl,Phymax,Sig_Ps2,EPAR,kd,kr,Tau_renew,Gamma_M,Tempfacppb
   DOUBLE PRECISION :: P0	
   ...
   f_pproduction = Tempfacppb * Gamma_M * AbsChl * Phymax * p0 * EPAR * 3600.	                     

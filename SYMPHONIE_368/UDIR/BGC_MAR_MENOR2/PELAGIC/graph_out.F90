...
!      include 'netcdf.inc'
      ...
      if( int(elapsedtime_now/graphperiod_supp)      &
         -int(elapsedtime_bef/graphperiod_supp)==1 .or.  &
         elapsedtime_now==0.) then !--> la dernière condition doit pouvoir être plus robuste) then !-->

        if(     imodeltrc==1  &
            .or.imodelbio==1  &
            .or.l_sedim_s==.true.) call graph_out_bio_supp   !08-11-14
      ...
      if( int(elapsedtime_now/graphperiod)          &
          -int(elapsedtime_bef/graphperiod)/=1 .and. &
           elapsedtime_now/=0. ) then !-->  la dernière condition doit pouvoir être plus robuste
        ! if(par%rank==0) print*,'on s arrete ici'
         return
      endif !-->
...
!     if(allocated(variance_timeaveraged).and.iteration2d==iteration2d_max_now+1) then !pmx>
...
!     if(allocated(flx2d_timeaveraged).and.iteration2d==iteration2d_max_now+1) then !pmx>
      if(allocated(flx2d_timeaveraged)) then !pmx>
      if(loop_netcdf==1) then !=======>
       do j=1,jmax ; do i=1,imax+1
        anyvar2d(i,j)=flx2d_timeaveraged(i,j)*mask_u(i,j,kmax)+(1-mask_u(i,j,kmax))*filval
       enddo ; enddo
      endif                  !=======>
      ...
      if(loop_netcdf==1) then !=======>
       do j=1,jmax+1 ; do i=1,imax
        anyvar2d(i,j)=fly2d_timeaveraged(i,j)*mask_v(i,j,kmax)+(1-mask_v(i,j,kmax))*filval
       enddo ; enddo
      endif                  !=======>
...
! Surface eulerian current: 18-01-16
      if(loop2_==10)then        !------->

      if(loop_netcdf==1) then !=======>
       do j=1,jmax
       do i=1,imax+1
        anyvar2d(i,j)=vel_u(i,j,kmax,0)*mask_u(i,j,kmax) & ! t=before car corrigE du mode externe
                              -9999.*(1-mask_u(i,j,kmax))
       enddo
       enddo
      endif                  !=======>
      texte80(1)='vel_u_kmax' ; texte80(2)='m/s'
      call netcdf_main('_u')

      if(loop_netcdf==1) then !=======>
       do j=1,jmax+1
       do i=1,imax
        anyvar2d(i,j)=vel_v(i,j,kmax,0)*mask_v(i,j,kmax) & ! t=before car corrigE du mode externe
                              -9999.*(1-mask_v(i,j,kmax))
       enddo
       enddo
      endif                  !=======>
      texte80(1)='vel_v_kmax' ; texte80(2)='m/s'
      call netcdf_main('_v')
...
!         anyvar3d(i,j,k)=-(grav/1024.4)*(tem_t(i,j,k,1)+1000.)*mask_t(i,j,kmax) & ! Baroclinic jet case
...
! Baroclinic jet case:
!     texte80(1)='b'   ; texte80(2)='m.s**-2'
!     texte80(3)='-grav*rh/1024.4'
!     texte80(4)='-grav*rh/1024.4'
      call netcdf_main('_t')
...
!******************************************************************************
      ...
      if(loop_netcdf==1) then !=======>
        do k=1,kmax ; do j=0,jmax+1 ; do i=0,imax+1
          anyvar3d(i,j,k)=sal_t(i,j,k,1)*mask_t(i,j,k) & !20-01-17
                               -9999.*(1-mask_t(i,j,k))
        enddo ; enddo ; enddo
      endif                  !=======>
...
!******************************************************************************
      ...
      if(loop_netcdf==1) then !=======>
        if(eos_author==0) then !eeee> !18-11-14
         ...
         call equation_of_state_potzref_jmfwg('grp',1) ! returns potential density in anyv3d(:,:,:,1) !20-10-14
                                                      ...
                                                      ! note: zref will be max(eos_tkezref,0)
        do k=1,kmax ; do j=1,jmax ; do i=1,imax
      ...
      texte80(3)='sea_water_potential_density_reference_level_' &
      ...
      if(loop_netcdf==1) then !=======>
! On profite de ce que rhp_t vient A l'instant d'etre calculE:
...
! b=-grav*rhp/rho0
      ...
      if(loop_netcdf==1) then !=======>
         do k=1,kmax ; do j=1,jmax ; do i=1,imax
          anyvar3d(i,j,k)=mask_t(i,j,k)*(rhpref_t(i,j,k)+rho-1000.) &
                     -(1.-mask_t(i,j,k))*9999.
         enddo ; enddo ; enddo
      endif                  !=======>
      texte80(1)='rhpref_t'     ; texte80(2)='kg/m3'    
      texte80(3)='background_sea_water_density_at_t_nodes'
      texte80(4)=texte80(3) ; texte80(11)='mask_obc_z1' !14-01-17
      call netcdf_main('_t')

      if(loop_netcdf==1) then !=======>
         do k=1,kmax ; do j=1,jmax   ; do i=1,imax+1
          anyvar3d(i,j,k)=mask_u(i,j,k)*(rhpref_u(i,j,k)+rho-1000.) &
                     -(1.-mask_u(i,j,k))*9999.
         enddo ; enddo ; enddo
      endif                  !=======>
      texte80(1)='rhpref_u'     ; texte80(2)='kg/m3'
      texte80(3)='background_sea_water_density_at_u_nodes'
      texte80(4)=texte80(3)
      call netcdf_main('_u')

      if(loop_netcdf==1) then !=======>
         do k=1,kmax ; do j=1,jmax+1 ; do i=1,imax  
          anyvar3d(i,j,k)=mask_v(i,j,k)*(rhpref_v(i,j,k)+rho-1000.) &
                     -(1.-mask_v(i,j,k))*9999.
         enddo ; enddo ; enddo
      endif                  !=======>
      texte80(1)='rhpref_v'     ; texte80(2)='kg/m3'
      texte80(3)='background_sea_water_density_at_v_nodes'
      texte80(4)=texte80(3)
      call netcdf_main('_v')
#endif
...
!******************************************************************************
      ...
      if(loop_netcdf==1) then !=======>
        call equation_of_state_potzref_jmfwg('obc',1) ! returns potential density in anyv3d(:,:,:,1) !14-12-14
                                                      ! note: zref will be max(eos_tkezref,0)
        do k=1,kmax
        do j=1,jmax
        do i=1,imax
...
! REFERENCE FIELDS FOR T,S,RH
      if(loop2_==17)then !17-17-17> !18-01-16
...
! TEMPERATURES DE REFERENCE:
      ...
      if(loop_netcdf==1) then !=======>
        do k=1,kmax ; do j=0,jmax+1 ; do i=0,imax+1
          anyvar3d(i,j,k)=temref_t(i,j,k)*mask_t(i,j,kmax) &
                                -9999.*(1-mask_t(i,j,kmax))
        enddo ; enddo ; enddo
      endif                  !=======>
      texte80(1)='temref_t'   ; texte80(2)='degrees_Celsius'
      texte80(3)='temref_t hihihi!'
      texte80(4)='temref_t hihihi!'
      call netcdf_main('_t')

      if(loop_netcdf==1) then !=======>
        do k=1,kmax ; do j=0,jmax+1 ; do i=0,imax+1
          anyvar3d(i,j,k)=temref_z(i,j,k)*mask_t(i,j,kmax) &
                                -9999.*(1-mask_t(i,j,kmax))
        enddo ; enddo ; enddo
      endif                  !=======>
      texte80(1)='temref_z'   ; texte80(2)='degrees_Celsius'
      texte80(3)='temref_z hihihi!'
      texte80(4)='temref_z hihihi!'
      call netcdf_main('_t')
      ...
      if(loop_netcdf==1) then !=======>
         ...
         do j=1,jmax   !30-07-14
         do i=1,imax+1
          anyvar3d(i,j,k)=mask_u(i,j,k)*temref_u(i,j,k) &
                     -(1.-mask_u(i,j,k))*9999.
         enddo
         enddo
         enddo
      endif                  !=======>
      texte80(1)='temref_u'     ; texte80(2)='degc'                       ! variable ; units
      call netcdf_main('_u')
      ...
      if(loop_netcdf==1) then !=======>
         ...
         do j=1,jmax+1 !30-07-14
         do i=1,imax  
          anyvar3d(i,j,k)=mask_v(i,j,k)*temref_v(i,j,k) &
                     -(1.-mask_v(i,j,k))*9999.
         enddo
         enddo
         enddo
      endif                  !=======>
      texte80(1)='temref_v'     ; texte80(2)='degc'                       ! variable ; units
      call netcdf_main('_v')

! SALINITES DE REFERENCE:

      if(loop_netcdf==1) then !=======>
        do k=1,kmax ; do j=0,jmax+1 ; do i=0,imax+1
          anyvar3d(i,j,k)=salref_t(i,j,k)*mask_t(i,j,kmax) &
                                -9999.*(1-mask_t(i,j,kmax))
        enddo ; enddo ; enddo
      endif                  !=======>
      texte80(1)='salref_t'   ; texte80(2)='1e-3'
      texte80(3)='salref_t hihihi!'
      texte80(4)='salref_t hihihi!'
      call netcdf_main('_t')

      if(loop_netcdf==1) then !=======>
        do k=1,kmax ; do j=0,jmax+1 ; do i=0,imax+1
          anyvar3d(i,j,k)=salref_z(i,j,k)*mask_t(i,j,kmax) &
                                -9999.*(1-mask_t(i,j,kmax))
        enddo ; enddo ; enddo
      endif                  !=======>
      texte80(1)='salref_z'   ; texte80(2)='1e-3'
      texte80(3)='salref_z hihihi!'
      texte80(4)='salref_z hihihi!'
      call netcdf_main('_t')
      ...
      texte80(1)='salref_u'     ; texte80(2)='1e-3'                       ! variable ; units
      call netcdf_main('_u')
      ...
      texte80(1)='salref_v'     ; texte80(2)='1e-3'                       ! variable ; units
      call netcdf_main('_v')
#endif
...
!******************************************************************************
      ...
      if(loop_netcdf==1) then !=======>
        ...
        do i=1,imax+1                     
         if (mask_u(i,j,kmax  ).ne.0) then
          anyvar3d(i,j,k)=  &
       abs((vel_u(i,j,k,1)+velstokes_u(i,j,k,1))*dy_u(i,j)*dz_u(i,j,k,1)) &
      *max(dti_lp*wetmask_t(i  ,j)/(dxdy_t(i  ,j)*dz_t(i  ,j,k,0))  &
          ,dti_lp*wetmask_t(i-1,j)/(dxdy_t(i-1,j)*dz_t(i-1,j,k,0)))
         else
          anyvar3d(i,j,k)=-9999.
         endif
        enddo
      ...
      texte80(1)='vel_u_dtodx' !12-01-17 
      texte80(2)='none'    
      texte80(3:4)='Oi_current_number_for_tracers'
      call netcdf_main('_u')
      ...
      if(loop_netcdf==1) then !=======>
        ...
        do i=1,imax
         if (mask_v(i,j,kmax  ).ne.0) then
          ...
          anyvar3d(i,j,k)=  &
       abs((vel_v(i,j,k,1)+velstokes_v(i,j,k,1))*dx_v(i,j)*dz_v(i,j,k,1)) &
      *max(dti_lp*wetmask_t(i,j  )/(dxdy_t(i,j  )*dz_t(i,j  ,k,0))  &
          ,dti_lp*wetmask_t(i,j-1)/(dxdy_t(i,j-1)*dz_t(i,j-1,k,0)))
      ...
      texte80(1)='vel_v_dtody' !12-01-17 
      texte80(2)='none'    
      texte80(3:4)='Oj_current_number_for_tracers'
      call netcdf_main('_v')
...
!******************************************************************************
      if(loop2_.eq.3)then !------->
      if(iturbulence==0.or.iturbulence==1) then !pmxpmx> !29-03-17
...
!******************************************************************************
      ...
      if    (iturbulence==0    &
         .or.iturbulence==1    &
         .or.iturbulence==3) then !pmxpmx> !29-03-17

...
!        open(unit=3,file='espartel_30042019')
         open(unit=3,file='espartel_04052019')
         do j=575,555,-1
...
!     if(par%rank==0)write(6,*)'85 565 sortie de lecture ',glob_h(85,565)
...
! Remove secondary bassins:
      if(flag_remove_secondary_bassins==1) & !28-05-17
      call remove_secondary_bassins          !30-04-11

! Add Water ways:
!     call set_rivers(2) !07-03-17

!......................
! Dans le cas d'un decoupage en sous domaine, initialiser le masque
! et la bathy pour chaque sous-domaine:
...
! rdv les options sur H:
      ...
      if(ioption.eq.1.and.i1d.ne.0) then                               !19/01/07


!................................................................
! Bornes mini et maxi (depend de la distance aux OBC):
      call initial_sponge_hmin ! 19-07-13
...
! Archiver l'ecart A la bathy brute dans un fichier:
      write(texte30,'(a,i0)')'tmp/err_h',par%rank
...
! qui entraine la fonction par_gatherall_2d en suivant
      ...
      call set_rivers(2) !07-03-17
!................
!     if(par%rank==0)write(6,*)'85 565 sortie de gather ',glob_h(85,565)

! Eciture nouveau fichier
!      if(par%rank==0)then
!      open(unit=3,file='../../../GLOBMED/BATHYMASK/bathy_GLOBMED_curvgib_dard_reparCors2fois_Sicil_Gib_dejaliss.ijh',recl=10000)
!       do i=1,iglb
!       write(3,'(100000i1)')(glob_mask(i,j),j=1,jglb)
!       enddo
!       do j=1,jglb
...
!                       / / /
      ...
      write(3,*)'corresponding grid point:',ihmax,jhmax
      write(3,*)'minimum depth hmin=',hmin,' meters'
      write(3,*)'corresponding grid point:',ihmin,jhmin
      if(iarchive.eq.1) then ! *******************************>        !06/02/01
      write(3,*)'une verification de la bonne imbrication des bathy'
      write(3,*)'peut etre faite en comparant le fichier'
      write(3,*)'bathy_imbriquee avec le fichier bathycote_in.dat'
      write(3,*)'du petit domaine'
      endif                  ! *******************************>        !06/02/01
      close(3)
      endif                !#mpi-->>-->                       !09-05-10
#ifdef parallele
      ...
      do i=1,imax
       ...
       jm1=max0(j-1,1)
        if(  mask_t(i,j,kmax+1).eq.1) then
         ...
         if(ksecu.eq.0) then
          k10=0
          write(6,*)'piscine en ',par%rank,i,j,i+par%timax(1),j+par%tjmax(1) !11-06-17
         endif
        endif
      enddo
      enddo
      if(k10==0) then                                                  !10-06-09
      write(6,*)
      write(6,*)'probleme d''initialisation de la grille:'
      write(6,*)'il faut boucher les piscines du fichier masque.'
      write(6,*)'modifier le fichier puis relancer.'
!     stop 'en attendant: stop dans bathycote.f'
      endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine remove_secondary_bassins
      use module_principal
      use module_parallele !#MPI
      use module_global
      implicit none
...
#ifdef synopsis
       subroutinetitle='remove_secondary_bassins'
       subroutinedescription= &
          'Subroutine called from initial_mask_and_bathy subroutine.' &
       //' Apply a land mask value to grid points located in'         &
       //' secondary independant domains'
       call main_synopsis(subroutinetitle,subroutinedescription)
#endif
      ...
      if(glob_mask(i1,j1)==1)then !11111111111111111111>
        ...
        loop_=loop_+1
        glob_mask(i1,j1)=loop_
        if(i1-par%timax(1)>=0.and.      &
           i1-par%timax(1)<=imax+1.and. &
           j1-par%tjmax(1)>=0.and.      &
           j1-par%tjmax(1)<=jmax+1)mask_t(i1-par%timax(1),j1-par%tjmax(1),kmax)=loop_
        sum1=0.
        sum2=0.
 1234   continue

        do j=1,jmax ; do i=1,imax
         if(mask_t(i,j,kmax)==loop_)then !---->
              if(mask_t(i+1,j  ,kmax)/=0)mask_t(i+1,j  ,kmax)=loop_
              if(mask_t(i-1,j  ,kmax)/=0)mask_t(i-1,j  ,kmax)=loop_
              if(mask_t(i  ,j+1,kmax)/=0)mask_t(i  ,j+1,kmax)=loop_
              if(mask_t(i  ,j-1,kmax)/=0)mask_t(i  ,j-1,kmax)=loop_
         endif                           !---->
        enddo       ; enddo
        call obc_mask2d_mpi('za',kmax)

        sum2=sum1
        sum1=0.
        do j=1,jmax
        do i=1,imax
         if(mask_t(i,j,kmax)*mask_i_w(i)*mask_j_w(j)==loop_) then
           sum1=sum1+1.
         endif
        enddo
        enddo
...
!............................
! POUR VERIF (commenter sinon)
!     if(par%rank==0) then !ooo>
!      do i=1,iglb ; do j=1,jglb
!       if(glob_mask(i,j)==loop_)write(66,*)i,j
!      enddo ; enddo
!     endif                !ooo>
!     call mpi_barrier(par%comm2d,k_out)      ! synchro processes      !09-05-10
!............................
      ...
      do j=1,jglb
      do i=1,iglb
       if(glob_mask(i,j)==biggest_bassin_) then
          glob_mask(i,j)=1
       else
          glob_mask(i,j)=0
       endif
...
! Debug dims:
       ...
       if(istr_==-9999)stop 'Err dim2 netcdf LSM H File'
...
! Lire glob_mask
       ...
       status=nf_get_vara_int(ncid_,var_id,varstart(1:var_dims_)   &
                                          ,varcount(1:var_dims_)   &
                             ,glob_mask(istr_:istp_,jstr_:jstp_))  !13-06-15

       else                 !>>>>>

       if(k0==nf_double)then !pmxpmx> !10-09-16

       allocate(glob_mask_r8(istr_:istp_,jstr_:jstp_))
       status=nf_get_vara_double(ncid_,var_id,varstart(1:var_dims_)   &
                                             ,varcount(1:var_dims_)   &
                   ,glob_mask_r8(istr_:istp_,jstr_:jstp_))
                    glob_mask   (istr_:istp_,jstr_:jstp_)   &
              =nint(glob_mask_r8(istr_:istp_,jstr_:jstp_))
         deallocate(glob_mask_r8)

       endif                 !pmxpmx>

       allocate(glob_mask_i1(istr_:istp_,jstr_:jstp_))
       status=nf_get_vara_int1(ncid_,var_id,varstart(1:var_dims_)   &
                                           ,varcount(1:var_dims_)   &
                   ,glob_mask_i1(istr_:istp_,jstr_:jstp_))
                    glob_mask   (istr_:istp_,jstr_:jstp_)   &
                  =(glob_mask_i1(istr_:istp_,jstr_:jstp_))
         deallocate(glob_mask_i1)
...
! Lire glob_h
       ...
       status=nf_inq_vartype(ncid_,var_id,k0)     ; if(k0/=nf_double.and.k0/=nf_real)stop 'Err1897 uncorrect vartype'

       if(k0==nf_double)then !>>>>>
       ...
       allocate(glob_h_r4(istr_:istp_,jstr_:jstp_))
       status=nf_get_vara_real(ncid_,var_id,varstart(1:2)   &
                                           ,varcount(1:2)   &
                      ,glob_h_r4(istr_:istp_,jstr_:jstp_))
                       glob_h   (istr_:istp_,jstr_:jstp_)   &
                      =glob_h_r4(istr_:istp_,jstr_:jstp_)
            deallocate(glob_h_r4)
...
! des corrections de grande longueur d'onde
      ...
      if(index(texte250,'.ijDeltaH')/=0) then ! m[°v°]m >
      ksecu=1
...
! La correction est dh=delta_h*exp( -(distance/d0)**2 )
 ...
 2265   read(3,*,end=2263)i1,j1,x3,i10 
        i1=i1-par%timax(1) ; j1=j1-par%tjmax(1)
        x0=-(1./real(i10))**2
        do j=lb2(2),ub2(2) ; do i=lb2(1),ub2(1)
         h_w(i,j)=h_w(i,j)+x3*exp(x0*( (i-i1)**2 + (j-j1)**2 ))
!        h_w(i,j)=1000.   +x3*exp(x0*( (i-i1)**2 + (j-j1)**2 )) ! Pour test
        enddo              ; enddo
        goto 2265
 2263  close(3)

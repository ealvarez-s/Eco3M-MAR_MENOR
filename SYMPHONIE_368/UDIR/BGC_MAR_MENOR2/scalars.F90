...
! KSL_Z(I,J) est le niveau vertical correspondant à la base de la couche
! de mélange:
...
! de reduire de 90% l'ecart à la reference en un temps equivalent à
        ...
        do i=spo_i1_t(loop1),spo_i2_t(loop1) ! debut boucle i
         if(  mask_t(i,j,kmax+1).eq.1) then  !>>>>>>>>>>>>>>>>
          ...
          do k=kmin_w(i,j),kmax                ! debut boucle k
           ...
           tem_t(i,j,k,2)=tem_t(i,j,k,2)*x6+temobc_t(i,j,k,1)*x5
           sal_t(i,j,k,2)=sal_t(i,j,k,2)*x6+salobc_t(i,j,k,1)*x5
...
!---------------------------------------------------------------------------------------------------------
#ifdef bidon
      subroutine test_advection_tken
      use module_principal
      use module_parallele
      implicit none
      real :: current_number_=0.1
...
! Initialiser un tourbillon:
         ...
         tken_w(i,j,k)=1.+dist1+x3*cos(pi*i1)*cos(pi*j1)*dist1
         tkea_w(i,j,k)=1.+dist1+x3*cos(pi*i1)*cos(pi*j1)*dist1
         epsn_w(i,j,k)=tken_w(i,j,k)
         epsa_w(i,j,k)=tken_w(i,j,k)
      ...
      k=kmax
      i=nint(real(iglb)/4.)
      open(unit=66,file='tken_init')
      do j=1,jmax
       write(66,*)j,tken_w(i,j,k)
      enddo
      close(66)
...
!___________________________________________________
! Module du courant horizontal
      x3=current_number_*dxb/dti_fw ! Nb de courant * DX / DT
...
!     call turbulence_gaspar_tke
      call turbulence_k_eps
!     call turbulence_adv_tken
!     call turbulence_adv_tkea ; tken_w=tkea_w
!     call turbulence_adv_epsa ; epsn_w=epsa_w
      dz_t(:,:,:,0)=dz_t(:,:,:,1)
      dz_t(:,:,:,1)=dz_t(:,:,:,2)
...
! Verifi de conservation:
       ...
       sum1=sum1+tkea_w(i,j,k)*dxdy_t(i,j) &
                *(0.25*( dz_t(i,j,k-1, 0)+dz_t(i,j,k, 0) &
                        +dz_t(i,j,k-1, 1)+dz_t(i,j,k, 1) &
                 )     )
       sum2=sum2+epsa_w(i,j,k)*dxdy_t(i,j) &
                *(0.25*( dz_t(i,j,k-1, 0)+dz_t(i,j,k, 0) &
                        +dz_t(i,j,k-1, 1)+dz_t(i,j,k, 1) &
                 )     )
       enddo ; enddo
 ...
 2000 continue
      ...
      k=kmax
      i=nint(real(iglb)/4.)
      write(texte90,'(a,i0)')'tken_',loop_
      open(unit=66,file=texte90)
      x1=0.
      do j=1,jmax
       write(66,*)j,tkea_w(i,j,k),epsa_w(i,j,k)
       x1=max(x1,tken_w(i,j,k))
      enddo
      close(66)
      write(67,*)current_number_,x1
      ...
      write(6,*)'dans sbr test_advection_bio'
      write(6,*)'-----------------------'
      stop 'test_advection_tken'


      end subroutine test_advection_tken

...
! S model
! release S26.1 - last update: 11-03-16
!______________________________________________________________________
!...............................................................................
...
!...............................................................................
      ...
      use module_optics !24-05-15
      use module_atmboundlayer
      implicit none
      include 'netcdf.inc'
        ...
        if(iairsea==2.or.iairsea==3) then !-=-=-=-=-=->
         if(kmax==1)then !-2Dcase-2Dcase-> ! 2D run assumes SST=T0m since SST
          if(iteration3d/=0) then !>>>>             !03-02-15
           ...
           if(flag_meteodata=='ecmwf')call airseaflux_ecmwfbk(case_)
        endif                             !-=-=-=-=-=->
...
!..............................................................................
      ...
      do j=1,jmax ; do i=1,imax
       omega_w(i,j,kmaxp1,1)=-mask_t(i,j,kmax)     &
                          *wetmask_t(i,j)          &       !31-10-15 
                         *(   slhf_w(i,j,1)*const1 &
                          +precipi_w(i,j,1)      )
       enddo ; enddo
! Note: wetmask_t(i,j) est sur la valeur de l'iteration precedente (et
! sur wetdry_cst3 (et non wetdry_cst1 comme les autres) 
...
! rembobiner la liste binaire de 1 (que le airseaflux_interp_driver precedent incrementa)
          ...
          do j=0,jmax+1 ; do i=0,imax+1
           ablheight_t(i,j,0)=ablheight_t(i,j,2)
...
! rembobiner la liste binaire de 1 (que le airseaflux_interp_driver precedent incrementa)
            ...
            do j=0,jmax+1 ; do i=0,imax+1
             uwind100_t(i,j,1)=uwind100_t(i,j,2)
             uwind100_t(i,j,2)=      xy_t(i,j,1)
            enddo; enddo
         endif                !ablablabl>
...
! rembobiner la liste binaire de 1 (que le airseaflux_interp_driver precedent incrementa)
            ...
            do j=0,jmax+1 ; do i=0,imax+1
             vwind100_t(i,j,1)=vwind100_t(i,j,2)
             vwind100_t(i,j,2)=      xy_t(i,j,1)
            enddo; enddo
         endif                !ablablabl>
        ...
        if(decision==1) then
         ...
         enddo; enddo
         pss_mean(0)=pss_mean(2) ; pss_mean(2)=101300. ! pression de reference constante
        endif
...
! et non pas a chaque iteration du modele S
        ...
        if( flag_phys1_==1)call airseaflux_physics(1) ! > teta2=temp pot
...
!      write(6,*)' BIDOUILLE SUR UWIND100 '
!      write(6,*)' BIDOUILLE SUR ABLHEIGHT '
!       do j=0,jmax+1
!       do i=0,imax+1
!        uwind100_t(i,j,:)=uwind_t(i,j,1)
!        vwind100_t(i,j,:)=vwind_t(i,j,1)
!        ablheight_t(i,j,0)=500.+real(i+par%timax(1))
!        ablheight_t(i,j,2)=500.+real(j+par%tjmax(1))
!       enddo
!       enddo
...
!..............................................................................
      ...
      if(flag_abl==1) then !>>>>>>>
       ...
       do j=0,jmax+1 ; do i=0,imax+1
        xy_t(i,j,1)=gridrotcos_t(i,j)*uwind100_t(i,j,2)       &
                   -gridrotsin_t(i,j)*vwind100_t(i,j,2)
        xy_t(i,j,2)=gridrotsin_t(i,j)*uwind100_t(i,j,2)       &
                   +gridrotcos_t(i,j)*vwind100_t(i,j,2)
       enddo ; enddo
       do j=0,jmax+1 ; do i=0,imax+1
        uwind100_t(i,j,2)=xy_t(i,j,1)
        vwind100_t(i,j,2)=xy_t(i,j,2)
       enddo ; enddo
...
!..............................................................................
      ...
      meteovarname(1:5)='undefined'

        if(loop_==ssr_id) then
         meteovarname(1)='SSR' ; meteovarname(2)='SSRD'  ! albedo impacted
         meteovarname(3)='ssr' ; meteovarname(4)='ssrd'  ! albedo impacted
         meteovarname(5)='rsds'
         return
        endif
        ...
        if(loop_==dp2m_id) then
         meteovarname(1)='D2M' ; meteovarname(2)='d2m'
         return
        endif
        if(loop_==p0m_id) then
         meteovarname(1)='SP' ; meteovarname(2)='sp'
         return
        endif
        if(loop_==ir_id) then
        ...
        if(loop_==u100m_id) then
         meteovarname(1)='u100'  ; meteovarname(2)='U100'
         meteovarname(3)='u100m' ; meteovarname(4)='U100M'
         return
        endif
        if(loop_==v100m_id) then
         meteovarname(1)='v100'  ; meteovarname(2)='V100'
         meteovarname(3)='v100m' ; meteovarname(4)='V100M'
         return
        endif
...
!     if(loop_==ssr_id) then                                         !----->
!     status=nf_get_att_text(ncid_,varid_,'long_name',texte90) ;if(status/=0)stop 'echec attribut longname pour ssr'
!      if(index(texte90,'downward')/=0) &
!                                then ; ialbedo=1 ; else ; ialbedo=0 ; endif
!     endif                                                          !----->
!      write(6,'(a,a)')'Fichier=',trim(texte80(1))
...
!      write(6,'(a,a)')'meteovarname=',meteovarname(1)
                   ...
                   status=nf_inq_varid(ncid_,meteovarname(1),varid_)
      if(status/=0)status=nf_inq_varid(ncid_,meteovarname(2),varid_)
      if(status/=0)stop 'Erreur nf_inq_varid airseaflux_inquire_var 2'
      status=nf_get_att_text(ncid_,varid_,'units',texte90);if(status/=0)stop 'echec attribut units pour ssr'
...
! il faut continuer de normaliser par time1- - time2_ !14-02-16
            ...
            enddo ; enddo
!           if(var_id_==1)write(68,*)elapsedtime_now,meteo_var(1,1)
...
!     if(loop_==1)then
!      write(68,*)'----------------------'
!      write(68,*)year_now,month_now,day_now,hour_now
!      write(68,'(a,a)')trim(texte80(1))
!     endif
...
!     if(status.ne.0)stop 'echec id land_sea_mask'
      if(status.ne.0) then
         if(flag_lsm==0)  stop 'echec id land_sea_mask'
...
!       deci=ij2meteo_i(i,j) ; i1=int(deci) ; rapi=deci-i1
!       decj=ij2meteo_j(i,j) ; j1=int(decj) ; rapj=decj-j1
        i1=int( ij2meteo_i(i,j) - (meteozoom_istr-1) ) !21-03-15
        j1=int( ij2meteo_j(i,j) - (meteozoom_jstr-1) )
        rapi=ij2meteo_i(i,j)-int(ij2meteo_i(i,j))
        rapj=ij2meteo_j(i,j)-int(ij2meteo_j(i,j))

              if(i1<1)         stop 'faute meteo 1'
              if(i1>meteo_imax)stop 'faute meteo 2'
              if(j1<1)         stop 'faute meteo 3'
              if(j1>meteo_jmax)stop 'faute meteo 4'
! si le modele plante sur fautes 1 2 3 ou 4 c'est que la grille du modele meteo
! n'englobe pas completement celle du modele oceanique

!             if(meteo_var(i1  ,j1  )==1.)meteo_var(i1  ,j1  )=-1.
!             if(meteo_var(i1  ,j1+1)==1.)meteo_var(i1  ,j1+1)=-1.
!             if(meteo_var(i1+1,j1+1)==1.)meteo_var(i1+1,j1+1)=-1.
!             if(meteo_var(i1+1,j1  )==1.)meteo_var(i1+1,j1  )=-1.
              if(meteo_var(i1  ,j1  )>0.)meteo_var(i1  ,j1  )=-1.   !18-09-12
              if(meteo_var(i1  ,j1+1)>0.)meteo_var(i1  ,j1+1)=-1.
              if(meteo_var(i1+1,j1+1)>0.)meteo_var(i1+1,j1+1)=-1.
              if(meteo_var(i1+1,j1  )>0.)meteo_var(i1+1,j1  )=-1.
...
! Trouver les boucheurs:
      open(unit=4,file=trim(tmpdirname)//'bouchetrou_meteo_'//dom_c//'.out')   !#MPI
 ...
 1864   continue
        ...
        do 1863 k1=0,1
         ...
         if(i1<1         )stop 'faute meteo 5'  !01-07-10
         if(i1>meteo_imax)stop 'faute meteo 6'
         if(j1<1         )stop 'faute meteo 7'
         if(j1>meteo_jmax)stop 'faute meteo 8'
...
!     if(par%rank==0)write(6,*)'bouchage des trous de la grille meteo'
      ...
      open(unit=3,file=trim(tmpdirname)//'bouchetrou_meteo_'//dom_c//'.out')   !#MPI
      do j1=1,meteo_jmax
      do i1=1,meteo_imax
...
!     double precision valr8_(8)
      integer ncid_,flag_                                 &
             ...
             ,loop1_,linemax_,loop2_,loop3_,vartype_
...
! name of the record direct access binary file list:
        k2=index(airseafile(loop1_),'/',back=.true.) ; k3=index(airseafile(loop1_),' ')
        airseabinreclist(loop1_)=trim(tmpdirname)//''//airseafile(loop1_)(k2+1:k3-1)//'.binrec'
...
! Get max_meteo_time_counter:
          ...
          if(status==0)status=nf_inq_dimlen(ncid_,dim_t_id,max_meteo_time_counter)
! Get time units:
...
! write in the ascii temporary file:
           ...
           ksecu=0
           if(loop1_/=1)ksecu=1 
           if(loop1_==1.and.loop2_==8) then
             ksecu=1 ; time_=time_+43200.
           endif
...
! d'attaquer la concatenation
      ...
      call mpi_barrier(par%comm2d,k_out)
       ...
       if(par%rank==0) then !00000000000>
        time2_=-999.
        open(unit=3,file=airseabinreclist(loop1_) &
            ,access='direct'                      &
            ,recl=232                             &
            ,form='unformatted')
...
! Detection de listes desordonnees: !27-02-15
               if(time2_<time1_) then   !dbdbdbdb>
                ...
                write(6,'(a,a)')'Previous file ',trim(texte80(2))
          ...
          if(time_<=elapsedtime_now) then !>>>> ce test garantit entre autres que model3d ne relancera pas une
           airseafile_nextrec(loop1_)=nc  !     procedure de remise a jour des champs a la premiere iteration
...
!     if(loop1_==1)stop 'riri'
      enddo ! loop1_ sur variables

! Share with all other ranks:
        allocate(tabr8_(nairsea))
        call mpi_allreduce(airseafile_nextime(1:nairsea)  &
                          ,tabr8_(1:nairsea)              &
                          ,nairsea                        &
                          ,mpi_double_precision           &
                          ,mpi_sum                        &
                          ,par%comm2d,ierr)
        airseafile_nextime(1:nairsea)=tabr8_(1:nairsea)
        deallocate(tabr8_)
        allocate(tabi4_(nairsea))
        call mpi_allreduce(airseafile_nextrec(1:nairsea)  &
                          ,tabi4_(1:nairsea)              &
                          ,nairsea                        &
                          ,mpi_integer                    &
                          ,mpi_sum                        &
                          ,par%comm2d,ierr)
       airseafile_nextrec(1:nairsea)=tabi4_(1:nairsea)
       deallocate(tabi4_)
...
!...........................................................................
      ...
      subroutine airseaflux_extract_zone
      implicit none
#ifdef synopsis
...
! lire les latitudes:
      ...
      if(var_dims==1) then !1111111111111111111111>
       ...
       meteo_londlt=(meteo_lon(meteo_imax,1)-meteo_lon(1,1))/meteo_imax
       meteo_latdlt=(meteo_lat(1,meteo_jmax)-meteo_lat(1,1))/meteo_jmax
...
! A partir des min & max des (lon,lat) des 2 grilles, reduire la zone d'extraction:
       ...
       if(minval(meteo_lon)>lonmin.or.     &
        ...
        write(6,*)'minval(meteo_lon),maxval(meteo_lon)',minval(meteo_lon),maxval(meteo_lon) & !11-03-15
                 ,'minval(meteo_lat),maxval(meteo_lat)',minval(meteo_lat),maxval(meteo_lat)
        write(6,*)'lonmin,lonmax',lonmin,lonmax,'latmin,latmax',latmin,latmax !11-03-15
        stop 'stop in subroutine airseaflux_extract_zone'
       endif
...
! On refait le test differement. On cherche le point le plus proche.
       if(ksecu==0)then !000000000000000>
        ...
        enddo
        enddo
        meteozoom_istr=i2 ; meteozoom_iend=i2
        meteozoom_jstr=j2 ; meteozoom_jend=j2
...
! que plusieurs grilles meteo (point u, v, t) peuvent etre presentes.
       ...
       meteozoom_istr=max0(meteozoom_istr-i0,1)
       meteozoom_iend=min0(meteozoom_iend+i0,meteo_imax)
       meteozoom_jstr=max0(meteozoom_jstr-i0,1)
       meteozoom_jend=min0(meteozoom_jend+i0,meteo_jmax)
...
! Fermer le fichier netcdf:
      ...
      if(flag_meteodata=='glorys') then !---------->
        ...
        call airseaflux_sgrid_to_meteogrid_perform('t')
      ...
      do loop_=1,nairsea
        ...
        if(loop_==ustrs_id) then !uuuuu>
         ...
         if(flag_gridu_==0) then
          flag_gridu_=1;call airseaflux_sgrid_to_meteogrid_perform('u')
         endif
        ...
        if(loop_==vstrs_id) then !vvvvv>
         ...
         if(flag_gridv_==0) then
          flag_gridv_=1;call airseaflux_sgrid_to_meteogrid_perform('v')
         endif
        ...
        if(loop_/=vstrs_id.and.loop_/=ustrs_id) then !ttttt>
         ...
         if(flag_gridt_==0) then
          flag_gridt_=1;call airseaflux_sgrid_to_meteogrid_perform('t')
         endif
...
!----------------------------------------------------------------------------
      ...
      subroutine airseaflux_sgrid_to_meteogrid_perform(text_)
      implicit none
...
! Fermer le fichier netcdf:
      ...
      allocate(ij2meteo_i   (0:imax+1,0:jmax+1))
      allocate(ij2meteo_j   (0:imax+1,0:jmax+1))
      allocate(ij2meteo_teta(0:imax+1,0:jmax+1))
...
!.............................................................
      ...
      flag_p0m_filter=0 ; relativewind=0. ; flag_abl2=0
...
! Detection des dates min et max pour allocation de certains tableaux
 ...
 200     continue
         ...
         if(status/=0)status=nf_inq_dimid(ncid_,'time_counter',var_id);if(status/=0)stop 'erreur nf_inq_dimid time airsea'
         status=nf_inq_dimlen(ncid_,var_id,max_time_counter) ;if(status/=0)stop 'erreur nf_inq_dimlen airsea'
                      status=nf_inq_varid(ncid_,'time',var_id)
         if(status/=0)status=nf_inq_varid(ncid_,'time_counter',var_id);if(status/=0)stop 'erreur nf_inq_varid time airsea'
         texte60=''
         status=nf_get_att_text(ncid_,var_id,'units',texte60);if(status/=0)stop 'erreur nf_get_att_text airsea'
         k=index(texte60,' since ')
...
!     allocate(sshf_w    (0:imax+1,0:jmax+1,ifb:2-ifb)  ) ; sshf_w=0
!     allocate(slhf_w    (0:imax+1,0:jmax+1,ifb:2-ifb)  ) ; slhf_w=0
!     allocate(wstress_u (0:imax+1,0:jmax+1,ifb:2-ifb)  ) ; wstress_u=0
!     allocate(wstress_v (0:imax+1,0:jmax+1,ifb:2-ifb)  ) ; wstress_v=0
      allocate(sshf_w    (0:imax+1,0:jmax+1,k1:k2)  ) ; sshf_w=0
      allocate(slhf_w    (0:imax+1,0:jmax+1,k1:k2)  ) ; slhf_w=0
      allocate(wstress_u (0:imax+1,0:jmax+1,k1:k2)  ) ; wstress_u=0 !24-11-15
      allocate(wstress_v (0:imax+1,0:jmax+1,k1:k2)  ) ; wstress_v=0 !24-11-15
      ...
      allocate(airseafile_prvtime(dim_airsea)) ;airseafile_prvtime=0.
      allocate(airseafile_nextime(dim_airsea)) ;airseafile_nextime=0.
      allocate(airseafile_prvtrec(dim_airsea)) ;airseafile_prvtrec=0
      allocate(airseafile_nextrec(dim_airsea)) ;airseafile_nextrec=0
      allocate(airseabinreclist  (dim_airsea)) ;airseabinreclist='reset'

      if(flag_abl==1) then !ooo>
        allocate(uwind100_t  ( 0:imax+1, 0:jmax+1,1:2)) ; uwind100_t=0.
        allocate(vwind100_t  ( 0:imax+1, 0:jmax+1,1:2)) ; vwind100_t=0.
        allocate(ablheight_t ( 0:imax+1, 0:jmax+1,0:2)) ; ablheight_t=0.
        allocate(teta0_t     ( 0:imax+1, 0:jmax+1,1:2)) ; teta0_t=0.
!       allocate(teta2delta_t(-1:imax+2,-1:jmax+2,1))   ; teta2delta_t=0.
!       allocate(q2delta_t   (-1:imax+2,-1:jmax+2,1))   ; q2delta_t=0.
        allocate(teta2delta_t( 1:imax  , 1:jmax  ,1))   ; teta2delta_t=0.
        allocate(q2delta_t   ( 1:imax  , 1:jmax  ,1))   ; q2delta_t=0.
        allocate(dt_abl_max  (0:2))                     ; dt_abl_max=0.
        
        if(flag_abl2==1) then !222>
!       allocate(uwinddelta_t( 0:imax+1, 0:jmax+1,1))   ; uwinddelta_t=0.
!       allocate(vwinddelta_t( 0:imax+1, 0:jmax+1,1))   ; vwinddelta_t=0.
        allocate(uwinddelta_t( 1:imax  , 1:jmax  ,1))   ; uwinddelta_t=0.
        allocate(vwinddelta_t( 1:imax  , 1:jmax  ,1))   ; vwinddelta_t=0.
        endif                 !222>
...
!.............................................................
      ...
      subroutine airseaflux_inquire_albedo
      implicit none
...
!      if(par%rank==0)write(6,*)'ialbedo=',ialbedo
...
!..............................................................................
      ...
      subroutine airseaflux_sphum_or_dewp
      implicit none
      integer ncid_,varid_
#ifdef synopsis
...
! Var id = dp2m_id
      ...
      if(status/=0)status=nf_inq_varid(ncid_,meteovarname(2),varid_)
      if(status/=0)stop &
      ' Stop airseaflux_sphum_or_dewp airseaflux_sphum_or_dewp'

...
! release 367 - last update: 28-02-23
!______________________________________________________________________
      ...
      use module_curvgrdtoolbox ; use module_q ; use module_cpl_oasis
      implicit none
#ifdef synopsis
...
! v252    07-04-19 rankcoords est le numero du rank en fonction de i=0:nbdom_imax-1, j=0:nbdom_jmax-1 !07-04-19
! v253    02-05-19 if(nbcanal>0) call webcanals_messages !02-05-19
! v255    18-05-19 message d'erreur
! v257    05-07-19 call my_outputs_zone1salttempflux('init',0) 
! v258    17-07-19 - set_parameters_visco !17-07-19 remis ici car z0_w peut avoir besoin de connaitre h
!                  - call wave_reset apres set_parameters_visco pour connaitre z0s
! v260    20-10-19 ajout subroutine initial_simple_restart
! v284    22-05-20 call set_parameters_postkmin !22-05-20
! v287    23-07-20 call cpl_oasis_grids_pmx
! v289    24-09-20 my_outputs_zonesalttempflux renommE my_outputs_zone1salttempflux
! v292    25-11-20 if(iwve/=0)call wave_driver !04-03-13!25-11-20
! v295    30-01-21 lecture en short dans fichier de redemarrage (initial_simple_restart)
! v310    03-11-21 placer quick_initial apres l'initialisation de la maree pour pouvoir 
!                  beneficier du tidal mixing sur temobc et salobc
! v325    12-02-22 call my_outputs_3dflux_s
! v345    06-04-22 simple_restart_biofile_txt
! v348    21-06-22 call my_outputs_3dflux_s deplace apres l'initialisation de S
! v351    14-08-22 - cas ioffline=3
! v362    06-01-23 Ajout formules bulk ecume
! v367    28-02-23 call drifter_initial ! deplacE pour utiliser un "z" initialisE
!...............................................................................
!    _________                    .__                  .__             !    (°o°) 
...
! rankcoords est le numero du rank en fonction de i=0:nbdom_imax-1, j=0:nbdom_jmax-1 !07-04-19
!     allocate(rankcoords_tmp(0:nbdom_imax-1,0:nbdom_jmax-1)) ; rankcoords_tmp=0
!     rankcoords_tmp(par%coords(1),par%coords(2))=par%rank
!     call mpi_allreduce(rankcoords_tmp,rankcoords,nbdom_imax*nbdom_jmax,mpi_integer,mpi_sum,par%comm2d,ierr)
!     deallocate(rankcoords_tmp)
...
#ifdef bilants6
call my_outputs_zone1salttempflux('init',0) ; call my_outputs_zone2salttempflux('init',0) ; call my_outputs_zone3salttempflux('init',0) ; call my_outputs_zone4salttempflux('init',0) ; call my_outputs_zone5salttempflux('init',0) ; call my_outputs_zone6salttempflux('init',0) !05-07-19
#endif
...
!     stop 'cl'
      ...
      if(oasis_symsym_onoff==1) then !-oasis-symsym->
       call cpl_oasis_grids_pmx !23-07-20
       call cpl_oasis_def_var_pmx
      endif                          !-oasis-symsym->
...
! lecture notebook_visco (parametres de turbulence, frottement, etc...)
      call set_parameters_visco  !10-09-17 ! commentE le 20-04-18 car appelE dans set_parameters
                                 !17-07-19 remis ici car z0_w peut avoir besoin de connaitre h
#ifdef stokes
! call wave_reset apres set_parameters_visco pour connaitre z0s
       call wave_reset !17-07-19
#endif
...
! Initialisation necessitant de connaitre kmin (ex: upwzone0_t)
      call set_parameters_postkmin !22-05-20
...
! computes the thickness of the cell boxes
      ...
      if(nbcanal>0) call webcanals_messages !02-05-19
...
! Initialisation des fleuves à partir de fichiers de débit:
      ...
      if(ioffline==3)call offline_bin2netcdf                           !14-08-22

! Initialisattion du modèle de houle
#ifdef key_oasis_symp_ww3
#else
      if(iwve/=0)call wave_driver !04-03-13!25-11-20
#endif                                             
...
!  longitude etc...) pour initialiser la stratification) et apres
! l'initialisation de la maree pour pouvoir beneficier du tidal mixing appliquE
! sur temobc et salobc

      call quick_initial !03-11-21
      ...
      if(simple_restart_file_txt/='none')call initial_simple_restart !20-10-19

      call initial_with_obc(1) ! initialise le model_ avec les tableaux obc !03-09-09

#ifdef bilan_s_3d
! deplace ici le 21-06-22 juste apres l'initialisation de la salinite
      call my_outputs_3dflux_s('allocate',0) !12-02-22
      call my_outputs_3dflux_s('reset',0)    !12-02-22
#endif
...
! Initialisation éventuelle d'un modèle biologique ou de transport sédimentaire
      call strada(0)
      if(imodeltrc/=0.and. &
         simple_restart_biofile_txt/='none')call initial_simple_biorestart  !06-04-22
...
! initialisation des drifter 3D   
      call drifter_initial ! ici pour utiliser un "z" initialise !28-02-23
...
! Ultimes verifications et initilisations si oasis sym sym
      if(oasis_symsym_onoff==1)call cpl_oasis_reset_pmx
      ...
      implicit none
       ...
       call main_synopsis(subroutinetitle,subroutinedescription)
        ...
        if(elapsedtime_now>elapsedtime_end) then !>>>>> !17-02-15
         write(10+par%rank,*)'Restart date error:' & !18-05-19
         ...
         write(10+par%rank,*)i1,i2,i3,i4,i5,i6 
         stop 'Start (or restart) time > elapsedtime_end. See err files'
        endif                                    !>>>>>
...
!        endif                   !ooo>
      ...
      if(bulk_scheme/=bulk_core.and.       & !20-11-17
         ...
         bulk_scheme/=bulk_moon.and.       &
         bulk_scheme/=bulk_ecume)      stop &
...
!.......................................................................
   ...
   46 close(3)
      ...
      write(6,'(a,a)')'tmpdirname=',trim(tmpdirname)
      write(6,'(a,a)')'file:',trim(tmpdirname)//'check_tmp_is_empty'
...
!............................................................
      ...
      subroutine initial_simple_restart
      use module_principal
      use module_parallele
      implicit none
      include 'netcdf.inc'
      integer,dimension(:,:,:),allocatable :: restartshort  !30-01-21
      real scalarval

! Cette routine lit un fichier (offline, graphiques) sur la mEme grille
! que la grille en cours et charge directement u,v,t,s,ssh comme etat
! initial. EN pratique on charge les tableaux "obc" puis tem_t, sal_t
! etc ect seront initialises avec la subroutine initial_with_obc

      k=len(trim(simple_restart_file_txt))
      if(par%rank==0)write(6,'(a,a)')'Simple restart file:',trim(simple_restart_file_txt)

! Verification du nom du fichier
      if(simple_restart_file_txt(k-2:k)/='.nc') then
       write(6,'(a,a)')'Err 116 on file name:',trim(simple_restart_file_txt)
       stop 'Err 116: incorrect file name in initial_simple_restart'
      endif

      status=nf_open(trim(simple_restart_file_txt),nf_nowrite,ncid)
      if(status/=0) then
       write(6,'(a,a)')'Err 1168 file:',trim(simple_restart_file_txt) 
       stop 'Err 1168'
      endif

! Verification de la date (an/mois/jour seulement)  
! ALGO BASE SUR DATE DANS LE NOM DU FICHIER
! Rechercher le debut du nom (hors chemin d'acces):
      k=index(trim(simple_restart_file_txt),'/',back=.true.)
              read(simple_restart_file_txt(k+1:k+4),*)i1 !year_
              read(simple_restart_file_txt(k+5:k+6),*)i2 !month_
              read(simple_restart_file_txt(k+7:k+8),*)i3 !day
#ifdef bidon
! Verification de la date (an/mois/jour seulement)  
! ALGO BASE SUR DATE DANS LA VARIABLE TIME DU FICHIER
            status=nf_inq_varid(ncid,'time',var_id)
            if(status/=0)stop 'Err 3 nf_inq_varid time initial main'
            status=nf_get_vara_double(ncid,var_id,1,1,x1)
            if(status/=0)stop 'Err 3 nf_get_vara_double time'

            texte30=''
            status=nf_get_att_text(ncid,var_id,'units',texte30)
            if(status/=0)stop &
            'Err 3 status=nf_get_att_text units initial main'

            k=index(texte30,'since')
            read(texte30(k+6:k+9),*)  i1 !year_
            read(texte30(k+11:k+12),*)i2 !month_
            read(texte30(k+14:k+15),*)i3 !day_
            read(texte30(k+17:k+18),*)i4 !hour_
            read(texte30(k+20:k+21),*)i5 !minute_
            read(texte30(k+23:k+24),*)i6 !second_
            call datetokount(i1,i2,i3,i4,i5,i6) !donne elapsedtime_out

! En principe x1=0. mais si exceptionnellement ce n'est pas le cas alors
! on reporte ce dernier sur le temps ecoulE et on recalcule le date:
            if(x1/=0.) then !ooo>
             time1=elapsedtime_out+x1 ! ajout du temps au temps repere puis deduction de la date:
             if(time1<0.) then !debug>
              write(10+par%rank,'(a)')'Incoherence entre date depart'
              write(10+par%rank,*)datesim(1:3,1)
              write(10+par%rank,'(a)')'et date'
              write(10+par%rank,'(a,a)')'du fichier de demarrage',trim(simple_restart_file_txt)
             stop 'Err 1204 date. See fort.xxx files'
             endif             !debug>
             call elapsedtime2date(time1,i1,i2,i3,i4,i5,i6) ! temps en s, y,m,d,h,m,s
            endif           !ooo>
#endif
           
! La date est elle la meme que la date initiale dans notebook_time?
            if(i1/=datesim(1,1).or.i2/=datesim(2,1).or.i3/=datesim(3,1)) then !m°v°m>
             write(10+par%rank,'(a)')'Incoherence entre date depart'
             write(10+par%rank,*)datesim(1:3,1)
             write(10+par%rank,'(a)')'et date'
             write(10+par%rank,*)i1,i2,i3
             write(10+par%rank,'(a,a)')'du fichier de demarrage',trim(simple_restart_file_txt)
             stop 'Err 1205 date. See fort.xxx files'
            endif                                                             !m°v°m>

! Verification des dimensions du fichiers
      status=nf_inq_dimid(ncid,'ni_t',var_id)
      if(status/=0)stop 'Err 1156 get_dim ni_t'
      status=nf_inq_dimlen(ncid,var_id,i0)
      if(status/=0)stop 'Err 1156 nf_inq_dimlen i0'


      status=nf_inq_dimid(ncid,'nj_t',var_id)
      if(status/=0)stop 'Err 1156 get_dim nj_t'
      status=nf_inq_dimlen(ncid,var_id,j0)
      if(status/=0)stop 'Err 1156 nf_inq_dimlen j0'

      status=nf_inq_dimid(ncid,'nk_t',var_id)
      if(status/=0)stop 'Err 1156 get_dim nk_t'
      status=nf_inq_dimlen(ncid,var_id,k0)
      if(status/=0)stop 'Err 1156 nf_inq_dimlen k0'

      if(i0/=iglb+2.or.j0/=jglb+2.or.k0/=kmax) &
      stop 'initial_simple_restart Err dimension'

      status=nf_inq_varid(ncid,'tem',var_id)
      if(status/=0)stop 'Err 1178 nf_inq_varid tem'

! SECTION POINT 3D T
      varstart(1)=1+par%timax(1) ; varstart(2)=1+par%tjmax(1) ; varstart(3)=1    ; varstart(4)=1
      varcount(1)=imax+2         ; varcount(2)=jmax+2         ; varcount(3)=kmax ; varcount(4)=1

! Temperature
      status=nf_inq_varid(ncid,'tem',var_id)
      if(status/=0)stop 'Err 1178 nf_inq_varid tem'

      status=nf_inq_vartype(ncid,var_id,k0)
      if(status/=0)stop 'Err 1181 nf_inq_vartype'
!     if(k0/=nf_real)stop 'Err 1182 k0/=nf_real'
      if(k0/=nf_real.and.k0/=nf_short) &
      stop 'Err 1182 k0/=nf_real or nf_int'   !30-01-21

! 30-01-21
      if(k0.eq.nf_real)then
      status=nf_get_vara_real(ncid,var_id,varstart(1:4)           &
                                         ,varcount(1:4)           &
                                         ,anyvar3d(0:imax+1,0:jmax+1,1:kmax))
      if(status/=0)stop 'Err 1191 nf_get_vara_real T'
      else
       if(.not.allocated(restartshort))allocate(restartshort(0:imax+1,0:jmax+1,kmax))
       status=nf_get_vara_int(ncid,var_id,varstart(1:4)           &
                                         ,varcount(1:4)           &
                                         ,restartshort(0:imax+1,0:jmax+1,1:kmax))
       status=nf_get_att_real(ncid1,var_id,'scale_factor',scalarval)
       if(status/=0)scalarval=1.
       var_scalefactor=scalarval
                    status=nf_get_att_real(ncid1,var_id,'add_offset',scalarval)
       if(status/=0)status=nf_get_att_real(ncid1,var_id,'offset',scalarval)
       if(status/=0)scalarval=0.
       var_addoffset=scalarval
       do i=0,imax+1 ; do j=0,jmax+1 ; do k=1,kmax
        anyvar3d(i,j,k)=restartshort(i,j,k)*var_scalefactor+var_addoffset
       enddo  ; enddo ; enddo
       deallocate (restartshort)
      endif


      status=nf_get_att_real(ncid,var_id,'_FillValue',filval)
      if(status/=0)stop 'Err 1191 nf_get_att_real filval'

      do k=1,kmax ; do j=0,jmax+1 ; do i=0,imax+1
        if(anyvar3d(i,j,k)==filval) then
         temobc_t(i,j,k,:)=0.
        else
         temobc_t(i,j,k,:)=anyvar3d(i,j,k)
        endif
      enddo       ; enddo         ; enddo

! salinite
      status=nf_inq_varid(ncid,'sal',var_id)
      if(status/=0)stop 'Err 1178 nf_inq_varid sal'

      status=nf_inq_vartype(ncid,var_id,k0)
      if(status/=0)stop 'Err 1198 nf_inq_vartype'
!     if(k0/=nf_real)stop 'Err 1199 k0/=nf_real'
      if(k0/=nf_real.and.k0/=nf_short) &
      stop 'Err 1183 k0/=nf_real or nf_int'  !30-01-21
...
!30-01-21
      ...
      status=nf_get_vara_real(ncid,var_id,varstart(1:4)           &
                                         ,varcount(1:4)           &
                                         ,anyvar3d(0:imax+1,0:jmax+1,1:kmax))
      if(status/=0)stop 'Err 1191 nf_get_vara_real S'
      else
       if(.not.allocated(restartshort))allocate(restartshort(0:imax+1,0:jmax+1,kmax))
       status=nf_get_vara_int(ncid,var_id,varstart(1:4)           &
                                         ,varcount(1:4)           &
      ...
      status=nf_get_att_real(ncid,var_id,'_FillValue',filval)
      if(status/=0)stop 'Err 1204 nf_get_att_real filval'

      do k=1,kmax ; do j=0,jmax+1 ; do i=0,imax+1
        if(anyvar3d(i,j,k)==filval) then
         salobc_t(i,j,k,:)=0.
        else
         salobc_t(i,j,k,:)=anyvar3d(i,j,k)
        endif
      enddo       ; enddo         ; enddo

! SECTION POINT 2D T
      varstart(1)=1+par%timax(1) ; varstart(2)=1+par%tjmax(1) ; varstart(3)=1    ; varstart(4)=1
      varcount(1)=imax+2         ; varcount(2)=jmax+2         ; varcount(3)=1    ; varcount(4)=1

! SSH
                   status=nf_inq_varid(ncid,'ssh_w',var_id)
      if(status/=0)status=nf_inq_varid(ncid,'ssh_inst',var_id)
      if(status/=0)stop 'Err 1220 nf_inq_varid sal'

      status=nf_inq_vartype(ncid,var_id,k0)
      if(status/=0)stop 'Err 1220 nf_inq_vartype'
      if(k0/=nf_real)stop 'Err 1220 k0/=nf_real'

      status=nf_get_vara_real(ncid,var_id,varstart(1:3)           &
                                         ,varcount(1:3)           &
                                         ,anyvar2d(0:imax+1,0:jmax+1))
      if(status/=0)stop 'Err 1220 nf_get_vara_real SSH'

      status=nf_get_att_real(ncid,var_id,'_FillValue',filval)
      if(status/=0)stop 'Err 1220 nf_get_att_real filval'

      do j=0,jmax+1 ; do i=0,imax+1
        if(anyvar2d(i,j)==filval) then
         sshobc_w(i,j,:)=0.
        else
         sshobc_w(i,j,:)=anyvar2d(i,j)
        endif
      enddo         ; enddo

! SECTION POINT 3D U
      varstart(1)=1+par%timax(1) ; varstart(2)=1+par%tjmax(1) ; varstart(3)=1    ; varstart(4)=1
      varcount(1)=imax+1         ; varcount(2)=jmax+2         ; varcount(3)=kmax ; varcount(4)=1

! VITESSE U
                   status=nf_inq_varid(ncid,'u',var_id)
      if(status/=0)status=nf_inq_varid(ncid,'vel_u',var_id)
      if(status/=0)stop 'Err 1239 nf_inq_varid u'

      status=nf_inq_vartype(ncid,var_id,k0)
      if(status/=0)stop 'Err 1239 nf_inq_vartype'
!     if(k0/=nf_real)stop 'Err 1239 k0/=nf_real'

! 30-01-21
      if(k0/=nf_real.and.k0/=nf_short) &
      stop 'Err 1184 k0/=nf_real or nf_int'

      if(k0.eq.nf_real)then
      status=nf_get_vara_real(ncid,var_id,varstart(1:4)           &
                                         ,varcount(1:4)           &
                                         ,anyvar3d(1:imax+1,0:jmax+1,1:kmax))
      if(status/=0)stop 'Err 1191 nf_get_vara_real U'
      else
       if(.not.allocated(restartshort))allocate(restartshort(1:imax+1,0:jmax+1,kmax))
       status=nf_get_vara_int(ncid,var_id,varstart(1:4)           &
                                         ,varcount(1:4)           &
                                         ,restartshort(1:imax+1,0:jmax+1,1:kmax))
       status=nf_get_att_real(ncid1,var_id,'scale_factor',scalarval)
       if(status/=0)scalarval=1.
       var_scalefactor=scalarval
                    status=nf_get_att_real(ncid1,var_id,'add_offset',scalarval)
       ...
       if(status/=0)scalarval=0.
       var_addoffset=scalarval
       do i=1,imax+1 ; do j=0,jmax+1 ; do k=1,kmax
        anyvar3d(i,j,k)=restartshort(i,j,k)*var_scalefactor+var_addoffset
       enddo  ; enddo ; enddo
       deallocate (restartshort)
      endif
      ...
      status=nf_get_att_real(ncid,var_id,'_FillValue',filval)
      if(status/=0)stop 'Err 1239 nf_get_att_real filval'

      do k=1,kmax ; do j=0,jmax+1 ; do i=1,imax+1
       if(anyvar3d(i,j,k)==filval) then
        velobc_u(i,j,k,:)=0.
       else
        velobc_u(i,j,k,:)=anyvar3d(i,j,k)
       endif
      enddo       ; enddo         ; enddo

! SECTION POINT 3D V
      varstart(1)=1+par%timax(1) ; varstart(2)=1+par%tjmax(1) ; varstart(3)=1    ; varstart(4)=1
      varcount(1)=imax+2         ; varcount(2)=jmax+1         ; varcount(3)=kmax ; varcount(4)=1

! VITESSE V
                   status=nf_inq_varid(ncid,'v',var_id)
      if(status/=0)status=nf_inq_varid(ncid,'vel_v',var_id)
      if(status/=0)stop 'Err 1259 nf_inq_varid v'

      status=nf_inq_vartype(ncid,var_id,k0)
      if(status/=0)stop 'Err 1259 nf_inq_vartype'
!     if(k0/=nf_real)stop 'Err 1259 k0/=nf_real'

! 30-01-21
      if(k0/=nf_real.and.k0/=nf_short) &
      stop 'Err 1185 k0/=nf_real or nf_int'

      if(k0.eq.nf_real)then
      status=nf_get_vara_real(ncid,var_id,varstart(1:4)           &
                                         ,varcount(1:4)           &
                                         ,anyvar3d(0:imax+1,1:jmax+1,1:kmax))
      if(status/=0)stop 'Err 1191 nf_get_vara_real V'
      else
       if(.not.allocated(restartshort))allocate(restartshort(0:imax+1,1:jmax+1,kmax))
       status=nf_get_vara_int(ncid,var_id,varstart(1:4)           &
                                         ,varcount(1:4)           &
                                         ,restartshort(0:imax+1,1:jmax+1,1:kmax))
       status=nf_get_att_real(ncid1,var_id,'scale_factor',scalarval)
       if(status/=0)scalarval=1.
       var_scalefactor=scalarval
                    status=nf_get_att_real(ncid1,var_id,'add_offset',scalarval)
       ...
       if(status/=0)scalarval=0.
       var_addoffset=scalarval
       do i=0,imax+1 ; do j=1,jmax+1 ; do k=1,kmax
        anyvar3d(i,j,k)=restartshort(i,j,k)*var_scalefactor+var_addoffset
       enddo  ; enddo ; enddo
       deallocate (restartshort)
      endif
      ...
      status=nf_get_att_real(ncid,var_id,'_FillValue',filval)
      if(status/=0)stop 'Err 1259 nf_get_att_real filval'

      do k=1,kmax ; do j=1,jmax+1 ; do i=0,imax+1
       if(anyvar3d(i,j,k)==filval) then
        velobc_v(i,j,k,:)=0.
       else
        velobc_v(i,j,k,:)=anyvar3d(i,j,k)
       endif
      enddo       ; enddo         ; enddo
      ...
      end subroutine initial_simple_restart

!............................................................

      subroutine initial_simple_biorestart !06-04-22
      use module_principal
      use module_parallele
      implicit none
      include 'netcdf.inc'
...
! Cette routine lit un fichier (offline, graphiques) sur la mEme grille
! que la grille en cours et charge directement bio_t comme etat
! initial. 

      if(par%rank==0)write(6,'(a,a)') &
      'Simple restart file for passive tracers:',trim(simple_restart_biofile_txt)

! Verification du nom du fichier
      k=len(trim(simple_restart_biofile_txt))
      if(simple_restart_biofile_txt(k-2:k)/='.nc') then
       write(6,'(a,a)')'Err 226 on file name:',trim(simple_restart_biofile_txt)
       stop 'Err 14: incorrect biofile name in initial_simple_restart'
      endif

      status=nf_open(trim(simple_restart_biofile_txt),nf_nowrite,ncid)
      if(status/=0) then
       write(6,'(a,a)')'Err 15 biofile:',trim(simple_restart_biofile_txt) 
       stop 'Err 15'
      endif
...
! ALGO BASE SUR DATE DANS LE NOM DU FICHIER
! Rechercher le debut du nom (hors chemin d'acces):
      k=index(trim(simple_restart_biofile_txt),'/',back=.true.)
              read(simple_restart_biofile_txt(k+1:k+4),*)i1 !year_
              read(simple_restart_biofile_txt(k+5:k+6),*)i2 !month_
              read(simple_restart_biofile_txt(k+7:k+8),*)i3 !day
...
! La date est elle la meme que la date initiale dans notebook_time?
            if(i1/=datesim(1,1).or.i2/=datesim(2,1).or.i3/=datesim(3,1)) then !m°v°m>
             ...
             write(10+par%rank,'(a)')'et date'
             write(10+par%rank,*)i1,i2,i3
             write(10+par%rank,'(a,a)')'du fichier de demarrage',trim(simple_restart_biofile_txt)
             stop 'Err 1205 date. See fort.xxx files'
            endif                                                             !m°v°m>

! Verification des dimensions du fichiers
      status=nf_inq_dimid(ncid,'ni_t',var_id)
      if(status/=0)stop 'Err 2256 get_dim ni_t'
      status=nf_inq_dimlen(ncid,var_id,i0)
      if(status/=0)stop 'Err 2256 nf_inq_dimlen i0'


      status=nf_inq_dimid(ncid,'nj_t',var_id)
      if(status/=0)stop 'Err 2256 get_dim nj_t'
      status=nf_inq_dimlen(ncid,var_id,j0)
      if(status/=0)stop 'Err 2256 nf_inq_dimlen j0'

      status=nf_inq_dimid(ncid,'nk_t',var_id)
      if(status/=0)stop 'Err 2256 get_dim nk_t'
      status=nf_inq_dimlen(ncid,var_id,k0)
      if(status/=0)stop 'Err 2256 nf_inq_dimlen k0'
      ...
      write(texte30,'(a3,i0)')'bio',vb

      status=nf_inq_varid(ncid,trim(texte30),var_id)
      if(status/=0)stop 'Err 2278 nf_inq_varid bio'

      status=nf_inq_vartype(ncid,var_id,k0)
      if(status/=0)stop 'Err 2281 nf_inq_vartype'
!     if(k0/=nf_real)stop 'Err 2282 k0/=nf_real'
      if(k0/=nf_real.and.k0/=nf_short) &
      stop 'Err 2282 k0/=nf_real or nf_int'   !30-01-21
...
! 30-01-21
      ...
      status=nf_get_vara_real(ncid,var_id,varstart(1:4)           &
                                         ,varcount(1:4)           &
                                         ,anyvar3d(0:imax+1,0:jmax+1,1:kmax))
      if(status/=0)stop 'Err 2291 nf_get_vara_real T'
      else
       if(.not.allocated(restartshort))allocate(restartshort(0:imax+1,0:jmax+1,kmax))
       status=nf_get_vara_int(ncid,var_id,varstart(1:4)           &
                                         ,varcount(1:4)           &
      ...
      status=nf_get_att_real(ncid,var_id,'_FillValue',filval)
      if(status/=0)stop 'Err 2291 nf_get_att_real filval'

      do k=1,kmax ; do j=0,jmax+1 ; do i=0,imax+1
        if(anyvar3d(i,j,k)==filval) then
         bio_t(i,j,k,vb)=0.
        else
         bio_t(i,j,k,vb)=anyvar3d(i,j,k)
        endif
      enddo       ; enddo         ; enddo
      ...
      end subroutine initial_simple_biorestart

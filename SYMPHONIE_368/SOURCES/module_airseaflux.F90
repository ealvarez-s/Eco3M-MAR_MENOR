...
! release 363 - last update: 08-01-23
!______________________________________________________________________
...
!         16-11-18  suppression d'un warning inoperant
! v247    28-02-19  ajout omaga_evaprec_w pour bilan sur 2 pas de temps
! v250    31-03-19  modification de la lecture de la date
! v256    06-06-19  une ligne inutile est commentEe
! v272    14-01-20  Aide debug (pour par exemple aider a detecter les erreurs de decumul) 
! v273    22-01-20  Aide debug suite
! v275    11-02-20  commande 'pause' commentEe
! v286    01-07-20  decumul "auto"
! v296    18-02-21  flag_omega_cumul=1 !18-02-21
!         27-02-21  if(iairsea==0)return !27-02-21
! v309    17-09-21  lire des champs WRF
! v317    22-12-21  Meme si flag_meteo_land_plug=1 le vent n'est pas concerne par le remplacement des valeurs 
!                   continentales, pour mieux preserver la continuite de sa divergence et de son rotationnel
! v319    27-12-21  flag_meteo_land_plug_wind permet de choisir de considerer ou non le vent au dessus 
!                   du continent dans la fonction d'interpolation, independement des autres champs meteo.
! v323    26-01-22  Le bouchage des trous est mis A jour en cours de simulation si LSM (land sea mask) dans ecmwf 
!                   change en cours de simulation
! v327    18-02-22  Que wetmask n'empeche pas les precipitations....
! v349    30-06-22  - if(allocated(......)) deallocate
!         01-07-22  - Ne pas verifier le masque si on lit les listes binaires du repertoire binary
! v354    12-09-22  ajout de nouveau meteovarname
!         13-09-22  cas oU le LSM est dans un fichier de grille A part
!         14-09-22  cas oU les champs meteo ne sont pas regroupEs dans le meme fichier mais 
!                   sont contenus dans des fichiers sEparEs (cas POPNCO)
! v360    10-12-22  Cas meteo popnco scale facteur sur precipitation non cumulees
! v363    08-01-23  stop 'huuum are you sure you want to use airseaflux_fbk?' !08-01-23
!...............................................................................
...
! On doit passer par cette subroutine meme si iairsea=0 car il faut de 
! toute facon faire le reset de omega=0
...
!     write(6,*)'rank, variable identifiant et flag_cumul:',par%rank,loop_,flag_cumul
...
!     write(10+par%rank,'(a,a)')'check airsea file prv time:',trim(texte80(2)) 
!     write(10+par%rank,'(a,a)')'check airsea file nex time:',trim(texte80(1))
...
! Bouchage des trous (points au dessus du continent) sauf pour le vent:
...
! Verifier que la liste binaire n°1 n'existe pas dEjA:
...
! l'aiguillage 4505 qui lui permet de se positionner dans la liste binaire existante.
...
!         if(loop_var_==1)write(66,'(a)')trim(texte80(1))
!         if(loop_var_==1)write(66,*)    k10
!         if(loop_var_==1)write(66,*)time_,time_/86400.
...
!.............................................................
      ...
      namelist/notebook_airseaflux/iairsea,flag_meteodata,airseaoption &!30-07-14
                                  ...
                                  ,flag_meteo_land_plug_wind           &!27-12-21
...
! Bilan long wave net ou descendant ?

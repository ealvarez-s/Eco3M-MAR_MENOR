...
! release 367 - last update: 26-02-23
!...............................................................................
...
! v315    10-12-21  ajout vitesse verticale
! v325    12-02-22  ajout my_outputs_3dflux_s
! v348    18-06-22  ajout des flux quickest partie advective pour u et v
! v361    25-12-22  elapsedtime_last_writing sert dans le cas oU la simulation est repartie d'un fichier restart, 
!                   pour ne pas ecrire tant qu'on a pas dEpassE le temps de la derniere ecriture dans le fichier
! v367    26-02-23  depth_t et depth_w ecrits avec dimension temps
!...............................................................................
!    _________                    .__                  .__             !(°o°) 
...
!        call my_outputs_point_vs_time('vanuc_station3','ijglob',15d0,461d0) !  attention A restpecter le format d0
!        call my_outputs_point_vs_time('vanuc_station2','ijglob',35d0,453d0) !  attention A restpecter le format d0
!        call my_outputs_point_vs_time('vanuc_station1','ijglob',62d0,454d0) !  attention A restpecter le format d0
...
!.............................................................
!#ifdef bidon
! Version avec dimensions k,temps (recommandee)
...
! elapsedtime_last_writing sert dans le cas oU la simulation est repartie d'un fichier restart, 
! pour ne pas ecrire tant qu'on a pas dEpassE le temps de la derniere ecriture dans le fichier: !25-12-22
      if(elapsedtime_now<elapsedtime_last_writing)return !25-12-22
      elapsedtime_last_writing=elapsedtime_now           !25-12-22
...
!     if(status/=0)stop 'erreur nf_def_dim nj_t'
      ...
      vardim(1)=k_t_dim ; vardim(2)=time_dim
...
! Time en secondes (egalement variable dimension)
      ...
      status=nf_def_var(ncid_,'time',nf_double,1,vardim(2),var_id)
...
! Time en jours
      ...
      status=nf_def_var(ncid_,'time_days',nf_double,1,vardim(2),var_id)
...
! Time en annee decimale:
      status=nf_def_var(ncid_,'time_years',nf_double,1,vardim(2),var_id)
      if(status/=0)stop ' erreur nf_def_var time_years'
      status=nf_put_att_text(ncid_,var_id,'long_name',13 &
      ,'Decimal_years')

! Time date yyyymmddhhmmss:
      status=nf_def_var(ncid_,'time_calendar',nf_double,1,vardim(2),var_id)
...
!     status=nf_put_att_text(ncid_,var_id,'units',1,'m')
      ...
      vardim(1)=k_t_dim 
!     status=nf_def_var(ncid_,'depth_t',nf_real,1,vardim(1),var_id)
      status=nf_def_var(ncid_,'depth_t',nf_real,2,vardim(1:2),var_id) !26-02-23
      ...
      vardim(1)=k_w_dim 
!     status=nf_def_var(ncid_,'depth_w',nf_real,1,vardim(1),var_id)
      status=nf_def_var(ncid_,'depth_w',nf_real,2,vardim(1:2),var_id) !26-02-23
      if(status/=0)stop ' erreur nf_def_var depth_w'
      status=nf_put_att_text(ncid_,var_id,'long_name',14 &
      ,'levels_depth_w')
      status=nf_put_att_text(ncid_,var_id,'units',1,'m')

!     write(6,*)'vardim(1:2)',vardim(1:2)
      vardim(1)=k_t_dim 
      status=nf_def_var(ncid_,'tem_t',nf_real,2,vardim(1:2),var_id)
      ...
      vardim(1)=k_t_dim 
      status=nf_def_var(ncid_,'temobc_t',nf_real,2,vardim(1:2),var_id)
      ...
      if(allocated(temlwf_t)) then !>>>>>
       vardim(1)=k_t_dim 
       status=nf_def_var(ncid_,'temlwf_t',nf_real,2,vardim(1:2),var_id)
      ...
      vardim(1)=k_t_dim 
      status=nf_def_var(ncid_,'sal_t',nf_real,2,vardim(1:2),var_id)
      ...
      vardim(1)=k_t_dim 
      status=nf_def_var(ncid_,'salobc_t',nf_real,2,vardim(1:2),var_id)
      if(status/=0)stop ' erreur nf_def_var tem_t'
      ...
      if(allocated(sallwf_t)) then !>>>>>
       vardim(1)=k_t_dim 
       status=nf_def_var(ncid_,'sallwf_t',nf_real,2,vardim(1:2),var_id)
      ...
      vardim(1)=k_t_dim !15-02-16
      status=nf_def_var(ncid_,'rhp_t',nf_real,2,vardim(1:2),var_id)
...
!      vardim(1)=k_t_dim 
!      status=nf_def_var(ncid_,'tracer1',nf_real,2,vardim(1:2),var_id)
...
!     endif                        !>>>>>
      ...
      if(allocated(bio_t)) then ! bio_t !05-09-19
        ...
        do vb=1,vbmax
          ...
          status=nf_def_var(ncid_,texte80(1),nf_real,2,vardim(1:2),var_id)
...
!     vardim(1)=k_t_dim 
!     status=nf_def_var(ncid_,'INITRATE',nf_real,2,vardim(1:2),var_id)
...
!     endif                 !bbbbbbbbbbbbbb>
      ...
      vardim(1)=k_t_dim 
      status=nf_def_var(ncid_,'Veastward',nf_real,2,vardim(1:2),var_id)
      ...
      vardim(1)=k_t_dim 
      status=nf_def_var(ncid_,'Vnorthward',nf_real,2,vardim(1:2),var_id)
      ...
      vardim(1)=k_t_dim 
      status=nf_def_var(ncid_,'vel_u',nf_real,2,vardim(1:2),var_id)
      ...
      vardim(1)=k_t_dim 
      status=nf_def_var(ncid_,'vel_v',nf_real,2,vardim(1:2),var_id)
      ...
      vardim(1)=k_t_dim 
      status=nf_def_var(ncid_,'V_OGCM_eastward',nf_real,2,vardim(1:2),var_id)
      ...
      vardim(1)=k_t_dim 
      status=nf_def_var(ncid_,'V_OGCM_northward',nf_real,2,vardim(1:2),var_id)
      ...
      vardim(1)=k_t_dim 
      status=nf_def_var(ncid_,'W',nf_real,2,vardim(1:2),var_id) !10-12-21
      if(status/=0)stop ' erreur nf_def_var W'
      status=nf_put_att_text(ncid_,var_id,'long_name',17 &
       ,'vertical_velocity')
      status=nf_put_att_text(ncid_,var_id,'units',3,'m/s')
      status=nf_put_att_real(ncid_,var_id,'_FillValue',nf_real,1,filvalr4_)
      ...
      vardim(1)=k_w_dim 
      status=nf_def_var(ncid_,'N2',nf_real,2,vardim(1:2),var_id)
...
! https://docs.google.com/document/d/1BKPwtP5EYh7q_iz5xHUPfai06GCnGtBKuqdl07rakig/edit
      ...
      vardim(1)=k_w_dim 
      status=nf_def_var(ncid_,'km_w',nf_real,2,vardim(1:2),var_id)
      ...
      vardim(1)=k_w_dim 
      status=nf_def_var(ncid_,'kh_w',nf_real,2,vardim(1:2),var_id)
      ...
      vardim(1)=k_w_dim 
      status=nf_def_var(ncid_,'tken_w',nf_real,2,vardim(1:2),var_id)
...
!ctke2*sqrt(tken_w(i,j,k))/(tkle_w(i,j,k)+small2))
      vardim(1)=k_w_dim 
      status=nf_def_var(ncid_,'epsn_w',nf_real,2,vardim(1:2),var_id)
...
! omega_w
      vardim(1)=k_w_dim 
      status=nf_def_var(ncid_,'omega_w',nf_real,2,vardim(1:2),var_id) !20-05-18
      ...
      vardim(1)=time_dim
      status=nf_def_var(ncid_,'Weastward',nf_real,1,vardim(1),var_id)
      ...
      vardim(1)=time_dim
      status=nf_def_var(ncid_,'Wnorthward',nf_real,1,vardim(1),var_id)
      ...
      status=nf_def_var(ncid_,'teta2_t',nf_double,1,vardim(1),var_id)
      status=nf_put_att_text(ncid_,var_id,'long_name',31 &
        ,'potential_air_temperature_at_2m')
      status=nf_put_att_text(ncid_,var_id,'units',1,'C')

      status=nf_def_var(ncid_,'teta0_t',nf_double,1,vardim(1),var_id)
      status=nf_put_att_text(ncid_,var_id,'long_name',41 &
       ,'temperature_at_0m_in_meteorological_model')
      status=nf_put_att_text(ncid_,var_id,'units',1,'C')

      status=nf_def_var(ncid_,'teta2delta_t',nf_double,1,vardim(1),var_id) !30-11-15
      ...
      status=nf_def_var(ncid_,'q2_t',nf_double,1,vardim(1),var_id)
      status=nf_put_att_text(ncid_,var_id,'long_name',23 &
        ,'specific_humidity_at_2m')

      status=nf_def_var(ncid_,'q2delta_t',nf_double,1,vardim(1),var_id)
      status=nf_put_att_text(ncid_,var_id,'long_name',31 &
        ,'specific_humidity_anomaly_at_2m')

      if(flag_abl2==1) then !222222222>

      status=nf_def_var(ncid_,'uwind_t',nf_double,1,vardim(1),var_id)
      status=nf_put_att_text(ncid_,var_id,'long_name',9 &
        ,'10m_uwind')
      status=nf_put_att_text(ncid_,var_id,'units',3,'m/s')

      status=nf_def_var(ncid_,'uwinddelta_t',nf_double,1,vardim(1),var_id)
      status=nf_put_att_text(ncid_,var_id,'long_name',18 &
        ,'10m_uwind_anomalie')
      status=nf_put_att_text(ncid_,var_id,'units',3,'m/s')

      status=nf_def_var(ncid_,'vwind_t',nf_double,1,vardim(1),var_id)
      status=nf_put_att_text(ncid_,var_id,'long_name',9 &
        ,'10m_vwind')
      status=nf_put_att_text(ncid_,var_id,'units',3,'m/s')

      status=nf_def_var(ncid_,'vwinddelta_t',nf_double,1,vardim(1),var_id)
      status=nf_put_att_text(ncid_,var_id,'long_name',18 &
        ,'10m_vwind_anomalie')
      status=nf_put_att_text(ncid_,var_id,'units',3,'m/s')
      ...
      vardim(1)=time_dim
      status=nf_def_var(ncid_,'wstress_u',nf_real,1,vardim(1),var_id)
      ...
      vardim(1)=time_dim
      status=nf_def_var(ncid_,'wstress_v',nf_real,1,vardim(1),var_id)
      ...
      vardim(1)=time_dim
      status=nf_def_var(ncid_,'Oi2Dflux',nf_real,1,vardim(1),var_id)
      ...
      vardim(1)=time_dim
      status=nf_def_var(ncid_,'Oj2Dflux',nf_real,1,vardim(1),var_id)
      ...
      vardim(1)=time_dim
      status=nf_def_var(ncid_,'ssr',nf_real,1,vardim(1),var_id)
      ...
      vardim(1)=time_dim
      status=nf_def_var(ncid_,'snsf',nf_real,1,vardim(1),var_id)
      ...
      vardim(1)=time_dim
      status=nf_def_var(ncid_,'slhf',nf_real,1,vardim(1),var_id)
      ...
      vardim(1)=time_dim
      status=nf_def_var(ncid_,'sshf',nf_real,1,vardim(1),var_id)
      ...
      vardim(1)=time_dim
      status=nf_def_var(ncid_,'precipitation',nf_real,1,vardim(1),var_id)
      ...
      vardim(1)=time_dim
      status=nf_def_var(ncid_,'ssh',nf_real,1,vardim(1),var_id)
...
! Ecrire longitude_t
!    !varstart(1:2)=1  ; varcount(1:2)=1
!     do j=j_,j_ ; do i=i_,i_ 
...
!                                  ,varstart(1:2) &
!                                  ,varcount(1:2) &
!                      ,anyvar2d(i_:i_,j_:j_))
!     if(status/=0)stop 'erreur nf_put_var_double h_w'

! Ecrire depth_t
!     varstart(1)=1    ; varcount(1)=kmax
!     do k=1,kmax ; do j=j_,j_ ; do i=i_,i_ 
...
!                                  ,varstart(1) &
!                                  ,varcount(1) &
!               ,anyvar3d(i_:i_,j_:j_,1:kmax))
!     if(status/=0)stop 'erreur nf_put_var_double depth_t'

! Ecrire depth_w
!     varstart(1)=1    ; varcount(1)=kmax+1
!     do k=1,kmax+1 ; do j=j_,j_ ; do i=i_,i_ 
...
! Ecrire le temps en secondes (variable dimension):
      varstart(2)=time_counter_  ; varcount(2)=1
...
!...................
! Ecrire la vitesse verticale !10-12-21
      varstart(2)=time_counter_  ; varcount(2)=1
      varstart(1)=1              ; varcount(1)=kmax
     !varstart(1:2)=1            ; varcount(1:2)=1
      call omega_vertical_velocity(2,i_,i_,j_,j_)
      status=nf_inq_varid(ncid_,'W',var_id)
      if(status/=0)stop ' erreur nf_inq_varid W'
      status=nf_put_vara_real(ncid_,var_id                &
                                   ,varstart(1:2)         &
                                   ,varcount(1:2)         &
                ,anyvar3d(i_:i_,j_:j_,1:kmax))
      if(status/=0)stop 'Erreur nf_put_vara_real W'
...
!epsilon dans le schema de Gaspar=ctke2*(tken_w(i,j,k)**1.5)/(tkle_w(i,j,k)+small2)
       ...
       enddo ; enddo ; enddo
        ...
        do k=1,kmax+1 ; do j=j_,j_ ; do i=i_,i_  !23-08-16
...
! Ecrire le vent WE, SN
      varstart(1)=time_counter_  ; varcount(1)=1
     !varstart(1:2)=1            ; varcount(1:2)=1
      do j=j_,j_ ; do i=i_,i_ 
...
! Ecrire la temperature potentielle de l'air a 2m:
      ...
      status=nf_put_vara_double(ncid_,var_id      &
                                   ,varstart(1) &
                                   ,varcount(1) &
                   ,xy_t(i_:i_,j_:j_,2))    !05-04-16
!               ,teta2_t(i_:i_,j_:j_,now))
...
!#endif
#ifdef bidon
! Version avec dimensions i,j,k,temps
      subroutine my_outputs_point_vs_time(name_,locationconv_,posx_,posy_)
      implicit none
      character(len=*) name_,locationconv_
      double precision :: lon_,lat_,posx_,posy_ &
...
!     endif                 !bbbbbbbbbbbbbb>
      ...
      vardim(3)=k_t_dim 
      status=nf_def_var(ncid_,'W',nf_real,4,vardim(1:4),var_id) !10-12-21
      if(status/=0)stop ' erreur nf_def_var W'
      status=nf_put_att_text(ncid_,var_id,'long_name',17 &
       ,'vertical_velocity')
      status=nf_put_att_text(ncid_,var_id,'units',3,'m/s')
...
! Ecrire la vitesse verticale !10-12-21
      ...
      varstart(3)=1              ; varcount(3)=kmax
      varstart(1:2)=1            ; varcount(1:2)=1+ii_
      call omega_vertical_velocity(2,i_,i_+ii_,j_,j_+ii_)
      status=nf_inq_varid(ncid_,'W',var_id)
      if(status/=0)stop ' erreur nf_inq_varid W'
      status=nf_put_vara_real(ncid_,var_id                &
                                   ,varstart(1:4)         &
...
!      zone1_stretch_dz=1.  ! epaisseur constante = 1/zone1_inv_dz
       zone1_stretch_dz=0.414 ! epaisseur augmentant avec la profondeur (en surface 1/zone1_inv_dz)
...
#ifdef bilan_s_3d
! Version de base ne distinguant pas le signe des flux
      subroutine my_outputs_3dflux_s(txt_,id_flux_) !12-02-22
      implicit none
      character(len=*)txt_
      integer id_flux_,id_wi_

! flus    = flux horizontal u       (quickest)
! fadvus  = flux horizontal u       (quickest partie advective)
! flvs    = flux horizontal v       (quickest)
! fadvvs  = flux horizontal v       (quickest partie advective)
! Note: flus-fadvus est la partie diffusive du quickest (composante u)
!       flvs-fadvvs est la partie diffusive du quickest (composante v)
! flws    = flux vertical explicite (quickest)
! fimplis = flux vertical implicite (turbulence + quickest)
! difs    = flux vertical implicite turbulence
! flwis   = flux vertical implicite quickest = fimplis - difs

! Note: les flux quickest sont A gauche du signe Egal:
! d(dxdydzS)/dt+div(flux quickest) = div(flux turbulence)

! Exemple d'application avec le lien suivant:
! https://docs.google.com/presentation/d/15P8Y272d_gg_GXFA5L6aJbwAKXfpTEFB-aj1NU4ToCI/edit?usp=sharing

      if(txt_=='allocate') then !-initial-> 
       allocate(flus    (1:imax+1,1:jmax  ,1:kmax  )) ; flus=0.
       allocate(fadvus  (1:imax+1,1:jmax  ,1:kmax  )) ; fadvus=0.
       allocate(flvs    (1:imax  ,0:jmax+1,1:kmax  )) ; flvs=0.
       allocate(fadvvs  (1:imax  ,0:jmax+1,1:kmax  )) ; fadvvs=0.
       allocate(flws    (1:imax  ,1:jmax  ,1:kmax+1)) ; flws=0.
       allocate(flwis   (1:imax  ,1:jmax  ,1:kmax+1)) ; flwis=0.
       allocate(difs    (1:imax  ,1:jmax  ,1:kmax+1)) ; difs=0.
       allocate(fimplis (1:imax  ,1:jmax  ,1:kmax+1)) ; fimplis=0.
       allocate(assels  (1:imax  ,1:jmax  ,1:kmax  )) ; assels=0.
       allocate(dxdydzs0(1:imax  ,1:jmax  ,1:kmax  )) ; dxdydzs0=0.
      endif                     !-initial->

      if(txt_=='reset') then !-reset-> 
       flus=0. ; flvs=0. ; flws=0. ; flwis=0. ; difs=0. ; fimplis=0. ; assels=0.
       fadvus=0. ; fadvvs=0. !18-06-22
       do k=1,kmax ; do j=1,jmax ; do i=1,imax
        dxdydzs0(i,j,k)=dz_t(i,j,k,0)*dxdy_t(i,j)*sal_t(i,j,k,0) 
       enddo       ; enddo       ; enddo
      endif                  !-reset-> 

      if(mod(iteration3d,2)/=0)RETURN  !FORCEMENT UN NOMBRE PAIR A CAUSE DU LEAPFROG

      if(txt_=='is') then   !-flux_Oi-> 
       do k=1,kmax ; do j=1,jmax ; do i=1,imax+1

              flus(i,j,k)= &
              flus(i,j,k)  &
           +anyv3d(i,j,k,id_flux_)*dti_lpsub  

            fadvus(i,j,k)= &
            fadvus(i,j,k)  &
        -veldydz_u(i,j,k,1)*0.5*(sal_t(i,j,k,now)+sal_t(i-1,j,k,now))*dti_lpsub !18-06-22

       enddo       ; enddo       ; enddo
      endif                 !-flux_Oi-> 

      if(txt_=='js') then   !-flux_Oj-> 
       do k=1,kmax ; do j=1,jmax+1 ; do i=1,imax
           ...
           +anyv3d(i,j,k,id_flux_)*dti_lpsub  
            ...
            fadvvs(i,j,k)= &
            fadvvs(i,j,k)  &
        -veldxdz_v(i,j,k,1)*0.5*(sal_t(i,j,k,now)+sal_t(i,j-1,k,now))*dti_lpsub

       enddo       ; enddo       ; enddo
      endif                 !-flux_Oj-> 

      if(txt_=='wes') then  !-flux_Ok-> 
! flux vertical partie explicite 
       do k=1,kmax+1
         flws(i,j,k)=flws(i,j,k)+(anyv1d(k,4)+anyv1d(k,2))*dti_lpsub*dxdy_t(i,j) 
       enddo
      endif                 !-flux_Ok-> 

      if(txt_=='implicite') then  !-asselin-KH-fluxvert> 
! Asselin:
       do k=1,kmax ; do j=1,jmax ; do i=1,imax
        assels(i,j,k)= &
        assels(i,j,k)+dxdy_t(i,j)*( & !ooo>
        dz_t(i,j,k,before)*tfilterfb*( sal_t(i,j,k,before2)         & 
                                      -sal_t(i,j,k,before ))        &

        +dz_t(i,j,k,now  )*tfilterlf*( sal_t(i,j,k,before2)         &
                                      -sal_t(i,j,k,before ))        &

        +dz_t(i,j,k,after)*(            tfilterfb *sal_t(i,j,k,after )  &
                            +(tfilterlf-tfilterfb)*sal_t(i,j,k,now   )  &
...
! Turbulence:
       do k=2,kmax ; do j=1,jmax ; do i=1,imax
        difs(i,j,k)= &
        difs(i,j,k)+kh_w(i,j,k)*(sal_t(i,j,k,2)-sal_t(i,j,k-1,2))/(depth_t(i,j,k)-depth_t(i,j,k-1)) &
                               *dti_lp*dxdy_t(i,j)
       enddo       ; enddo       ; enddo

! Comme le tableau anyv3d(i,j,k,id_wi_) a ete effacE A ce stade du calcul le seul moyen de revenir 
! A l'advection implicite c'est de considerer l'ensemble des contributions implicites contenues 
! dans tridia_in(i,j,k,1:3) puis de soutraire la contribution du melange et d'eventuels autres 
! contributeurs (filtre temporel etc...)
       do j=1,jmax ; do i=1,imax
        sum1=0.
        do k=1,kmax
         sum1= &
         sum1  &
         -dxdy_t(i,j)*(sal_t(i,j,k-1,2)* tridia_in(i,j,k,1)                                  &
                      +sal_t(i,j,k  ,2)*(tridia_in(i,j,k,2)-(1-tfilterfb)*dz_t(i,j,k,after)) &
                      +sal_t(i,j,k+1,2)* tridia_in(i,j,k,3))
 
         fimplis(i,j,k+1)=     &
         fimplis(i,j,k+1)+     & ! la somme pour cumuler sur plusieurs iterations...
                          sum1  
        enddo
       enddo       ; enddo

       do k=1,kmax+1 ; do j=1,jmax ; do i=1,imax
        flwis(i,j,k)=fimplis(i,j,k)-difs(i,j,k)
       enddo         ; enddo       ; enddo

      endif                       !-asselin-KH-fluxvert> 

      if(txt_=='check') then      !-ecrire-fichier-> 
       i=338-par%timax(1)
       j=248-par%tjmax(1)
       k=kmax ! -3
       if(i>1.and.i<imax.and.j>1.and.j<jmax) then
! La colonne 2 doit etre la meme que la somme des colonnes 3+4+5+6+7
        open(unit=3,file=trim(tmpdirname)//'bilan_3d_s_check' &
                   ,position='append')
         write(3,'(13(1x,e14.7))')                                 &
          elapsedtime_now/86400.                                   & !1
! Cette colonne:
         ,dz_t(i,j,k,2)*dxdy_t(i,j)*sal_t(i,j,k,2)-dxdydzs0(i,j,k) & !2   
! Doit etre egale A la somme des colonnes 3,4,5,6,7
         ,flus(i+1,j,k)-flus(i,j,k)        & !3 divergence x QUICKEST
         ,flvs(i,j+1,k)-flvs(i,j,k)        & !4 divergence y QUICKEST
         ,flws(i,j,k+1)-flws(i,j,k)        & !5 divergence z explicite (flux vertical QUICKEST)
         ,fimplis(i,j,k+1)-fimplis(i,j,k)  & !6 divergence z implicite (flux vertical TKE + QUICKEST)
         ,assels(i,j,k)                    & !7 filtre temporel (lui meme une divergence t)
! Ensuite la dissociation de fimplis en difs (melange vertical TKE) + flwis (flux vertical QUICKEST implicit)
         ,difs(i,j,k+1)-difs(i,j,k)        & !8 divergence z melange TKE
         ,flwis(i,j,k+1)-flwis(i,j,k)      & !9 divergence z QUICKEST implicite
! Ensuite la contribution purement advective, fadvus, au terme flus,
! et la partie diffusive deduite par complementarite, soit flus-fadvus
         ,fadvus(i+1,j,k)-fadvus(i,j,k)    & !10 divergence x QUICKEST partie advective
         , (  flus(i+1,j,k)  -flus(i,j,k)) & !11
          -(fadvus(i+1,j,k)-fadvus(i,j,k)) & !11 divergence x QUICKEST partie diffusive
! Idem mais pour la composante y:
         ,fadvvs(i,j+1,k)-fadvvs(i,j,k)    & !12 divergence y QUICKEST partie advective
         , (  flvs(i,j+1,k)  -flvs(i,j,k)) & !13 
          -(fadvvs(i,j+1,k)-fadvvs(i,j,k))   !13 divergence y QUICKEST partie diffusive
! Note: et bien sur pas de flux air/mer car zero sel dans l'air...
        close(3)
       endif
      endif                       !-ecrire-fichier-> 

      end subroutine my_outputs_3dflux_s
#endif

...
! release 260 - last update: 02-10-19
!______________________________________________________________________
...
! v251     09-04-19  modifs pour resurgences
! v260     02-10-19  possibilitE de cumuler plusieurs fleuves (plusieurs debits) sur
!                    un mEme point d'entrEe
!........ .......................................................................
...
! Cas onde incidente i croissant
      if(iteration2d==iteration2d_begin)call q_poisson_ondes_diag ! donne u_amplitude_,periode etc...
      stop 'bibi'
      period_=3600.
      ssh_amplitude_=0.01
      x1=sin(2.*pi*iteration2d*dte_fw/period_)
!     if(iteration2d>20000)x1=0.
!     if(par%rank==0) then
!     if( int((iteration2d+1)*dte_fw/period_) &
!        -int( iteration2d   *dte_fw/period_)==1) then
!      write(6,*)'x1 cycle',x1,int(   (iteration2d+1)*dte_fw/period_)
!     endif
!     endif

! RAMPE:

      x0=min(1.,iteration2d/1000.)
      do j=2,jmax-1

! CAS AVEC UN ANGLE
!     x1=x0*sin(2.*pi*iteration2d*dte_fw/period_-2.*pi*real(j+par%tjmax(1))/real(jglb-2))

!      vbrrefobc_j(j,1)=0.01*x1
!      sshrefobc_j(j,1)=sqr_hoverg_u(j,1)*vbrrefobc_j(j,1)
       sshrefobc_j(j,1)=ssh_amplitude_*x1
       vbrrefobc_j(j,1)=sshrefobc_j(j,1)/sqr_hoverg_u(j,1)
      enddo
...
! sur un autre sous-domaine:
      ...
      if(nriver.ge.1) then !------------------------>
       do kr=1,nriver
       if(riverdir(kr)>0)sum2=sum2-abs(riverflux(kr,1))             !25/11/01 !09-04-19
       enddo
      endif                !------------------------>
...
! dans ce cas la "memoire" de lagrange_zta a un sens...
      ...
      if(nriver.ge.1) then !------------------------>
       do kr=1,nriver
       if(riverdir(kr)>0)b2delev=b2delev-abs(riverflux(kr,1))       !25/11/01 !09-04-19
       enddo
      endif                !------------------------>
...
! Onde incidente:
      x1=sin(2.*pi*iteration3d*dti_fw/period_) &
        *min(1.,iteration3d/1000.) ! RAMPE
      do j=1,jmax 
       sshrefobc_j(j,2)=ssh_amplitude_*x1
       vbrrefobc_j(j,2)=  &
!     -sshrefobc_j(j,2)*c_/(hz_u(imax,j,1)*dy_u(imax,j))
      -sshrefobc_j(j,2)*c_/(hz_u(imax,j,1)             ) ! faute corrigee le 18/01/2018
      enddo
#endif
      do j=1,jmax !2,jmax-1
                 ssh_int_w(imax,j,2)=                              &

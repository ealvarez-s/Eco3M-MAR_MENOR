...
!______________________________________________________________________________
      ...
      subroutine pour_nest_2d(ichoix)                                  !02/04/07
      use module_principal
      implicit none
      integer ichoix
#ifdef synopsis
       subroutinetitle='pour_nest_2d'
       subroutinedescription=' '
       call main_synopsis(subroutinetitle,subroutinedescription)
#endif

! Routine requise pour que nest_inout.F fonctionne correctement.
! Pour cela il faut fournir zta_int_z(i,j,1) (zta au temps t)
! et vlxavr_x(i,j,1) et vlyavr_y(i,j,1) le courant barotrope
! moyenné sur la periode t-1 t+1.

      if(ichoix.eq.2) then !22222222>

      const2=0.5/real(iteration2d_max_now)        !19-04-11
      do j=1,jmax+1
      do i=1,imax+1
      ...
      if(ichoix.eq.1) then !11111111>
       do j=1,jmax+1
       do i=1,imax+1
        ssh_int_u(i,j,1)=ssh_int_u(i,j,2)
        ssh_int_v(i,j,1)=ssh_int_v(i,j,2)
      ...
      if(ichoix.eq.0) then !00000000>
       ...
       enddo
       enddo
       if(extmodtype.ne.'single') then !debug>
        write(6,*)'model_ 2d doit etre single'
        stop ' model_2d'
       endif                           !debug>
      return
      endif                !00000000>

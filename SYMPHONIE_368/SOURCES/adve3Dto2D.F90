...
!______________________________________________________________________
      ...
      implicit none
#ifdef synopsis
       subroutinetitle='adve3dto2d'
       subroutinedescription= &
       'Depth-averaged residual of the non-linear 3D momentum terms' &
       //' used as frozen forcing term in the barotropic momentum ' &
       //'equations'
...
! pas ete calculEs, on ne calcule donc pas l'advection
      if(iteration3d==0) then !03-09-18
       adve3d2d_u=0. ; adve3d2d_v=0 ; RETURN !03-09-18
      endif

      call momentum_exp_adv_u_v321 !16-01-22
      call momentum_exp_adv_v_v321 
      ...
      do k=1,kmax ; do j=2,jmax-1 ; do i=2,imax
       adve3d2d_u(i,j)= &
       adve3d2d_u(i,j)+ &
            +dz_u(i,j,k,now)*(  & ! choix de "now" coherent avec frozenterm multiplieS par hz_u(:,:,1)
            vel_u(i,j,k,0)      & ! signe coherent avec frozenterm
           -vel_u(i,j,k,2)   ) 
      enddo         ; enddo       ; enddo
      do j=2,jmax-1 ; do i=2,imax
       adve3d2d_u(i,j)= &
       adve3d2d_u(i,j)*inv_dti_lp
      enddo         ; enddo
      ...
      do k=1,kmax ; do j=2,jmax ; do i=2,imax-1
       adve3d2d_v(i,j)= &
       adve3d2d_v(i,j)+ &
            +dz_v(i,j,k,now)*(  & ! choix de "now" coherent avec frozenterm multiplieS par hz_u(:,:,1)
            vel_v(i,j,k,0)      & ! signe coherent avec frozenterm
           -vel_v(i,j,k,2)   ) 
      enddo         ; enddo       ; enddo
      do j=2,jmax ; do i=2,imax-1
       adve3d2d_v(i,j)= &
       adve3d2d_v(i,j)*inv_dti_lp
      enddo       ; enddo

! NOTE: Le tableau adve3d2d est possiblement complEtE par l'advection barotrope de reference
! dans module_external

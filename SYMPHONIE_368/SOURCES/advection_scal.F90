...
!     call obc_depth(2)
      ...
      i1=i-( nint(sign(un,veldydz_u(i,j,k,id_veltot))) + 1 )/2
...
!     x5=max(1.-abs(veldydz_u(i,j,k,id_veltot)*dti_lp              &
!                     /dxdy_u(i,j)                         &
...
! Flux T:
!#ifdef bidon
      anyv3d(i,j,k,1)=-veldydz_u(i,j,k,id_veltot)*const1*(                     &
...
! Flux S:
      anyv3d(i,j,k,4)=-veldydz_u(i,j,k,id_veltot)*const1*(                     &
...
!#endif
...
#ifdef bidon
      anyv3d(i,j,k,1)=-veldydz_u(i,j,k,id_veltot)*dti_lp    &
      *( dz_t(i  ,j,k,1)*(tem_t(i  ,j,k,1))         &
        +dz_t(i-1,j,k,1)*(tem_t(i-1,j,k,1)))        &
      /( dz_t(i  ,j,k,1)                            &
        +dz_t(i-1,j,k,1)                 )            

      anyv3d(i,j,k,4)=-veldydz_u(i,j,k,id_veltot)*dti_lp    &
      *( dz_t(i  ,j,k,1)*(sal_t(i  ,j,k,1))         &
      ...
      j1=j-( nint(sign(un,veldxdz_v(i,j,k,id_veltot))) + 1 )/2
...
! Attention prendre veldydz plutot que vel_u a cause entre autres de stokes
!     x5=max(1.-abs(veldxdz_v(i,j,k,id_veltot)*dti_lp      &
!                     /dxdy_v(i,j)                 &
...
!#ifdef bidon
! Flux T:
      anyv3d(i,j,k,2)=-veldxdz_v(i,j,k,id_veltot)*const1*(                     &
...
! Flux S:
      anyv3d(i,j,k,5)=-veldxdz_v(i,j,k,id_veltot)*const1*(                     &
...
#ifdef bidon
      anyv3d(i,j,k,2)=-veldxdz_v(i,j,k,id_veltot)*dti_lp    &
      *( dz_t(i,j  ,k,1)*(tem_t(i,j  ,k,1))         &
        +dz_t(i,j-1,k,1)*(tem_t(i,j-1,k,1)))        &
      /( dz_t(i,j  ,k,1)                            &
        +dz_t(i,j-1,k,1)                 )            

      anyv3d(i,j,k,5)=-veldxdz_v(i,j,k,id_veltot)*dti_lp    &
      *( dz_t(i,j  ,k,1)*(sal_t(i,j  ,k,1))         &
      ...
      if(flag_refstate==0) return !11-11-15
       ...
       do i=1,imax+1
         ...
         anyv3d(i,j,k,1)=anyv3d(i,j,k,1)           &
                     -veldydz_u(i,j,k,id_veltot)*dti_lp    &
                      *temref_u(i,j,k)
       ...
       do i=1,imax
         ...
         anyv3d(i,j,k,2)=anyv3d(i,j,k,2)         &
                     -veldxdz_v(i,j,k,id_veltot)*dti_lp  &
                      *temref_v(i,j,k)
...
#ifdef bilan_s_3d
      if(mod(iteration3d,2)==0) & !FORCEMENT UN NOMBRE PAIR A CAUSE DU LEAPFROG
...
#ifdef bidon
                if(max(max(max(abs(veldydz_u(i  ,j  ,k,id_veltot))  &
                              ,abs(veldydz_u(i+1,j  ,k,id_veltot))) &
                              ,abs(veldxdz_v(i  ,j  ,k,id_veltot))) &
                              ,abs(veldxdz_v(i  ,j+1,k,id_veltot))) & 
                    *dti_lp                                 &
                    *wetmask_t(i,j)                         &
                       /anyv3d(i,j,k,id_dxdydz) > 1. ) then 
       write(10+par%rank,*)i,j,k,i+par%timax(1),j+par%tjmax(1),'coucou'
       write(10+par%rank,*)abs(veldydz_u(i  ,j  ,k,id_veltot)) &
                    *dti_lp                                    &
                    *wetmask_t(i,j)                            &
                       /anyv3d(i,j,k,id_dxdydz)                 
       write(10+par%rank,*)abs(veldydz_u(i+1,j  ,k,id_veltot)) &
                    *dti_lp                                    &
                    *wetmask_t(i,j)                            &
                       /anyv3d(i,j,k,id_dxdydz)                 
       write(10+par%rank,*)abs(veldxdz_v(i  ,j  ,k,id_veltot)) &
                    *dti_lp                                    &
                    *wetmask_t(i,j)                            &
                       /anyv3d(i,j,k,id_dxdydz)                 
       write(10+par%rank,*)abs(veldxdz_v(i  ,j+1,k,id_veltot)) &
                    *dti_lp                                    &
                    *wetmask_t(i,j)                            &
                       /anyv3d(i,j,k,id_dxdydz)                 
       write(10+par%rank,*)veldxdz_v(i  ,j+1,k,id_veltot),i,j+1,k
      endif
#endif
...
#ifdef bilan_s_3d
      call my_outputs_3dflux_s('wes',0) !12-02-22
#endif
...
#ifdef bilants6
call my_outputs_zone1salttempflux('it',id_flx_) ; call my_outputs_zone2salttempflux('it',id_flx_) ; call my_outputs_zone3salttempflux('it',id_flx_) ; call my_outputs_zone4salttempflux('it',id_flx_) ; call my_outputs_zone5salttempflux('it',id_flx_) ; call my_outputs_zone6salttempflux('it',id_flx_)
#endif
...
#ifdef bilants6
call my_outputs_zone1salttempflux('jt',id_fly_) ; call my_outputs_zone2salttempflux('jt',id_fly_) ; call my_outputs_zone3salttempflux('jt',id_fly_) ; call my_outputs_zone4salttempflux('jt',id_fly_) ; call my_outputs_zone5salttempflux('jt',id_fly_) ; call my_outputs_zone6salttempflux('jt',id_fly_) 
#endif
...
#ifdef bilants6
call my_outputs_zone1salttempflux('is',id_flx_) ; call my_outputs_zone2salttempflux('is',id_flx_) ; call my_outputs_zone3salttempflux('is',id_flx_) ; call my_outputs_zone4salttempflux('is',id_flx_) ; call my_outputs_zone5salttempflux('is',id_flx_) ; call my_outputs_zone6salttempflux('is',id_flx_)
#endif
...
#ifdef bilants6
call my_outputs_zone1salttempflux('js',id_fly_) ; call my_outputs_zone2salttempflux('js',id_fly_) ; call my_outputs_zone3salttempflux('js',id_fly_) ; call my_outputs_zone4salttempflux('js',id_fly_) ; call my_outputs_zone5salttempflux('js',id_fly_) ; call my_outputs_zone6salttempflux('js',id_fly_) 
#endif
...
!..............................................................................
#ifdef bidon
      subroutine advection_scal_vertical_laxwen
      use module_principal
      use module_parallele
      implicit none
!     double precision :: grdamp_=2. ! lower gradient amplification
      double precision :: laxwen_or_upwind_tem    & !=nc si laxwen, =1 si upwind
                         ,laxwen_or_upwind_sal      !=nc si laxwen, =1 si upwind
      integer :: id_cn_t_=5           &! current number "limited"
                ,id_cn_s_=6           &! current number "limited"
                ,id_we_=1           &! Explicit limited vertical velocity
                ,id_wi_=2           &! residual implicit vertical velocity
!               ,id_tdiv=4          &! cumulated T*div(current) !26-01-17
!               ,id_sdiv=5          &! cumulated S*div(current)
...
! Schema Lax-Wendroff avec limiteur d'oscillation et diffusion negative
...
!        anyv1d(k,id_cn_t_)=0. ! 100%C2
!        anyv1d(k,id_cn_t_)=1. ! 100%UP2

! Sauf si limiteur actif, c'est la meme chose pour S:
          anyv1d(k,id_cn_s_)=anyv1d(k,id_cn_t_)

       enddo ! fin de boucle sur k

!#ifdef bidon
      do k=3,kmax-1  ! do k=kmin_w(i,j)+1,kmax                         !22-12-14
!---- LIMITEURS ---->
! Calcule l'aiguillage "laxwen ou upwind" pour T et S
! Si =nombre_de_courant alors le schema est Laxwen
! Si =1                 alors le schema est upwind

       if(anyv3d(i,j,k,id_we_)>0.) then !www>

                          if(abs(tem_t(i,j,k-1,2)-tem_t(i,j,k-2,2))<  &
          anyv1d(k,id_cn_t_)*abs(tem_t(i,j,k  ,2)-tem_t(i,j,k-1,2))) then !pmx>
          anyv1d(k,id_cn_t_)=1.  !upwind
                            endif                                         !pmx>

                          if(abs(sal_t(i,j,k-1,2)-sal_t(i,j,k-2,2))<  &
          anyv1d(k,id_cn_s_)*abs(sal_t(i,j,k  ,2)-sal_t(i,j,k-1,2))) then !pmx>
          anyv1d(k,id_cn_s_)=1.  !upwind
                            endif                                         !pmx>

         else                           !www>

                          if(abs(tem_t(i,j,k+1,2)-tem_t(i,j,k  ,2))<  &
          anyv1d(k,id_cn_t_)*abs(tem_t(i,j,k  ,2)-tem_t(i,j,k-1,2))) then !pmx>
          anyv1d(k,id_cn_t_)=1.  !upwind
                            endif                                         !pmx>

                          if(abs(sal_t(i,j,k+1,2)-sal_t(i,j,k  ,2))<  &
          anyv1d(k,id_cn_s_)*abs(sal_t(i,j,k  ,2)-sal_t(i,j,k-1,2))) then !pmx>
          anyv1d(k,id_cn_s_)=1.  !upwind
                            endif                                         !pmx>
         endif                          !www>
...
!      anyv1d(1            ,id_cn_t_)=1. !05-03-21
!      anyv1d(kmax+1       ,id_cn_t_)=1. ! A la surface schema upwind 
       anyv1d(1:2          ,id_cn_t_)=1. !05-03-21
       anyv1d(kmax:kmax+1  ,id_cn_t_)=1. ! A la surface schema upwind 
       anyv1d(1:2          ,id_cn_s_)=1. !05-03-21
       anyv1d(kmax:kmax+1  ,id_cn_s_)=1. ! A la surface schema upwind 
! Noter que le choix nc=0 ou 1 n'aurait pas d'importance si la vitesse verticale est
! 100% implicite en kmax+1 . Ceci etant dit, noter que la vitesse
! verticale associee au flux atmospherique de surface est potentiellement explicite 
! (ca depend des versions, donc A verifier) et donc cela a de l'importance.
...
! BIDOUILLE 100% UPWIND ----->
!      anyv1d(kmax-10:kmax+1,id_cn_t_)=1. 
!      anyv1d(kmax-10:kmax+1,id_cn_s_)=1. 
! BIDOUILLE 100% UPWIND ----->

! Flux centres frozen:
       do k=2,kmax

        anyv1d(k,1)=-(1-anyv1d(k,id_cn_t_))*anyv3d(i,j,k,id_we_)  &
                  *( ( dz_t(i,j,k  ,1)*tem_t(i,j,k  ,1)             &
                      +dz_t(i,j,k-1,1)*tem_t(i,j,k-1,1))            &
                    /( dz_t(i,j,k  ,1) +dz_t(i,j,k-1,1)) )      

        anyv1d(k,2)=-(1-anyv1d(k,id_cn_s_))*anyv3d(i,j,k,id_we_)  &
                  *( ( dz_t(i,j,k  ,1)*sal_t(i,j,k  ,1)             &
                      +dz_t(i,j,k-1,1)*sal_t(i,j,k-1,1))            &
                    /( dz_t(i,j,k  ,1) +dz_t(i,j,k-1,1)) )      
       ...
       anyv1d(1     ,1:2)=0.
       k=kmax+1
       anyv1d(k,1)=-(1-anyv1d(k,id_cn_t_))*anyv3d(i,j,k,id_we_)*tem_t(i,j,k-1,1)
       anyv1d(k,2)=-(1-anyv1d(k,id_cn_s_))*anyv3d(i,j,k,id_we_)*sal_t(i,j,k-1,1)
...
! Flux de la partie iterative:

! Temperature
        anyv1d(k,3)=  &
          -0.5*( & !ooo>
! partie advective complementaire:
                anyv1d(k,id_cn_t_) & ! pour complementer le flux adv anyv1d(:,1)
               *anyv3d(i,j,k,id_we_) &
                                    *(tem_t(i,j,k,2)+tem_t(i,j,k-1,2)) &
! partie diffusive laxwen ou upwind:
           -abs(anyv3d(i,j,k,id_we_))   &
               *anyv1d(k,id_cn_t_)    & !=nc si laxwen, =1 si upwind
                                     *( & !m°v°m>
                                       tem_t(i,j,k,2)-tem_t(i,j,k-1,2)  & !Dif
!           -(1.-anyv1d(k,id_cn_t_))*(temf_t(i,j,k) -temf_t(i,j,k-1)  ) & !Neg Dif. !04-05-21
            -(1.-anyv1d(k,id_cn_t_))*ratio_negdif_ver*0.25*( & !-difneg->
                                tem_t(i,j,k+1,2) & !peu importe tem(kmax+1) car anyv1d(kmax,id_cn_t_)=0.
                               +tem_t(i,j,k  ,2) &
                               -tem_t(i,j,k-1,2) &
                               -tem_t(i,j,k-2,2) & !peu importe tem(0) car anyv1d(1,id_cn_t_)=0.
                                                       ) & !-difneg->
                                      ) & !m°v°m>
               )   !ooo>
...
! Salinite
        anyv1d(k,4)=  &
          -0.5*( & !ooo>
! partie advective complementaire:
                anyv1d(k,id_cn_s_) & ! pour complementer le flux adv anyv1d(:,1)
               *anyv3d(i,j,k,id_we_) &
                                    *(sal_t(i,j,k,2)+sal_t(i,j,k-1,2)) &
! partie diffusive laxwen ou upwind:
           -abs(anyv3d(i,j,k,id_we_))   &
               *anyv1d(k,id_cn_s_)    & !=nc si laxwen, =1 si upwind
                                     *( & !m°v°m>
                                       sal_t(i,j,k,2)-sal_t(i,j,k-1,2)  & !Dif
!           -(1.-anyv1d(k,id_cn_s_))*(salf_t(i,j,k) -salf_t(i,j,k-1)  ) & !Neg Dif. !04-05-21
            -(1.-anyv1d(k,id_cn_s_))*ratio_negdif_ver*0.25*( & !-difneg->
                                sal_t(i,j,k+1,2) & !peu importe sal(kmax+1) car anyv1d(kmax,id_cn_s_)=0.
                               +sal_t(i,j,k  ,2) &
                               -sal_t(i,j,k-1,2) &
                               -sal_t(i,j,k-2,2) & !peu importe sal(0) car anyv1d(1,id_cn_s_)=0.
                                                       ) & !-difneg->
                                      ) & !m°v°m>
               )   !ooo>
...
! conduisant a A2=A2+A1 !10-04-19
      ...
      end subroutine advection_scal_vertical_laxwen
#endif
!................................................................
#ifdef bidon
      subroutine advection_scal_lw_tem  !26-08-21
      use module_principal
      use module_parallele
      use module_my_outputs
      implicit none
...
! PARTIE SI U POSITIF:
      ...
      if(abs(anyv3d(i-1,j,k,id_tbef_)-anyv3d(i-2,j,k,id_tbef_))*mask_u(i-1,j,k)  &
        <abs(anyv3d(i  ,j,k,id_tbef_)-anyv3d(i-1,j,k,id_tbef_))*mask_u(i-1,j,k)  & 
        *(1.-anyv3d(i,j,k,id_hybcoefu))) then !-u-positif->

! SCHEMA UPWIND:
      anyv3d(i,j,k,id_flx_)=0.5*( & !prime>
! terme advectif complementaire et partie diffusive
      +( & !pmx>
           -veldydz_u(i,j,k,id_veltot)*(anyv3d(i,j,k,id_tbef_)+anyv3d(i-1,j,k,id_tbef_)) & 
       +abs(veldydz_u(i,j,k,id_veltot))*( & !mmm>
                                anyv3d(i,j,k,id_tbef_)-anyv3d(i-1,j,k,id_tbef_)   &
                                ) & !mmm>
       ) & !pmx>
...
! SCHEMA LAX-WENDROFF
      anyv3d(i,j,k,id_flx_)=0.5*( & !prime>
! terme advectif fixe:
      anyv3d(i,j,k,id_hybcoefu)*(-veldydz_u(i,j,k,id_veltot)*(tem_t(i,j,k,now)+tem_t(i-1,j,k,now)))  & 
! terme advectif complementaire et partie diffusive
      +(1.-anyv3d(i,j,k,id_hybcoefu))*( & !pmx>
           -veldydz_u(i,j,k,id_veltot)*(anyv3d(i,j,k,id_tbef_)+anyv3d(i-1,j,k,id_tbef_)) & 
       +abs(veldydz_u(i,j,k,id_veltot))*( & !mmm>
                                anyv3d(i,j,k,id_tbef_)-anyv3d(i-1,j,k,id_tbef_)   &
     -anyv3d(i,j,k,id_hybcoefu)*ratio_negdif_ver*( & !-difneg->
                                anyv3d(i+1,j,k,id_tbef_)*      0.25*mask_t(i+1,j,k) &
                               +anyv3d(i  ,j,k,id_tbef_)*( 0.5-0.25*mask_t(i+1,j,k)) &
                               +anyv3d(i-1,j,k,id_tbef_)*(-0.5+0.25*mask_t(i-2,j,k)) &
                               +anyv3d(i-2,j,k,id_tbef_)*(    -0.25*mask_t(i-2,j,k)) &
                                             ) & !-difneg->
                                ) & !mmm>
                                      ) & !pmx>
...
! PARTIE SI U NEGATIF:
      ...
      if(abs(anyv3d(i+1,j,k,id_tbef_)-anyv3d(i  ,j,k,id_tbef_))*mask_u(i+1,j,k)  &
        <abs(anyv3d(i  ,j,k,id_tbef_)-anyv3d(i-1,j,k,id_tbef_))*mask_u(i+1,j,k)  & 
        *(1.-anyv3d(i,j,k,id_hybcoefu))) then !-u-negatif->
...
#endif
#ifdef bidon
! UNIQUEMENT SCHEMA LAX-WENDROFF
      anyv3d(i,j,k,id_flx_)=0.5*( & !prime>
! terme advectif fixe:
      anyv3d(i,j,k,id_hybcoefu)*(-veldydz_u(i,j,k,id_veltot)*(tem_t(i,j,k,now)+tem_t(i-1,j,k,now)))  & 
! terme advectif complementaire et partie diffusive
      +(1.-anyv3d(i,j,k,id_hybcoefu))*( & !pmx>
       ...
       +abs(veldydz_u(i,j,k,id_veltot))*( & !mmm>
                                anyv3d(i,j,k,id_tbef_)-anyv3d(i-1,j,k,id_tbef_)   &
!   -anyv3d(i,j,k,id_hybcoefu)*(temf_t(i,j,k)         -temf_t(i-1,j,k)) & !05-05-21
                                ) & !mmm>
                                      ) & !pmx>
...
#endif
#ifdef bidon
! UNIQUEMENT SCHEMA CENTRE
      anyv3d(i,j,k,id_flx_)=0.5*(-veldydz_u(i,j,k,id_veltot)*(tem_t(i,j,k,now)+tem_t(i-1,j,k,now)))  
#endif
...
! PARTIE SI V POSITIF:
      ...
      if(abs(anyv3d(i,j-1,k,id_tbef_)-anyv3d(i,j-2,k,id_tbef_))*mask_v(i,j-1,k)  &
        <abs(anyv3d(i  ,j,k,id_tbef_)-anyv3d(i,j-1,k,id_tbef_))*mask_v(i,j-1,k)  & 
        *(1.-anyv3d(i,j,k,id_hybcoefv))) then !-v-positif->

! SCHEMA UPWIND:
      anyv3d(i,j,k,id_fly_)=0.5*( & !prime>
! terme advectif complementaire et partie diffusive
      +( & !pmx>
           -veldxdz_v(i,j,k,id_veltot)*(anyv3d(i,j,k,id_tbef_)+anyv3d(i,j-1,k,id_tbef_)) & 
       +abs(veldxdz_v(i,j,k,id_veltot))*( & !mmm>
                                anyv3d(i,j,k,id_tbef_)-anyv3d(i,j-1,k,id_tbef_)   &
                                ) & !mmm>
       ) & !pmx>
...
! SCHEMA LAX-WENDROFF
      anyv3d(i,j,k,id_fly_)=0.5*( & !prime>
! terme advectif fixe:
      anyv3d(i,j,k,id_hybcoefv)*(-veldxdz_v(i,j,k,id_veltot)*(tem_t(i,j,k,now)+tem_t(i,j-1,k,now)))  & 
! terme advectif complementaire et partie diffusive
      +(1.-anyv3d(i,j,k,id_hybcoefv))*( & !pmx>
           -veldxdz_v(i,j,k,id_veltot)*(anyv3d(i,j,k,id_tbef_)+anyv3d(i,j-1,k,id_tbef_)) & 
       +abs(veldxdz_v(i,j,k,id_veltot))*( & !mmm>
                                anyv3d(i,j,k,id_tbef_)-anyv3d(i,j-1,k,id_tbef_)   &
     -anyv3d(i,j,k,id_hybcoefv)*ratio_negdif_ver*( & !-difneg->
                                anyv3d(i,j+1,k,id_tbef_)*      0.25*mask_t(i,j+1,k) &
                               +anyv3d(i,j  ,k,id_tbef_)*( 0.5-0.25*mask_t(i,j+1,k)) &
                               +anyv3d(i,j-1,k,id_tbef_)*(-0.5+0.25*mask_t(i,j-2,k)) &
                               +anyv3d(i,j-2,k,id_tbef_)*(    -0.25*mask_t(i,j-2,k)) &
                                             ) & !-difneg->
                                ) & !mmm>
                                      ) & !pmx>
...
! PARTIE SI V NEGATIF:
      ...
      if(abs(anyv3d(i,j+1,k,id_tbef_)-anyv3d(i  ,j,k,id_tbef_))*mask_v(i,j+1,k)  &
        <abs(anyv3d(i  ,j,k,id_tbef_)-anyv3d(i,j-1,k,id_tbef_))*mask_v(i,j+1,k)  & 
        *(1.-anyv3d(i,j,k,id_hybcoefv))) then !-v-negatif->
...
!#endif
      ...
      end subroutine advection_scal_lw_tem 
#endif
!..............................................................................
#ifdef bidon
      subroutine advection_scal_lw_sal  !26-08-21
      use module_principal
      use module_parallele
      use module_my_outputs
      implicit none
...
! PARTIE SI U POSITIF:
      ...
      if(abs(anyv3d(i-1,j,k,id_sbef_)-anyv3d(i-2,j,k,id_sbef_))*mask_u(i-1,j,k)  &
        <abs(anyv3d(i  ,j,k,id_sbef_)-anyv3d(i-1,j,k,id_sbef_))*mask_u(i-1,j,k)  & 
        *(1.-anyv3d(i,j,k,id_hybcoefu))) then !-u-positif->
...
! terme advectif complementaire et partie diffusive
      +( & !pmx>
           -veldydz_u(i,j,k,id_veltot)*(anyv3d(i,j,k,id_sbef_)+anyv3d(i-1,j,k,id_sbef_)) & 
       +abs(veldydz_u(i,j,k,id_veltot))*( & !mmm>
                                anyv3d(i,j,k,id_sbef_)-anyv3d(i-1,j,k,id_sbef_)   &
                                ) & !mmm>
       ) & !pmx>
...
! SCHEMA LAX-WENDROFF
      anyv3d(i,j,k,id_flx_)=0.5*( & !prime>
! terme advectif fixe:
      anyv3d(i,j,k,id_hybcoefu)*(-veldydz_u(i,j,k,id_veltot)*(sal_t(i,j,k,now)+sal_t(i-1,j,k,now)))  & 
! terme advectif complementaire et partie diffusive
      +(1.-anyv3d(i,j,k,id_hybcoefu))*( & !pmx>
           -veldydz_u(i,j,k,id_veltot)*(anyv3d(i,j,k,id_sbef_)+anyv3d(i-1,j,k,id_sbef_)) & 
       +abs(veldydz_u(i,j,k,id_veltot))*( & !mmm>
                                anyv3d(i,j,k,id_sbef_)-anyv3d(i-1,j,k,id_sbef_)   &
     -anyv3d(i,j,k,id_hybcoefu)*ratio_negdif_ver*( & !-difneg->
                                anyv3d(i+1,j,k,id_sbef_)*      0.25*mask_t(i+1,j,k) &
                               +anyv3d(i  ,j,k,id_sbef_)*( 0.5-0.25*mask_t(i+1,j,k)) &
                               +anyv3d(i-1,j,k,id_sbef_)*(-0.5+0.25*mask_t(i-2,j,k)) &
                               +anyv3d(i-2,j,k,id_sbef_)*(    -0.25*mask_t(i-2,j,k)) &
                                             ) & !-difneg->
                                ) & !mmm>
                                      ) & !pmx>
...
! PARTIE SI U NEGATIF:
      ...
      if(abs(anyv3d(i+1,j,k,id_sbef_)-anyv3d(i  ,j,k,id_sbef_))*mask_u(i+1,j,k)  &
        <abs(anyv3d(i  ,j,k,id_sbef_)-anyv3d(i-1,j,k,id_sbef_))*mask_u(i+1,j,k)  & 
        *(1.-anyv3d(i,j,k,id_hybcoefu))) then !-u-negatif->
...
! PARTIE SI V POSITIF:
      ...
      if(abs(anyv3d(i,j-1,k,id_sbef_)-anyv3d(i,j-2,k,id_sbef_))*mask_v(i,j-1,k)  &
        <abs(anyv3d(i  ,j,k,id_sbef_)-anyv3d(i,j-1,k,id_sbef_))*mask_v(i,j-1,k)  & 
        *(1.-anyv3d(i,j,k,id_hybcoefv))) then !-v-positif->
...
! terme advectif complementaire et partie diffusive
      +( & !pmx>
           -veldxdz_v(i,j,k,id_veltot)*(anyv3d(i,j,k,id_sbef_)+anyv3d(i,j-1,k,id_sbef_)) & 
       +abs(veldxdz_v(i,j,k,id_veltot))*( & !mmm>
                                anyv3d(i,j,k,id_sbef_)-anyv3d(i,j-1,k,id_sbef_)   &
                                ) & !mmm>
       ) & !pmx>
...
! SCHEMA LAX-WENDROFF
      anyv3d(i,j,k,id_fly_)=0.5*( & !prime>
! terme advectif fixe:
      anyv3d(i,j,k,id_hybcoefv)*(-veldxdz_v(i,j,k,id_veltot)*(sal_t(i,j,k,now)+sal_t(i,j-1,k,now)))  & 
! terme advectif complementaire et partie diffusive
      +(1.-anyv3d(i,j,k,id_hybcoefv))*( & !pmx>
           -veldxdz_v(i,j,k,id_veltot)*(anyv3d(i,j,k,id_sbef_)+anyv3d(i,j-1,k,id_sbef_)) & 
       +abs(veldxdz_v(i,j,k,id_veltot))*( & !mmm>
                                anyv3d(i,j,k,id_sbef_)-anyv3d(i,j-1,k,id_sbef_)   &
     -anyv3d(i,j,k,id_hybcoefv)*ratio_negdif_ver*( & !-difneg->
                                anyv3d(i,j+1,k,id_sbef_)*      0.25*mask_t(i,j+1,k) &
                               +anyv3d(i,j  ,k,id_sbef_)*( 0.5-0.25*mask_t(i,j+1,k)) &
                               +anyv3d(i,j-1,k,id_sbef_)*(-0.5+0.25*mask_t(i,j-2,k)) &
                               +anyv3d(i,j-2,k,id_sbef_)*(    -0.25*mask_t(i,j-2,k)) &
                                             ) & !-difneg->
                                ) & !mmm>
                                      ) & !pmx>
...
! PARTIE SI V NEGATIF:
      ...
      if(abs(anyv3d(i,j+1,k,id_sbef_)-anyv3d(i  ,j,k,id_sbef_))*mask_v(i,j+1,k)  &
        <abs(anyv3d(i  ,j,k,id_sbef_)-anyv3d(i,j-1,k,id_sbef_))*mask_v(i,j+1,k)  & 
        *(1.-anyv3d(i,j,k,id_hybcoefv))) then !-v-negatif->
...
!#endif
      ...
      end subroutine advection_scal_lw_sal 
#endif
!..............................................................................
...
#endif
#ifdef bilan_s_3d
      call my_outputs_3dflux_s('is',id_flx_) !12-02-22
#endif
...
#endif
#ifdef bilan_s_3d
      call my_outputs_3dflux_s('js',id_fly_) !12-02-22
#endif
...
#ifdef bidon
      if(.not.allocated(tridia_in_oi)) then !--->
               allocate(tridia_in_oi(0:imax+1,0:jmax+1,kmax,-1:4)) ; tridia_in_oi=0
      endif                                 !--->
#endif
...
!....................................................
#ifdef bidon
! VERSION pour tableau generique anyv3d
...
!      write(10+par%rank,*)'ENVOI send_status_=',send_status_
       ...
       if(send_status_==1) then !>>>          
        ...
        do k=1,kmax ; do j=1,jmax
         k0=k0+1
         adv_send_est(k0)=anyv3d(i,j,k,idvar_)
        enddo       ; enddo
...
!      write(10+par%rank,*)'RECOI recv_status_=',recv_status_
      ...
      if(recv_status_==1) then !>>>          
       ...
       do k=1,kmax ; do j=1,jmax
        k0=k0+1
        anyv3d(i,j,k,idvar_)=adv_recv_ouest(k0)
       enddo       ; enddo
...
! Frontiere ouest:
      if (par%tvoisin(ouest) /= mpi_proc_null) then !pmx>
       ...
       if(send_status_==1) then !>>>          
        ...
        do k=1,kmax ; do j=1,jmax
         k0=k0+1
         adv_send_ouest(k0)=anyv3d(i,j,k,idvar_)
        enddo       ; enddo
        send_status_=2 ! pour memoriser le fait que l'envoi est fait
       else                     !>>>          
        k0=1
...
! Frontiere Est:
      ...
      if(recv_status_==1) then !>>>          
       ...
       do k=1,kmax ; do j=1,jmax
        k0=k0+1
        anyv3d(i,j,k,idvar_)=adv_recv_est(k0)
       enddo       ; enddo
       recv_status_=2 ! pour memoriser le fait que la reception est faite
      endif                    !>>>          
      endif                                       !m°v°m>
...
!      write(10+par%rank,*)'ENVOI send_status_=',send_status_
       ...
       if(send_status_==1) then !>>>          
        ...
        do k=1,kmax ; do i=1,imax
         k0=k0+1
         adv_send_nord(k0)=anyv3d(i,j,k,idvar_)
        enddo       ; enddo
        send_status_=2 ! pour memoriser le fait que l'envoi est fait
       else                     !>>>          
        k0=1
...
!      write(10+par%rank,*)'RECOI recv_status_=',recv_status_
      ...
      if(recv_status_==1) then !>>>          
       ...
       do k=1,kmax ; do i=1,imax
        k0=k0+1
        anyv3d(i,j,k,idvar_)=adv_recv_sud(k0)
       enddo       ; enddo
       recv_status_=2 ! pour memoriser le fait que la reception est faite
      endif                    !>>>          
      endif                                         !m°v°m>
...
! Frontiere Sud:
      if (par%tvoisin(sud) /= mpi_proc_null) then !pmx>
       ...
       if(send_status_==1) then !>>>          
        ...
        do k=1,kmax ; do i=1,imax
         k0=k0+1
         adv_send_sud(k0)=anyv3d(i,j,k,idvar_)
        enddo       ; enddo
        send_status_=2 ! pour memoriser le fait que l'envoi est fait
       else                     !>>>          
        k0=1
...
! Frontiere Nord:
      ...
      if(recv_status_==1) then !>>>          
       ...
       do k=1,kmax ; do i=1,imax
        k0=k0+1
        anyv3d(i,j,k,idvar_)=adv_recv_nord(k0)
       enddo       ; enddo
       recv_status_=2 ! pour memoriser le fait que la reception est faite
      endif                    !>>>          
      endif                                       !m°v°m>

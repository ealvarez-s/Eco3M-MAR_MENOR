...
! release 334 - last update: 03-03-22
!______________________________________________________________________
...
! v310    05-11-21  - Le flux A la surface prend en compte le nouvel algo wet dry
!                   - Commenter call vertmix_merged_levels_bio 
! v329    23-02-22  if(flag_merged_levels==1)call vertmix_merged_levels_bio!07-03-19!23-02-22
! v334    03-03-22  melange dans zone intertidale
!...............................................................................
...
!     if(par%rank==0)write(6,*)'bidouille mixsedbio'
!     tendancebio_t=0.
!     fluxbio_w=0.
...
! L'augmentation du melange dans la couche intertidale: !03-03-22
          +(1-wetmask_wi_t(i,j))*5.  
...
! Surface: nouvelles hypotheses du !05-11-21
! Dans advection_bio la vitesse verticale explicite est nulle en kmax+1. La vitesse verticale en kmax+1 est 
! donc 100% implicite MAIS ATTENTION on doit distinguer 2 contributions A cette vitesse verticale de surface.
! Premierement une contribution physique en rapport avec les flux d'eau physique A la surface (precip, evap). Cette
! contribution est associEe A un flux nul (hypothese la plus naturelle, et si jamais on veut introduire des 
! flux atmospherique on le fait ailleurs avec le tableau fluxbio_w(:,:,2) qui est nul par defaut). La consequence
! de ce raisonnement est que la vitesse verticale implicite de surface ne doit pas contenir cette vitesse verticale physique
! Pour info cette vitesse est donnEe par 0.5*(omega_evaprec_w(i,j,0)+omega_evaprec_w(i,j,1)) qui est donc
! soustraite A omega pour fournir anyv3d(i,j,kmax+1,id_wi_) en suivant. 
! Deuxiemement une contribution numerique consequence du nouvel algo de banc decouvrant. La conditon associee A cette
! contribution est gradient nul (voir codage de tridia_in(:,:,1:3). 
          anyv3d(i,j,kmax+1,id_wi_)=omega_w(i,j,kmax+1,1) & !05-11-21
                      -0.5*(omega_evaprec_w(i,j,0)+omega_evaprec_w(i,j,1)) 
...
! ligne suivante commentee le 05-11-21 retablie le 23-02-22
      if(flag_merged_levels==1)call vertmix_merged_levels_bio!07-03-19!23-02-22

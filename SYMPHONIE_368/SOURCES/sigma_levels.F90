...
!______________________________________________________________________
      ...
      implicit none
#ifdef synopsis
       subroutinetitle='sigma_levels'
       subroutinedescription= &
       'Computes depth and dimensionless "sigma" depth of grid nodes'
...
!.............................................................
      ...
      implicit none
#ifdef synopsis
       subroutinetitle='sigma_levels_depth_w'
       subroutinedescription= &
       'Computes depth and dimensionless "sigma" depth of grid nodes'
       call main_synopsis(subroutinetitle,subroutinedescription)
#endif
...
!-----------------------------------------------------------------------------
#ifdef bidon
      subroutine sigma_levels_hydrocons
      use module_principal
      use module_parallele
      implicit none
      integer loop_
#ifdef synopsis
       subroutinetitle='sigma_levels_hydrocons'
       subroutinedescription=' '
       call main_synopsis(subroutinetitle,subroutinedescription)
#endif

! Memoriser le first guess pour depth_w
      id_z=0
      do k=0,kmax+1
      do j=0,jmax+1
      do i=0,imax+1
       anyv3d(i,j,k,id_z)=depth_w(i,j,k)
      enddo
      enddo
      enddo


! Contrainte 1: le  niveau z(i,j,k) doit etre au dessus des voisins du niveau k-1
! (egalement contrainte de la coherence hydrostatique de Haney 1991)

      do k=2,kmax+1

        do j=0,jmax+1 
        jp1=min(j+1,jmax+1) ; jm1=max(j-1,0)
         do i=0,imax+1
          ip1=min(i+1,imax+1) ; im1=max(i-1,0)
          xy_t(i,j,1)=max(max(max(max(               &
                            depth_w(i  ,j  ,k  )     &
                           ,depth_w(im1,j  ,k-1))    &
                           ,depth_w(ip1,j  ,k-1))    &
                           ,depth_w(i  ,jm1,k-1))    &
         ...
         enddo
        enddo
        call obc_ext_mpi_xyt(1,'z1') ! (t_,txt_)

        do j=0,jmax+1 
        do i=0,imax+1
         depth_w(i,j,k)=xy_t(i,j,1)
        enddo 
        enddo

! Reactualliser la position du niveau suivant (k+1) en suivant
! une logique de maintient d'une proportionalite de type sigma:
! sigma=(Zsurf-Z(k+1))/(Zsurf-ZRef) ou ZRef est Z(k) "avant" et "apres"
       if(k<kmax+1) then !>>>>
        do j=0,jmax+1 
        do i=0,imax+1
         depth_w(i,j,k+1)=depth_w(i,j,k)                               &
                        +(anyv3d(i,j,k+1   ,id_z)-anyv3d(i,j,k,id_z))  &
                        /(anyv3d(i,j,kmax+1,id_z)-anyv3d(i,j,k,id_z))  &
                        *(anyv3d(i,j,kmax+1,id_z)-depth_w(i,j,k))
...
#ifdef bidon
! Contrainte 2: la regularite de l'espacement qui impose que la resolution du
! niveau k ne differe pas trop de la resolution au niveau k-1. Cette etape passe
! par la construction de dsig_t la resolution adimensionnee
      x1=0.99 ; x2=1/x1 ! les facteurs de regularite
      do j=0,jmax+1
      do i=0,imax+1 
       x0=1./(depth_w(i,j,kmax+1)-depth_w(i,j,1))
       do k=1,kmax
        dsig_t(i,j,k)=x0*(depth_w(i,j,k+1   )-depth_w(i,j,k))
       enddo ! k

! sum1 sert a corriger l'erreur sur la somme verticale de dsig_t qui doit etre 1
       sum1=dsig_t(i,j,1)
       do k=2,kmax 
        dsig_t(i,j,k)=min(max(dsig_t(i,j,k),x1*dsig_t(i,j,k-1)) &
                                           ,x2*dsig_t(i,j,k-1)) 
        sum1=sum1+dsig_t(i,j,k) 
       enddo ! k
       x0=1./max(sum1,small1) ! pour eviter division par zero
! correction somme dsig_t=1
       anyv3d(i,j,1,0)=0.
       do k=1,kmax
        dsig_t(i,j,k)=x0*dsig_t(i,j,k)
        anyv3d(i,j,k+1,0)=anyv3d(i,j,k,0)+dsig_t(i,j,k) ! sigma=sum(dsig)
       enddo ! k
      enddo
      enddo
#endif
...
! Un peu de lissage sur sigma
      do k=1,kmax+1
      do j=0,jmax+1
      do i=0,imax+1 
       anyv3d(i,j,k,0:1)=(depth_w(i,j,k     )-depth_w(i,j,1)) &
                        /(depth_w(i,j,kmax+1)-depth_w(i,j,1))  
      enddo
      enddo
      enddo
      ...
      do loop_=1,5
       ...
       do j=1,jmax
       do i=1,imax
        anyv3d(i,j,k,1)=0.5*    anyv3d(i  ,j  ,k,0)    &
                       +0.125*( anyv3d(i-1,j  ,k,0)    &
                               +anyv3d(i+1,j  ,k,0)    &
                               +anyv3d(i  ,j-1,k,0)    &
                               +anyv3d(i  ,j+1,k,0))    
       enddo
       ...
       do j=1,jmax
       do i=1,imax
        anyv3d(i,j,k,0)=anyv3d(i,j,k,1) 
       enddo
       enddo
       enddo
       call obc_int_anyv3d(0,'z1')
      enddo ! loop_

! Retrouver dsig_t depuis sigma
       do k=1,kmax
       do j=0,jmax+1
       do i=0,imax+1
        dsig_t(i,j,k)=anyv3d(i,j,k+1,0)-anyv3d(i,j,k,0)
       enddo
       enddo
       enddo

! Retrouver depth_w depuis dsig_t
      do j=0,jmax+1
      do i=0,imax+1
       x0=depth_w(i,j,kmax+1)-depth_w(i,j,1)
       do k=2,kmax
        depth_w(i,j,k)=depth_w(i,j,k-1)+x0*dsig_t(i,j,k-1)
       enddo
      enddo
      enddo
!#endif

      end subroutine sigma_levels_hydrocons
#endif
!-----------------------------------------------------------------------------
...
!#ifdef bidon
      ...
      implicit none
#ifdef synopsis
       subroutinetitle='sigma_levels_2dv_plot'
       subroutinedescription=' '
       call main_synopsis(subroutinetitle,subroutinedescription)
#endif
...
!..................................................................
      ...
      integer loop_
#ifdef synopsis
       subroutinetitle='sigma_levels_mpi_f'
       subroutinedescription='mpi continuity f grid parameters'
       call main_synopsis(subroutinetitle,subroutinedescription)
#endif
#ifdef parallele
...
!.................................................................
      ...
      implicit none
#ifdef synopsis
       subroutinetitle='sigma_levels_consistency'
       subroutinedescription= &
       'Computes dsig_t dsig_u dsig_v from depth_w'
       call main_synopsis(subroutinetitle,subroutinedescription)
#endif
...
! dsig_u
       ...
       do j=0,jmax+1
       do i=1,imax+1
        dsig_u(i,j,k)=(  ( depth_w(i  ,j  ,k+1   )     &
                          +depth_w(i-1,j  ,k+1   ) )   &
                        -( depth_w(i  ,j  ,k     )     &
                          +depth_w(i-1,j  ,k     ) ) ) &
                     /(  ( depth_w(i  ,j  ,kmax+1)     &
                          +depth_w(i-1,j  ,kmax+1) )   &
                        -( depth_w(i  ,j  ,1     )     &
                          +depth_w(i-1,j  ,1     ) ) )
       enddo
       enddo
       enddo
       dsig_u=max(dsig_u,small1) !06-06-19

! dsig_v
       do k=1,kmax
       do j=1,jmax+1
       do i=0,imax+1
        dsig_v(i,j,k)=(  ( depth_w(i  ,j  ,k+1   )     &
                          +depth_w(i  ,j-1,k+1   ) )   &
                        -( depth_w(i  ,j  ,k     )     &
                          +depth_w(i  ,j-1,k     ) ) ) &
                     /(  ( depth_w(i  ,j  ,kmax+1)     &
                          +depth_w(i  ,j-1,kmax+1) )   &
                        -( depth_w(i  ,j  ,1     )     &
                          +depth_w(i  ,j-1,1     ) ) )
       enddo
       enddo
       enddo
       dsig_v=max(dsig_v,small1) !06-06-19
#endif

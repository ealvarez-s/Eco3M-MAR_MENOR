...
! release 358 - last update: 29-10-22
!______________________________________________________________________
...
! https://docs.google.com/document/d/1VMmzC7jXV7sycAxoFCUcOBxVbydbyXD3imgM0wAnAFQ/edit
      ...
      real*4 , dimension(:,:,:), allocatable :: ogcmvar_r4 !09-11-21
...
!     integer :: vel_id=2,trc_id=1,ssh_id=3,var_num=3
      ...
      integer,dimension(:,:),allocatable :: ogcmplugs !26-03-20
      integer :: ogcmmisspointmax=0       &           !26-03-20
                ,flag_oasis_lr_msk=0      &           !27-07-20
                ,ogcmmisspointmax2d=0                 !16-09-20

!     double precision :: ogcm_time_lag=1200.
...
! v245    23-01-19  ajout merge avec bathy ogcm cas sympa
! v255    15-05-19  pour moins de cpu changement de l'ordre des operations pour
...
! v260    19-10-19  cas sympa couches fusionneEs
! v273    26-01-20  debug ecriture/lecture ogcmgridcode
! v276    26-03-20  suite aux accidents de lecture du fichier bouchetrou avec
!                   OCCIGEN il a ete decide de passer par le tableau ogcmplugs
! v287    27-07-20  Ecrire dans un fichier les points de la grille LR dont a besoin la
!                   grille HR (fichier pre-traitement d'un couplage OASIS) 
!         30-08-20  De meilleurs diag de debug
! v288    16-09-20  correction d'un bug sur l'interpolation de la ssh introduit dans la v276
! v290    06-10-20  Test "sous le fond" utilise depth_w en k=1 (et pas k=kmin)
! v309    13-09-21  ajout de noms pour lecture netcdf roms
! v310    13-10-21  suite du point precedent
!         09-11-21  mise A jour pour lecture bathy copernicus
! v313    25-11-21  detecter automatiquement si date contient heures, minutes, secondes
!         26-11-21  loop3_ remplace k1
! v317    22-12-21  modif thom 
! v319    27-12-21  if(allocated(ogcmplugs))deallocate(ogcmplugs) !27-12-21
! v337    17-03-22  +ogcm_time_lag, nul par defaut, est un decallage de temps ajoutE au temps de l'ogcm
! v345    07-04-22  +offset_sshobc deplacE depuis update_obcforcing dans module_ogcm
! v358    29-10-22  if(obctime_order==4)
!...............................................................................
...
!     status=nf_open(texte250(1:k-1),nf_nowrite,ncid1)
      ...
      if(par%rank==0)write(6,*)'About to load SSH'
       ...
       if(status/=0)status=nf_inq_varid(ncid1,'zeta',var_id)   !13-09-21
...
!     if(ichoix==1.and.obc_ogcm_type(1:6)=='nemo_z') then
      ...
      if(obc_ogcm_type(1:5)=='sympa') then !symphonie> !15-11-17
       ...
       if(ichoix==2.or.ichoix==4) then
        ...
        if(status/=0)status=nf_inq_varid(ncid_,'salt',var_id) !13-09-21
...
!     enddo        ; enddo        ; enddo
      if(status/=0) then
       write(10+par%rank,*)'max_x,max_y,max_z',max_x,max_y,max_z
       write(10+par%rank,*)'lbound(short3d)',lbound(short3d)
       write(10+par%rank,*)'ubound(short3d)',ubound(short3d)
       write(10+par%rank,*)'ogcm_var_ndims',ogcm_var_ndims
       write(10+par%rank,*)'varcount(1:ogcm_var_ndims)',varcount(1:ogcm_var_ndims)
       write(10+par%rank,*)'varstart(1:ogcm_var_ndims)',varstart(1:ogcm_var_ndims)
       write(10+par%rank,*)'varstart(1)+varcount(1)',varstart(1)+varcount(1)
       write(10+par%rank,*)'varstart(2)+varcount(2)',varstart(2)+varcount(2)
       write(10+par%rank,*)'varstart(3)+varcount(3)',varstart(3)+varcount(3)
      endif
...
!------------------------------------------------------------------------------
      ...
      if(status/=0)status=nf_inq_dimid(ncid_,'xi_rho',dim_x_id) !13-10-21
      if(status/=0)status=nf_inq_dimid(ncid_,'longitude',dim_x_id) !09-11-21
      ...
      if(status/=0)status=nf_inq_dimid(ncid_,'eta_rho',dim_y_id) !13-10-21
      if(status/=0)status=nf_inq_dimid(ncid_,'latitude',dim_y_id) !09-11-21
      ...
      if(status/=0)status=nf_inq_dimid(ncid_,'s_rho',dim_z_id) !13-10-21
      if(status/=0)status=nf_inq_dimid(ncid_,'deptht',dim_z_id) !21-04-14
      if(status/=0)status=nf_inq_dimid(ncid_,'depth',dim_z_id) !09-11-21
...
! LIRE BATHY OGCM sympa
       ...
       if(status/=0)status=nf_inq_varid(ncid_,'hsmth',var_id) !13-10-21
...
! LIRE BATHY ou E3T:
      ...
      if(status/=0)status=nf_inq_varid(ncid_,'deptho',var_id)
      ...
      allocate(ogcmvar_r4(max_x,max_y,varcount(3)))
      if(k0==6) then !dpdpdpdpdpdp>
       status=nf_get_att_double(ncid_,var_id,'_FillValue',filvalr8) !09-11-21
       if(status/=0)stop 'Erreur nf_get_att_double filvalr8'
       status=nf_get_vara_double(ncid_,var_id                          &
                              ,varstart(1:ogcm_var_ndims)              &
                              ,varcount(1:ogcm_var_ndims)              &
                            ,ogcmvar_r8(1:max_x,1:max_y,1:varcount(3)))
        if(status/=0)stop ' erreur nf_get_vara_double ogcmvar_r8'
        do k=1,varcount(3) ; do j=1,max_y ; do i=1,max_x
         if(ogcmvar_r8(i,j,k)==filvalr8)ogcmvar_r8(i,j,k)=0. !09-11-21
        enddo              ; enddo        ; enddo
      endif          !dpdpdpdpdpdp>
      if(k0==5) then !spspspspspsp> !09-11-21
       status=nf_get_att_real(ncid_,var_id,'_FillValue',filval) !09-11-21
       if(status/=0)stop 'Erreur nf_get_att_real filval'
       status=nf_get_vara_real(ncid_,var_id                            &
                              ,varstart(1:ogcm_var_ndims)              &
                              ,varcount(1:ogcm_var_ndims)              &
                            ,ogcmvar_r4(1:max_x,1:max_y,1:varcount(3)))
        if(status/=0)stop ' erreur nf_get_vara_real ogcmvar_r4'
        do k=1,varcount(3) ; do j=1,max_y ; do i=1,max_x
         if(isnan(ogcmvar_r4(i,j,k))) ogcmvar_r4(i,j,k)=0.
         if(ogcmvar_r4(i,j,k)==filval)ogcmvar_r4(i,j,k)=0. !09-11-21
            ogcmvar_r8(i,j,k)=ogcmvar_r4(i,j,k)
        enddo              ; enddo        ; enddo
      endif          !spspspspspsp>
...
! Interpolation mask:
      ...
      if(allocated(ogcmvar_r4)) deallocate(ogcmvar_r4) !09-11-21
      if(allocated(ogcmvar_r8)) deallocate(ogcmvar_r8)
...
!      mask_t(i,j,kmaxp1)=nint( (1.-x2)*mask_t(i,j,kmaxp1)+x2*xy_t(i,j,2) ) !21-04-14
       if (x1/=0.) h_w(i,j)=(1.-x1)*h_w(i,j)+x1*xy_t(i,j,1)                         !22-12-21 modif thom
       if (x2/=0.) mask_t(i,j,kmax)=nint( (1.-x2)*mask_t(i,j,kmax)+x2*xy_t(i,j,2) ) !22-12-21 modif thom
...
!------------------------------------------------------------------------------
      ...
      ,linemax_,loop2_,loop1_,var_count_,unit_,loop3_
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      do loop3_=1,8 !10 ! debut de boucle loop3_

        if(loop3_<=2)          var_count_=trc_id
        if(loop3_>=3.and.loop3_<=4)var_count_=vel_id
        if(loop3_==5)          var_count_=ssh_id
        if(loop3_>=6)          var_count_=0

        if(loop3_==1) texte30=trim(liste_ogcm_t)
        if(loop3_==2) texte30=trim(liste_ogcm_s)
        if(loop3_==3) texte30=trim(liste_ogcm_u)
        if(loop3_==4) texte30=trim(liste_ogcm_v)
        if(loop3_==5) texte30=trim(liste_ogcm_ssh)
        if(loop3_==6) texte30=trim(liste_ogcm_grid_t)
        if(loop3_==7) texte30=trim(liste_ogcm_grid_u)
        if(loop3_==8) texte30=trim(liste_ogcm_grid_v)
...
! Sinon il continue comme d'hab.
       ...
       if(par%rank==0)write(6,'(a,a)')'Faire la liste ',trim(texte30)
        ...
        open(unit=3,file=obcfile(loop3_,1))
...
! Combien y a t'il de lignes dans le fichier?
 ...
 1832    rewind(3)
         if(linemax_==0) then         !>>>>> debug !23-04-14
          write(6,'(a,a,a)')'File ',trim(obcfile(loop3_,1)),' is empty'
          call ogcm_stop('ogcm_lire_les_listes erreur 1928',1)
         endif                        !>>>>>
!        if(loop3_>5)linemax_=1  !13-02-16 ! commentE le !16-05-19 pour possibilite plusieurs grilles differentes en cours de simulation
...
! Fichiers de variables seulement (fichier de grille non concernes)
         if(loop3_<6) then !>>>>>>>
            ...
            hour_=0 ; minute_=0 ; second_=0 !25-11-21
            if(len_trim(txt_units)>=k+18)read(txt_units(k+17:k+18),*)hour_   !25-11-21
            if(len_trim(txt_units)>=k+21)read(txt_units(k+20:k+21),*)minute_ !25-11-21
            if(len_trim(txt_units)>=k+24)read(txt_units(k+23:k+24),*)second_ !25-11-21
...
! Refaire proprement nc=1 en se servant de nc=2
               if(nc==2) then !nc=2 nc=2 nc=2>
                ...
                write(unit_,rec=1,iostat=i10)texte250,i0,x0      &
                                 ...
                                 ,ogcmgridcode                   & !16-05-19
                                 ,datesim(1:6,1)  ! date repere nc=1 (deplace apres ogcmgridcode le !26-01-20)
...
!     do loop3_=1,3 ! debut de boucle K1
      do loop3_=1,4 ! debut de boucle loop3_
        nc=1
        ogcm_rec_max=nc                                                    !07-09-09
        open(unit=3,file=obcfile(loop3_,1))
        ...
        if(loop3_==1)texte30=trim(liste_ogcm_t)         !13-10-10
        if(loop3_==2)texte30=trim(liste_ogcm_s)         !13-10-10
        if(loop3_==3)texte30=trim(liste_ogcm_u)         !13-10-10
        if(loop3_==4)texte30=trim(liste_ogcm_v)         !13-10-10
...
!------------------------------------------------------------------------------
      ...
      integer k_,loop1_   ,loop2_,fileunit_,ncid_
      integer , dimension(:,:) , allocatable :: oasis_grid_msk,oasis_grid_msk_glb
...
!.....OASIS...>
! Dans l'hypothese d'une etape preparatoire A un couplage symphonie-symphonie avec OASIS
! ecrire dans un fichier les points de la grille LR dont a besoin la grille HR !27-07-20
      if(k_==1.and.flag_oasis_lr_msk==1) then
        allocate(oasis_grid_msk    (max_x,max_y))
        allocate(oasis_grid_msk_glb(max_x,max_y))
        oasis_grid_msk=1
        oasis_grid_msk_glb=1
      endif
!.....OASIS...>
...
!                                et e Dlat=lat(i,j)-latmeteo(i0,j0)
      ...
      if(ogcm_lon(i0  ,j0)==-9999.or. &
         ogcm_lat(i0  ,j0)==-9999) then !debug > !30-08-20

        flag_stop=1

        write(10+par%rank,'(a)')'Problem with parent''s grid lon lat:'
        write(10+par%rank,'(a)')'ogcm_lon(i0  ,j0)',ogcm_lon(i0  ,j0)
        write(10+par%rank,'(a)')'ogcm_lat(i0  ,j0)',ogcm_lat(i0  ,j0)
        write(10+par%rank,'(a)')'i0,j0',i0,j0
        write(10+par%rank,'(a)')'These inconsistent values are probably' &
       ,' related to an incomplete grid (without the useless domains).' &
       ,' Use a complete parent grid.'

       goto 3240 !30-08-20

      endif                             !debug > !30-08-20
...
! Determinant principal:
      x1=( dlon_di_                                             &
       ...
       if(x1==0) then !debug> !15-11-17
        flag_stop=1
        write(10+par%rank,'(a)')trim(nomfichier(8))
        write(10+par%rank,*)'iobc_ogcm',iobc_ogcm
        write(10+par%rank,*)'Determinant principal=',x1
        write(10+par%rank,*)'i,j loc',i,j
        write(10+par%rank,*)'i,j glb',i+par%timax(1),j+par%tjmax(1)
        write(10+par%rank,*)'i0,j0',i0,j0
        write(10+par%rank,*)'dlon_di_',dlon_di_
        write(10+par%rank,*)'2 lon   ',ogcm_lon(i0+1,j0  ),ogcm_lon(i0-1,j0  )
        write(10+par%rank,*)'dlon_dj_',dlon_dj_
        write(10+par%rank,*)'2 lon   ',ogcm_lon(i0  ,j0+1),ogcm_lon(i0 ,j0-1)
        write(10+par%rank,*)'dlat_dj ',ogcm_lat(i0  ,j0+1)-ogcm_lat(i0  ,j0-1)
        write(10+par%rank,*)'2 lat   ',ogcm_lat(i0  ,j0+1),ogcm_lat(i0  ,j0-1)
        write(10+par%rank,*)'dlat_di ',ogcm_lat(i0+1,j0  )-ogcm_lat(i0-1,j0  )
        write(10+par%rank,*)'2 lat   ',ogcm_lat(i0+1,j0  ),ogcm_lat(i0-1,j0  )
        goto 3240 !30-08-20
...
!.....OASIS...>
           if(k_==1.and.flag_oasis_lr_msk==1) then !oasis> !28-07-20
            do j7=-1,1 ; do i7=-1,1 
! i7 et j7 servent A introduire une incertitude sur deci, decj pouvant conduire A elargir le masque oasis d'un point supplementaire
            oasis_grid_msk( min(max(int(deci+i7*0.01)  ,1),max_x),min(max(int(decj+j7*0.01)  ,1),max_y)  )=0
            oasis_grid_msk( min(max(int(deci+i7*0.01)+1,1),max_x),min(max(int(decj+j7*0.01)  ,1),max_y)  )=0
            oasis_grid_msk( min(max(int(deci+i7*0.01)+1,1),max_x),min(max(int(decj+j7*0.01)+1,1),max_y)  )=0
            oasis_grid_msk( min(max(int(deci+i7*0.01)  ,1),max_x),min(max(int(decj+j7*0.01)+1,1),max_y)  )=0
            enddo      ; enddo
           endif                                   !oasis>
!.....OASIS...>
 ...
 3240 call mpi_allreduce(flag_stop,k0,1,mpi_integer,mpi_sum,par%comm2d,ierr)
      if(k0/=0)stop 'Err 3240 module_ogcm see fort.xx files'

!.....OASIS...>
      if(k_==1.and.flag_oasis_lr_msk==1) then !m°v°m> !28-07-20
       k=max_x*max_y
       call mpi_allreduce(oasis_grid_msk,oasis_grid_msk_glb,k,mpi_integer,mpi_min,par%comm2d,ierr)
       if(ierr/=0)stop 'Err oasis_grid_msk_glb'
       if(par%rank==0) then !000000000000000000000000>
        status=nf_create('oasis_grid_msk.nc',nf_clobber,ncid_)   ; if(status/=0)write(10+par%rank,*)'err 3205'
        status=nf_def_dim(ncid_,'ni_t',max_x,i_t_dim)            ; if(status/=0)write(10+par%rank,*)'err 3208'
        status=nf_def_dim(ncid_,'nj_t',max_y,j_t_dim)            ; if(status/=0)write(10+par%rank,*)'err 3210'
        vardim(1)=i_t_dim ; vardim(2)=j_t_dim
        status=nf_def_var(ncid_,'lon',nf_double,2,vardim,var_id) ; if(status/=0)write(10+par%rank,*)'err 3313'
        status=nf_def_var(ncid_,'lat',nf_double,2,vardim,var_id) ; if(status/=0)write(10+par%rank,*)'err 3413'
        status=nf_def_var(ncid_,'oasis_grid_msk',nf_int,2  &
                                           ,vardim,var_id)       ; if(status/=0)write(10+par%rank,*)'err 3213'
        status=nf_enddef(ncid_)                                  ; if(status/=0)write(10+par%rank,*)'err 3215'

        status=nf_inq_varid(ncid_,'lon',var_id)                    ; if(status/=0)write(10+par%rank,*)'err 3322'
        status=nf_put_var_double(ncid_,var_id &
                                 ,ogcm_lon(1:max_x,1:max_y))       ; if(status/=0)write(10+par%rank,*)'err 3424'
        status=nf_inq_varid(ncid_,'lat',var_id)                    ; if(status/=0)write(10+par%rank,*)'err 3522'
        status=nf_put_var_double(ncid_,var_id &
                                  ,ogcm_lat(1:max_x,1:max_y))      ; if(status/=0)write(10+par%rank,*)'err 3624'
        status=nf_inq_varid(ncid_,'oasis_grid_msk',var_id)         ; if(status/=0)write(10+par%rank,*)'err 3222'
        status=nf_put_var_int(ncid_,var_id &
                             ,oasis_grid_msk_glb(1:max_x,1:max_y)) ; if(status/=0)write(10+par%rank,*)'err 3224'
        status=nf_close(ncid_)                                     ; if(status/=0)write(10+par%rank,*)'err 3217'
       endif                !000000000000000000000000>
       deallocate(oasis_grid_msk_glb)
       deallocate(oasis_grid_msk)
      endif                                   !m°v°m> !28-07-20
!.....OASIS...>
...
! note: parce que depth_w(i,j,kmin_w(i,j)) peut bouger avec le temps, mieux vaut prendre depth_w(i,j,1) !06-10-20
            if(ogcm_z(i2,j2,1)<depth_w(i,j,1          )) then !***> !06-10-20
...
! Lecture des valeurs des dimensions
      ...
      if(txt_   =='t') then !ttttttttttttttttttttttttttttttttttttttt>
       ...
       if(status/=0)status=nf_inq_dimid(ncid_   ,'xi_rho',dim_x_id) !13-09-21
       ...
       if(status/=0)status=nf_inq_dimid(ncid_   ,'eta_rho',dim_y_id) !13-09-21
       ...
       if(status/=0)status=nf_inq_dimid(ncid_   ,'s_rho',dim_z_id) !13-09-21
      ...
      if(txt_   =='u') then !uuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu>
       ...
       if(status/=0)status=nf_inq_dimid(ncid_   ,'xi_u',dim_x_id) !13-09-21
      ...
      if(txt_   =='v') then !vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv>
       ...
       if(status/=0)status=nf_inq_dimid(ncid_   ,'eta_v',dim_y_id) !13-09-21
...
!       call ogcm_get_dim(ncid_   ,'t') ! get max_x max_y max_z
        ...
        if(status/=0)status=nf_inq_varid(ncid_   ,'hsmth',var_id) !13-09-21
        if(status/=0)stop 'erreur identifiant bathymetrie'
        status=nf_inq_varndims(ncid_,var_id,ogcm_var_ndims)
...
! Ouvrir le fichier de grille:
        ...
        if(k0==nf_real) then !------->
         ...
         stop ' E3502 ogcm_z must be real' 
        endif                !------->
        if(status/=0) then !>>>>>>
...
!..........................................................................
      ...
      write(10+par%rank,*)'//////////////////////////////////////////////'
      if(t_or_s_==1)write(10+par%rank,*)'Pas de valeurs pour interpoler T'
      if(t_or_s_==2)write(10+par%rank,*)'Pas de valeurs pour interpoler S'
      write(10+par%rank,*)'sum1=',sum1
      write(10+par%rank,*)'sum2=',sum2
      write(10+par%rank,*)'par%rank i j k',par%rank,i,j,k
      write(10+par%rank,'(a,a)')'dom_c=',dom_c
      write(10+par%rank,*)'depth_w(k:k+1)',depth_w(i,j,k:k+1)
      write(10+par%rank,*)'ogcm_z1d min max=',ogcm_z1d(1),ogcm_z1d(max_z)
      write(10+par%rank,'(a,5i4)')'Indices de correspondances sous le fond=',i0,j0,i2,j2,k10
      write(10+par%rank,*)'Bathy ogcm en i2 j2:',ogcm_var2d(i2,j2)
      write(10+par%rank,*)'ogcm_z(i2,j2,1:max_z)=------>'
      do k_=max_z,1,-1
      write(10+par%rank,*)k_,ogcm_z(i2,j2,k_)
      enddo
...
! Load ogcm longitude:
       ...
       if(status/=0)status=nf_inq_varid(ncid_,'lon_rho',var_id) !13-09-21
       endif                       !tttttttttt>
...
! Load ogcm latitude:
       ...
       if(status/=0)status=nf_inq_varid(ncid_,'lat_rho',var_id) !13-09-21
       endif                       !tttttttttt>
       if(gridlocation_=='u') then !uuuuuuuuuu>
      ...
      if(par%rank==0) then !--[m0v0m]--> !05-05-18
       ...
       read(4,rec=nc_   )texte250,ogcm_time_counter           &
                                 ,ogcm_readtime_next(var_id_) &
                                   ...
                                   ,ogcmgridcode !16-05-19
! Si interpolation de 4 echeances, il faut lire le champs en nc_+1 (mais on garde ogcm_readtime_next,ogcm_period_next et ogcmgridcode de
! l'echeance nc_):
       if(obctime_order==4)read(4,rec=nc_+1)texte250,ogcm_time_counter !29-10-22
       close(4)
...
! Pour s'assurer:
! 1- que les niveaux de surface des 2 codes coincident bien,
! 2- et etendre la correction en 3D, c.a.d. recaller les profondeurs respectives vis 
!    A vis de la mauvaise prise en compte de la SSH (d'une part z du modele forcant 
!    est fixE A la valeur du fichier de grille, et d'autre part z de symphonie ne sait 
!    pas anticiper la SSH de la future echeance) 
! c'est z-ssh*pseudosigma que l'on considere et pas z, ou pseudosigma est une estimation simplifiee 
! (c.a.d. k/kmax) de la correction sigma associee A l'erreur sur SSH
...
! Note: cette routine correspond au cas nemo 16-09-20
...
! 2 passages:
! loop_=0: reperage pour determiner la dimension du tableau ogcmplugs
! loop_=1: renseigner le tableau ogcmplugs
      do loop_=0,1

      if(loop_==1) then !m°v°m>
! Si l'ogcm change en cours de simu, ogcmplugs est deja allouE !27-12-21
        if(allocated(ogcmplugs))deallocate(ogcmplugs)          !27-12-21
        allocate(ogcmplugs(ogcmmisspointmax,6))
      endif             !m°v°m>
      ogcmmisspointmax=0
      
      do 1862 k=max_z,1,-1 !16-09-20
      do 1862 j=1,max_y
      do 1862 i=1,max_x
...
!       if(k==max_z)write(3,'(4(i4,1x))')i,j,i4,j4
      ogcmmisspointmax=ogcmmisspointmax+1
      if(k==max_z)ogcmmisspointmax2d=ogcmmisspointmax !16-09-20

        if(loop_==1) then !m°v°m>
         ogcmplugs(ogcmmisspointmax,1)=i !26-03-20
         ogcmplugs(ogcmmisspointmax,2)=j
         ogcmplugs(ogcmmisspointmax,3)=i4
         ogcmplugs(ogcmmisspointmax,4)=j4
         ogcmplugs(ogcmmisspointmax,5)=k
         ogcmplugs(ogcmmisspointmax,6)=k4
        endif             !m°v°m>
...
!     close(3)
      enddo ! loop_=0,1
!     if(par%rank==0)write(6,*)'ogcmplugs dim ogcmmisspointmax=',ogcmmisspointmax
...
! Note: cette routine correspond au cas sympa 16-09-20
...
! loop_=1: renseigner le tableau ogcmplugs
      ...
      if(loop_==1)allocate(ogcmplugs(ogcmmisspointmax,4))
      ogcmmisspointmax=0
...
! note: dans le cas sympa ogcmmisspointmax2d=ogcmmisspointmax !16-09-20
      ogcmmisspointmax2d=ogcmmisspointmax                     !16-09-20
...
!     close(3)
      ...
      do k10=1,ogcmmisspointmax
      i=ogcmplugs(k10,1) ; j=ogcmplugs(k10,2) ; i4=ogcmplugs(k10,3) ; j4=ogcmplugs(k10,4)  !26-03-20
       do k=1,max_z
        ogcm_var3d(i,j,k)=ogcm_var3d(i4,j4,k)
            ogcm_z(i,j,k)=    ogcm_z(i4,j4,k)
       enddo
...
!     close(3)
      ...
      do k10=1,ogcmmisspointmax2d !16-09-20
      i=ogcmplugs(k10,1) ; j=ogcmplugs(k10,2) ; i4=ogcmplugs(k10,3) ; j4=ogcmplugs(k10,4) !26-03-20
       ogcm_var2d(i,j)=ogcm_var2d(i4,j4)
      enddo
...
!      read(3,*,end=1760)i,j,k,i4,j4,k4
      do k10=1,ogcmmisspointmax
      i=ogcmplugs(k10,1) ; j=ogcmplugs(k10,2) ; i4=ogcmplugs(k10,3) ; j4=ogcmplugs(k10,4) !26-03-20
      k=ogcmplugs(k10,5) ; k4=ogcmplugs(k10,6) 
